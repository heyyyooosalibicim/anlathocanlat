...
function saveAll(){
  ...
  var payload={playlists,activePlaylistId,currentVideoId,currentInPlaylist,currentPlaylistId,oopItems,currentSpeed,theme,autoPlayEnabled,sw:{accum:swEl,laps:swLaps}};
  /* DRIVE ASSET STORAGE: localStorage tüm aha_* key'leri arasında paylaşılan tek
     bir kotaya sahip — büyük bir PDF/görsel eki bu kotayı aşarsa eskiden bu
     fonksiyon sessizce (catch(e){}) HİÇBİR ŞEY kaydetmiyordu, kullanıcı fark
     etmeden o oturumdaki tüm değişiklikleri kaybediyordu. ahaSafeSetItem
     (features.js → "DRIVE ASSET STORAGE") önce normal yolu dener, kota aşılırsa
     büyük alanları otomatik Google Drive'a taşıyıp yeniden dener. features.js
     core.js'den SONRA yüklendiği için typeof ile kontrol ediyoruz — bu fonksiyon
     zaten yalnızca kullanıcı etkileşimiyle (runtime'da) çağrıldığından o ana
     kadar features.js çoktan yüklenmiş olur; typeof kontrolü sadece ekstra
     güvenlik/geri uyumluluk içindir. */
  if(typeof ahaSafeSetItem==='function'){
    ahaSafeSetItem(LS_DATA,payload);
  }else{
...
function loadSaved(){
  try{
    ...
    playlists.forEach(function(pl){
      ...
      pl.items.forEach(function(it){if(!it.watched)it.watched=false;if(it.watched&&!it.watchedAt)it.watchedAt=Date.now();if(!it.attachments)it.attachments=[];});
...
var _panels = {
  settings: {
    open: function(){ document.getElementById('settingsOverlay').classList.add('open'); _updateSettingsThemeBtns(); _updateSettingsAutoplay(); _updateSettingsSpeedBtns(); _updateSettingsGoogleUI(); _updateSettingsAiKeyUI(); _updateSettingsStopDailyHideToggle(); _updateSettingsSwIndependentToggle(); if(typeof _updateSettingsWaterToggle==='function') _updateSettingsWaterToggle(); },
  ...
  calendar: {
    open: function(){
      ...
      if(typeof _applyCalFullscreenPref==='function') _applyCalFullscreenPref();
      if(typeof switchCalTab==='function') switchCalTab(typeof calActiveTab!=='undefined'?calActiveTab:'planner');
    },
...
/* Ayarlar > "Günlük playlist & deneme gizlemesini durdur" — bkz. features.js
   checkMidnightReset() ve denemeApplyDailyCatHide(). Standalone bir anahtarda
   ("aha_stop_daily_hide_v1") tutulur — aha_v4_data'nın büyük JSON blob'una
   değil, autoPlayEnabled'ın aksine, çünkü su takibi ayarıyla (aha_water_enabled_v1)
   AYNI "basit bağımsız bayrak" desenini izler. */
function _updateSettingsStopDailyHideToggle(){
  var tog=document.getElementById('settingsStopDailyHideToggle');
  if(tog)tog.classList.toggle('on',stopDailyHideEnabled);
}
function toggleStopDailyHideFromSettings(){
  stopDailyHideEnabled=!stopDailyHideEnabled;
  try{localStorage.setItem('aha_stop_daily_hide_v1',stopDailyHideEnabled?'1':'0');}catch(e){}
  _updateSettingsStopDailyHideToggle();
  showToast(stopDailyHideEnabled?'⏸ Günlük gizleme durduruldu — playlistler ve deneme kategorileri artık gece yarısı gizlenmeyecek.':'🌅 Günlük gizleme tekrar açık — playlistler ve deneme kategorileri her gece yarısı gizlenecek.');
}
/* (2026-09) Ayarlar > "Kronometreyi Videodan Bağımsız Çalıştır" — bkz. handleStateChange().
   AYNI "basit bağımsız bayrak" deseni (stopDailyHideEnabled ile BİREBİR AYNI kalıp). */
function _updateSettingsSwIndependentToggle(){
  var tog=document.getElementById('settingsSwIndependentToggle');
  if(tog)tog.classList.toggle('on',swIndependentEnabled);
}
function toggleSwIndependentFromSettings(){
  swIndependentEnabled=!swIndependentEnabled;
  try{localStorage.setItem('aha_sw_independent_v1',swIndependentEnabled?'1':'0');}catch(e){}
  _updateSettingsSwIndependentToggle();
  showToast(swIndependentEnabled?'⏱ Kronometre artık videodan bağımsız — sadece S ile başlat/durdur.':'🔗 Kronometre tekrar videoya bağlı — video oynayınca başlar, durunca durur.');
}
...
function buildPlItem(item,i,cat){
  ...
  // Play (aynı videoya, o an oynatılırken tekrar tıklanırsa: ekranı boşalt/durdur — bir daha tıklanınca tekrar açılır)
  div.addEventListener('click',function(e){
    if(e.target.closest('.pl-item-actions')||e.target.classList.contains('pl-drag-handle')||e.target.classList.contains('pl-watched-chk'))return;
    var isPlayingNow=(currentVideoId===item.id&&currentInPlaylist&&currentPlaylistId===activePlaylistId);
    if(isPlayingNow){destroyPlayer();renderPlaylist();saveAll();return;}
    playPlaylistVideo(i);
  });
  return div;
}
...
/* ══════════ WATCHED ══════════
   (2026-09) İzlenme durumu artık KALICI — checkMidnightReset()/testMidnightReset()
   bunu bir daha sıfırlamıyor (bkz. features.js). Her işaretlemede watchedAt zaman
   damgası da yazılır/temizlenir; bu, arşivin "o gün kaç video izlendi" özetini
   (bkz. features.js → checkMidnightReset) cumulative durumdan bağımsız, GERÇEK
   günlük sayım olarak hesaplayabilmesi için gerekli — deneme.js'teki completedAt
   ile AYNI desen. */
...
function removeFromPlaylist(i){
  ...
  if(item&&item.attachments)item.attachments.forEach(function(a){if(typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(a.data)&&typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(a.data);});
}
function toggleWatchedByVideoId(vid){
  var pl=getActivePlaylist();if(!pl)return;
  var item=pl.items.find(function(x){return x.id===vid;});
  if(item){item.watched=!item.watched;item.watchedAt=item.watched?Date.now():null;renderPlaylist();renderTabs();saveAll();}
}
...
// Track Ctrl+S+F combo state — Tüm Verileri Sıfırla (Factory Reset) kısayolu.
// A+S kombosunun aksine sıra ÖNEMLİ DEĞİL (S önce de F önce de basılabilir) —
// her iki tuş da AYRI AYRI izlenir, Ctrl hâlâ basılıyken ikisi de true olduğu
// AN tetiklenir. Ctrl+S ("Kaydet") ve Ctrl+F ("Bul") tarayıcının kendi
// diyaloglarını açmasın diye HER İKİSİ de tek başına basıldığında bile
// preventDefault edilir (bkz. Ctrl+A+S'in aksine burada ayrıca gerekli,
// çünkü S/F browser'ın kendi çok yaygın kısayolları).
var _ctrlSHeldForReset=false, _ctrlFHeldForReset=false;
document.addEventListener('keydown',function(e){
  ...
  // Ctrl+S+F = Tüm Verileri Sıfırla (Factory Reset) — bkz. Ayarlar > ⚠️ Tehlikeli Bölge
  if(e.ctrlKey&&(e.key==='s'||e.key==='S')){ e.preventDefault(); _ctrlSHeldForReset=true; }
  if(e.ctrlKey&&(e.key==='f'||e.key==='F')){ e.preventDefault(); _ctrlFHeldForReset=true; }
  if(e.ctrlKey&&_ctrlSHeldForReset&&_ctrlFHeldForReset){
    _ctrlSHeldForReset=false;_ctrlFHeldForReset=false;
    confirmFullFactoryReset();
    return;
  }
...
document.addEventListener('keyup',function(e){if(!e.ctrlKey){_ctrlAHeld=false;_ctrlSHeldForReset=false;_ctrlFHeldForReset=false;}});
...
function testMidnightReset(){
  ...
  var hideStepText=stopDailyHideEnabled?'3. Playlistler GİZLENMİYOR (ayar: gizleme durduruldu)':'3. Playlistler gizleniyor (silinmiyor)';
  banner.innerHTML='<div style="font-size:18px;font-weight:800;color:#e84545;margin-bottom:8px">🕛 GECE YARISI TEST</div>'+
    ...
    hideStepText+'<br>'+
    '3b. "todolist" playlisti siliniyor<br>'+
  ...
  // --- checkMidnightReset ile AYNI KOD (bkz. features.js → BİRLEŞİK GÜNLÜK ARŞİV) ---
  // (2026-09) BUG FIX: bu test kısayolu stopDailyHideEnabled ayarını hiç kontrol
  // etmiyordu (playlistleri HER ZAMAN gizliyordu) ve "todolist" otomatik playlistini
  // hiç silmiyordu — artık checkMidnightReset() ile TUTARLI. Bu iki fonksiyon ayrı
  // yaşadığı için buraya yeni bir adım eklersen checkMidnightReset()'i de güncelle.
  // (2026-09) DEĞİŞİKLİK: videosWatched artık "o gün itibariyle toplam izlenmiş"
  // değil, "watchedAt'i tam olarak O GÜNE denk gelen" video sayısı (GERÇEK günlük
  // sayım) — çünkü watched artık kalıcı, cumulative bir sayaç günlük istatistik
  // olarak anlamsız olurdu (bkz. checkMidnightReset, features.js — İKİSİ AYNI
  // MANTIĞI kullanmalı, Altın Kural #18). videosWatchedTotal ise o ana kadarki
  ...
  saveArchive();
  swReset();
  // (2026-09) BUG FIX — checkMidnightReset() ile TUTARLI: zaman damgası
  // notları (pl.notes) artık burada da sıfırlanmıyor, SADECE izlenme durumu
  // (bkz. features.js → checkMidnightReset() üstündeki açıklama; Altın Kural
  // #18'deki "iki fonksiyon senkron kalmalı" dersi burada da uygulandı).
  // (2026-09) YENİ DEĞİŞİKLİK — kullanıcı isteğiyle: 'watched' alanı ARTIK
  // burada da sıfırlanmıyor (kalıcı hale getirildi, bkz. checkMidnightReset).
  // SADECE playlist GÖRÜNÜRLÜĞÜ (hidden) hâlâ ayara bağlı olarak sıfırlanıyor.
  if(!stopDailyHideEnabled){
    playlists.forEach(function(pl){ pl.hidden=true; });
  }
  // "todolist" playlist'i kalıcı değildir — bu ayardan BAĞIMSIZ, her zaman silinir
  // (bkz. TODO_AUTO_PLAYLIST_NAME / features.js checkMidnightReset).
...
/* ══════════ TAM SIFIRLAMA (Factory Reset) ══════════
   Uygulamadaki TÜM kullanıcı verilerini kalıcı olarak siler — playlistler,
   izleme geçmişi/arşiv, notlar, flashcard'lar, deneme (pratik sınav) kayıtları,
   AI sohbet geçmişi, API anahtarları, su takibi verisi, TÜM ayarlar dahil —
   ve sayfayı yeniden yükleyerek uygulamayı sanki ilk kez açılmış gibi bir
   duruma getirir. Ctrl+S+F kısayolu VEYA Ayarlar > ⚠️ Tehlikeli Bölge >
   "Tüm Verileri Sıfırla" butonundan tetiklenir.

   ÖNEMLİ — checkMidnightReset() İLE KARIŞTIRMA: gece yarısı sıfırlaması SADECE
   o günkü playlist izleme durumunu gizler/arşivler; deneme kayıtları (Altın
   Kural #6), notlar, flashcard'lar vb. ASLA silinmez. BU fonksiyon ise
   TAMAMEN FARKLI bir şey — kullanıcının AÇIKÇA istediği, GERİ ALINAMAZ bir
   TAM silmedir; deneme kayıtları dahil gerçekten HER ŞEYİ siler.

   Anahtar listesi ELLE TUTULMUYOR — localStorage'daki 'aha_' önekli HER
   anahtar (artı birkaç adı bilinen yardımcı anahtar) otomatik toplanıp
   silinir. Bu bilinçli bir tercih: yeni bir 'aha_*' anahtarı eklendiğinde
   (örn. ai.js/deneme.js/music.js içinde) bu fonksiyona AYRICA eklemeyi
   unutmak, o anahtarın sıfırlamadan sağ çıkıp "ilk açılış" görünümünü
   bozmasına yol açardı — tıpkı BACKUP_KEYS'e yeni bir anahtar eklemeyi
   unutmanın Drive yedeklemesini eksik bırakması gibi (bkz. README Altın
   Kural #13). Prefix taraması bu sınıf hatayı yapısal olarak imkansız kılar. */
function confirmFullFactoryReset(){
  if(!confirm('⚠️ TÜM uygulama verileri kalıcı olarak silinecek:\n\n• Playlistler ve izleme geçmişi/arşiv\n• Notlar\n• Flashcard\'lar\n• Deneme (pratik sınav) kayıtların\n• AI sohbet geçmişi ve API anahtarların\n• Su takibi verisi ve TÜM ayarların\n\nUygulama ilk açılış hâline dönecek. Bu işlem GERİ ALINAMAZ.\n\nDevam etmek istediğine emin misin?')) return;
  performFullFactoryReset();
}
function performFullFactoryReset(){
  try{
    ...
    for(var i=0;i<localStorage.length;i++){
      ...
      if(k&&(k.indexOf('aha_')===0||k==='ytSearchCount')) keysToRemove.push(k);
    }
    keysToRemove.forEach(function(k){ try{ localStorage.removeItem(k); }catch(e){} });
  }catch(e){}
  showToast('🗑 Tüm veriler sıfırlandı — uygulama yeniden başlatılıyor...');
...
function destroyPlayer(){
  ...
  _todoActiveVideoTask=null;_mainVideoDuration=0;_autoWatchDoneVidId=null;
}
function startPoll(iframe){
  ...
  pollTimer=setInterval(function(){
    ...
    try{
      ...
      f.contentWindow.postMessage(JSON.stringify({event:'command',func:'getDuration',args:[],id:1}),'*');
      f.contentWindow.postMessage(JSON.stringify({event:'command',func:'getVideoData',args:[],id:1}),'*');
    }catch(err){}
  },500);
}
/* (2026-09) Planlayıcıdan açılan bir videoyu %90 izleyince görevi otomatik
   "yapıldı" işaretlemek için: hangi görevin hangi videoyu izlediğini tutar
   (bkz. todoOpenVideoRef / features.js) — video değişince veya ekran
   boşalınca sıfırlanır ki başka bir video yanlışlıkla bu görevi tetiklemesin. */
var _todoActiveVideoTask=null; // {dateStr,itemId,vidId}
var _mainVideoDuration=0;
function _todoCheckVideoAutoComplete(curSec){
  if(!_todoActiveVideoTask||!_mainVideoDuration||_mainVideoDuration<=0)return;
  if(_todoActiveVideoTask.vidId!==currentVideoId)return;
  if((curSec/_mainVideoDuration)<0.9)return;
  var ref=_todoActiveVideoTask;
  _todoActiveVideoTask=null; // tek seferlik — tekrar tetiklenmesin
  if(typeof _todoAutoCompleteItem==='function') _todoAutoCompleteItem(ref.dateStr,ref.itemId);
}
/* (2026-09) YENİ ÖZELLİK — Playlist videosu %90 izlenince (veya bitince, bkz.
   handleStateChange state===0) otomatik "izlendi" işaretlensin. Yukarıdaki
   _todoCheckVideoAutoComplete ile AYNI infoDelivery/duration-oranı mantığını
   kullanır ama TAMAMEN bağımsızdır — planlayıcıdan açılmış olmasına gerek
   YOK, aktif playlistteki HERHANGİ bir video için çalışır. _autoWatchDoneVidId
   video başına tek seferlik tetiklenmeyi garanti eder (aksi halde her 500ms'lik
   poll'da tekrar tekrar saveAll() çağrılırdı). */
var _autoWatchDoneVidId=null;
function _checkAutoMarkWatched(curSec){
  if(!currentInPlaylist||currentPlaylistId!==activePlaylistId)return;
  if(!currentVideoId||_autoWatchDoneVidId===currentVideoId)return;
  if(!_mainVideoDuration||_mainVideoDuration<=0)return;
  if((curSec/_mainVideoDuration)<0.9)return;
  var pl=getActivePlaylist();if(!pl)return;
  var item=pl.items.find(function(x){return x.id===currentVideoId;});
  if(!item||item.watched)return;
  _autoWatchDoneVidId=currentVideoId;
  item.watched=true;
...
window.addEventListener('message',function(e){
  ...
  if(data.event==='infoDelivery'&&data.info){
    ...
    if(typeof info.duration==='number'&&info.duration>0)_mainVideoDuration=info.duration;
    if(info.currentTime!==undefined){_todoCheckVideoAutoComplete(info.currentTime);_checkAutoMarkWatched(info.currentTime);}
...
function handleVideoChange(newVid,newTitle){
  /* Planlayıcıdan izlenen görev bağlantısı — video değiştiyse (artık farklı bir
     video oynuyorsa) eski görevi tetiklemesin diye bağlantıyı düşür. */
  if(_todoActiveVideoTask&&_todoActiveVideoTask.vidId!==newVid)_todoActiveVideoTask=null;
  if(_autoWatchDoneVidId&&_autoWatchDoneVidId!==newVid)_autoWatchDoneVidId=null;
  _mainVideoDuration=0;
...
function handleStateChange(state){
  // MODÜL 2 – Pomodoro çakışma önleyici: pomodoro aktifken video oynatıldığında
  // normal kronometre başlatma tetiklenmesin.
  if(state===1 && typeof pomodoroActive !== 'undefined' && pomodoroActive){
    // Hız ayarını yine de uygula, sadece sw başlatma atlanır
    if(currentSpeed!==1) setTimeout(function(){ applySpeed(currentSpeed,true); },300);
    /* Music bridge */
    if(typeof onMainVideoStart==='function') onMainVideoStart();
    return;
  }
  // (2026-09) Ayarlar > "Kronometreyi Videodan Bağımsız Çalıştır" açıksa video
  // oynatma/duraklatma/bitiş durumu kronometreyi hiç etkilemesin — kronometre
  // sadece manuel S tuşu/butonuyla (swToggle) kontrol edilir.
  if(!swIndependentEnabled){
    if(state===1&&!swRunning)swStart();
    if(state===2&&swRunning)swPause();
    if(state===0)swPause();
  }
  if(state===0){
    // (2026-09) YENİ ÖZELLİK — video bitince (state===0/ended) autoPlayEnabled
    // AÇIK/KAPALI fark etmeksizin aktif playlistteki video otomatik "izlendi"
    // işaretlenir. %90 eşiği zaten _checkAutoMarkWatched ile (infoDelivery poll,
    // 500ms) yakalanıyor olmalı ama bu, videonun tam bitişini bağımsız bir
    // güvenlik ağı olarak da garanti eder (ör. çok kısa video / poll'un henüz
    // yetişemediği bir kapanış anı).
    if(currentInPlaylist&&currentPlaylistId===activePlaylistId){
      var plW=getActivePlaylist();
      if(plW){
        var itW=plW.items.find(function(x){return x.id===currentVideoId;});
        if(itW&&!itW.watched){itW.watched=true;itW.watchedAt=Date.now();_autoWatchDoneVidId=currentVideoId;renderPlaylist();renderTabs();saveAll();}
      }
    }
...
/* MODÜL 2 – Timer Fullscreen (CSS class tabanlı)
   Element.requestFullscreen() KASITLI OLARAK KULLANILMIYOR: başta iPhone/iPad
   Safari olmak üzere birçok tarayıcıda rastgele bir <div> için desteklenmiyor
   ya da tutarsız çalışıyor — çağrı sessizce reddediliyor, panel de küçük
   haliyle sidebar'da kalıyordu. Onun yerine panele .sw-fs-active class'ı
   eklenip position:fixed + vmin tabanlı bir CSS layout'a geçiriliyor (bkz.
   styles.css). Bu, native API'den bağımsız olduğu için hangi cihaz/tarayıcıdan
   girilirse girilsin (telefon dik, tablet yan yatırılmış vs.) aynı şekilde
   çalışır. */
(function(){
  var _swFsActive = false;

  function _swFsSet(on){
    var timerArea = document.querySelector('.stopwatch-panel');
    var btn = document.getElementById('timerFullscreenBtn');
    if(!timerArea) return;
    _swFsActive = on;
    timerArea.classList.toggle('sw-fs-active', on);
    if(btn){
      var icon = btn.querySelector('.material-symbols-outlined');
      if(icon) icon.textContent = on ? 'close_fullscreen' : 'open_in_full';
      btn.title = on ? 'Tam Ekrandan Çık' : 'Kronometreyi Tam Ekran Yap';
    }
  }

  function _initTimerFullscreen(){
    var btn = document.getElementById('timerFullscreenBtn');
    if(!btn) return;
    btn.addEventListener('click', function(){
      _swFsSet(!_swFsActive);
    });
    // Masaüstünde ESC ile de çıkılabilsin (dokunmatik cihazda zaten
    // sağ üstteki buton her zaman erişilebilir durumda)
    document.addEventListener('keydown', function(e){
      if(e.key === 'Escape' && _swFsActive) _swFsSet(false);
    });
  }
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', _initTimerFullscreen);
  } else {
    _initTimerFullscreen();
  }
})();
