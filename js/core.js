  ...
  function _initTimerFullscreen(){
    ...
    btn.addEventListener('click', function(){
      ...
      if(!document.fullscreenElement){
        timerArea.requestFullscreen().catch(function(err){ console.warn('Fullscreen error:', err); });
      } else {
        document.exitFullscreen();
      }
    });
    // Fullscreen değişince ikon güncelle
    document.addEventListener('fullscreenchange', function(){
      var icon = btn.querySelector('.material-symbols-outlined');
      if(icon) icon.textContent = document.fullscreenElement ? 'close_fullscreen' : 'open_in_full';
    });
  }
