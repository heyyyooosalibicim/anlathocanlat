...
function _noteTryExitEmptyBlockquoteOnEnter(){
  ...
  var bq = node.nodeType === 3 ? node.parentNode : node;
  while(bq && bq !== content && bq.tagName !== 'BLOCKQUOTE') bq = bq.parentNode;
  if(!bq || bq === content || bq.tagName !== 'BLOCKQUOTE') return false;
  ...
  // kendisine kadar çıkar — o durumda "satır" = blockquote'un TAMAMI demektir.
  var line = node.nodeType === 3 ? node.parentNode : node;
  while(line && line !== bq && line.parentNode !== bq) line = line.parentNode;
  if(!line) return false;

  var isWholeBlockquote = (line === bq);
  var checkNode = isWholeBlockquote ? bq : line;
  var lineText = (checkNode.textContent || '').replace(/\u200B/g, '').trim();
  if(lineText !== '') return false; // gerçek içerik var, normal Enter davransın

  // Boş satır bulundu → blockquote'tan çık. Yeni bir paragraf oluşturup
  // blockquote'tan hemen sonraya yerleştir.
  var p = document.createElement('div');
  p.innerHTML = '<br>';
  if(!bq.parentNode) return false;
  bq.parentNode.insertBefore(p, bq.nextSibling);

  if(isWholeBlockquote){
    // Blockquote'un içi zaten tamamen boş — tüm blockquote'u kaldır.
    bq.parentNode.removeChild(bq);
  } else {
    // Sadece son (boş) satırı kaldır; blockquote'un diğer satırları kalsın.
    if(line.parentNode) line.parentNode.removeChild(line);
    // Bu satırı kaldırınca blockquote tamamen boş kaldıysa onu da temizle.
    if(!bq.parentNode){ /* zaten kaldırılmış olabilir, no-op */ }
    else if((bq.textContent || '').trim() === ''){
      bq.parentNode.removeChild(bq);
    }
  }
