...
function _applyPendingNoteStyleToInput(e){
  ...
  if(result.node.parentNode) _mergeAdjacentSameStyleSpans(result.node.parentNode.parentNode);

  // NADİR EDGE CASE GÜVENLİĞİ: merge sırasında result.node teorik olarak bir
  // komşusuna emilip DOM'dan çıkmış olabilir (örn. caret metnin ORTASINDAYKEN
  // stil değişip hemen sağında rastlantısal olarak AYNI stilde içerik varsa).
  // Böyle bir durumda YANLIŞ bir konuma zorlamak yerine mevcut seçimi
  // olduğu gibi bırakmak daha güvenlidir.
...
function _mergeAdjacentSameStyleSpans(container){
  if(!container) return;
  ...
  while(child){
    ...
    if(child.nodeType === 1 && child.classList && child.classList.contains('note-style-span') &&
      ...
      continue; // aynı child'ı BİR SONRAKİ komşuyla tekrar dene (3'lü+ zincir birleşmesi için)
    }
    child = next;
  }
}
...
function _applyStyleToRange(range, prop, value){
  ...
  _mergeAdjacentSameStyleSpans(container);
  return { firstSpan: resultSpans[0], lastSpan: resultSpans[resultSpans.length - 1] };
}
