...
function changeAiKey(){
  ...
  if(!confirm('API anahtarından çıkış yapmak (silmek) istediğine emin misin?')) return;
  clearAiKey();
  showToast('🚪 API anahtarından çıkış yapıldı.');
  renderAiBody();
  if(typeof _updateSettingsAiKeyUI==='function') _updateSettingsAiKeyUI();
}
...
function saveAiChat(){
  /* BUG FIX: eskiden burada çıplak bir setItem vardı — kota aşılınca
     YAKALANMAMIŞ bir hata fırlatıp sessizce (ya da çağrı zincirini kırarak)
     başarısız oluyordu. Sohbet metin olduğu için normalde ahaSafeSetItem'ın
     ikili-offload'u burada bir şey BULAMAZ (taşınacak PDF/görsel yok) — asıl
     güvenlik ağı TIER-2: hâlâ sığmıyorsa en eski mesajların bir kısmını tek
     bir Drive dosyasına arşivler (bkz. ahaArchiveOldChatMessages), hiçbir
     mesajı SİLMEDEN. */
  if(typeof ahaSafeSetItem !== 'function'){
    try{ localStorage.setItem(LS_AI_CHAT, JSON.stringify(aiChatHistory)); }catch(e){}
    return;
  }
  ahaSafeSetItem(LS_AI_CHAT, aiChatHistory).then(function(ok){
    if(ok)return;
    if(typeof ahaArchiveOldChatMessages !== 'function')return;
    ahaArchiveOldChatMessages().then(function(archived){
      if(archived) ahaSafeSetItem(LS_AI_CHAT, aiChatHistory);
    });
  });
}

/* — TIER-2 SON ÇARE: eski sohbet mesajlarını Drive'a arşivle —
   Sadece ahaSafeSetItem'ın ikili-offload'u HİÇBİR ŞEY bulamadığında (çünkü
   sohbet düz metindir) VE hâlâ sığmadığında çağrılır — yani gerçekten nadir
   bir "son çare". aiChatHistory[0] {archived:true,count,data:pointer} biçiminde
   TEK bir işaretleyiciyse, üzerine ekleyerek (indirip birleştirip tekrar
   yükleyerek) büyütür; yoksa yeni bir işaretleyici oluşturur. Hiçbir mesaj
   KAYBOLMAZ — sadece nadiren, gerektiğinde Drive'a taşınır ve istenince
   (aiShowArchivedChat) geri okunabilir. */
function ahaArchiveOldChatMessages(){
  if(!Array.isArray(aiChatHistory))return Promise.resolve(false);
  if(typeof ahaUploadAssetToDrive!=='function'||typeof ahaResolveAssetData!=='function')return Promise.resolve(false);
  var hasMarker=aiChatHistory.length>0&&aiChatHistory[0]&&aiChatHistory[0].archived===true;
  var realMsgs=hasMarker?aiChatHistory.slice(1):aiChatHistory.slice();
  if(realMsgs.length<20)return Promise.resolve(false); // arşivlemeye değecek kadar mesaj yok, boşuna Drive'a gitme
  var cut=Math.floor(realMsgs.length*0.6); // en eski %60'ı arşivle, en yeni %40 hızlı erişim için yerelde kalsın
  var toArchive=realMsgs.slice(0,cut);
  var keepLocal=realMsgs.slice(cut);
  var oldMarker=hasMarker?aiChatHistory[0]:null;

  function finish(mergedArr){
    var json=JSON.stringify(mergedArr);
    var dataUrl='data:application/json;base64,'+btoa(unescape(encodeURIComponent(json)));
    return ahaUploadAssetToDrive(dataUrl,'ai_chat_archive.json','application/json').then(function(ptr){
      if(oldMarker&&oldMarker.data&&typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(oldMarker.data);
      aiChatHistory=[{role:'_archive',archived:true,count:mergedArr.length,data:ptr}].concat(keepLocal);
      return true;
    });
  }

  if(hasMarker){
    return ahaResolveAssetData(oldMarker).then(function(oldJsonDataUrl){
      var oldArr=[];
      try{ oldArr=JSON.parse(decodeURIComponent(escape(atob(oldJsonDataUrl.split(',')[1])))); }catch(e){}
      return finish(oldArr.concat(toArchive));
    }, function(){ return finish(toArchive); }); // eski arşiv okunamazsa bile en azından yeni parçayı kaybetmeden arşivle
  }
  return finish(toArchive);
}

/* Arşivlenmiş eski mesajları getirip (salt-okunur olarak) sohbet ekranına ekler.
   aiChatHistory'ye GERİ BİRLEŞTİRMEZ — yoksa hemen tekrar arşivlenmesi
   gerekebilir; sadece görüntüleme amaçlı, yerinde bir DOM ekleme yapar. */
function aiShowArchivedChat(markerIdx){
  var marker=aiChatHistory[markerIdx];
  if(!marker||!marker.archived)return;
  if(typeof ahaResolveAssetData!=='function')return;
  showToast('⏳ Eski mesajlar Drive\'dan getiriliyor…');
  ahaResolveAssetData(marker).then(function(jsonDataUrl){
    var arr=[];
    try{ arr=JSON.parse(decodeURIComponent(escape(atob(jsonDataUrl.split(',')[1])))); }catch(e){}
    ...
    var html=arr.map(function(m){
      ...
      return '<div class="ai-msg '+cls+'">'+(m.role==='user'?escapeHtml(txt):renderAiMdSafe(txt))+'</div>';
    }).join('');
    var markerEl=box.children[markerIdx];
    if(markerEl){
      var wrap=document.createElement('div');
      wrap.innerHTML=html;
      var nodes=Array.prototype.slice.call(wrap.childNodes);
      nodes.forEach(function(n){ markerEl.parentNode.insertBefore(n, markerEl); });
      markerEl.remove();
    }
  }, function(err){
    showToast('❌ '+(err&&err.message?err.message:'Eski mesajlar yüklenemedi.'));
  });
}
...
function saveAiFlashcards(){
  /* DRIVE ASSET STORAGE: eskiden buradaki çıplak setItem kota aşılınca
     YAKALANMAMIŞ bir hata fırlatıyordu (sessiz veri kaybından bile kötü —
     çağrı zincirini kırabilirdi). ahaSafeSetItem önce normal yolu dener,
     kota aşılırsa büyük ekran görüntülerini (c.img — bkz. features.js
     "DRIVE ASSET STORAGE" kind:'rawimg') otomatik Drive'a taşıyıp
     yeniden dener. */
  if(typeof ahaSafeSetItem === 'function'){
    ahaSafeSetItem(LS_AI_CARDS, aiFlashcards);
  } else {
    try{ localStorage.setItem(LS_AI_CARDS, JSON.stringify(aiFlashcards)); }catch(e){
      showToast('❌ Kartlar kaydedilemedi (depolama alanı dolu olabilir).');
    }
  }
}
...
function saveAiCategories(){ try{ localStorage.setItem(LS_AI_CATS, JSON.stringify(aiCategories)); }catch(e){ showToast('❌ Kategoriler kaydedilemedi (depolama alanı dolu olabilir).'); } }
...
var LS_AI_FULLSCREEN = 'aha_ai_fullscreen_v1';

function _applyAiFullscreenPref(){
  var ov = document.getElementById('aiPanelOverlay');
  var icon = document.getElementById('aiFullscreenIcon');
  if(!ov) return;
  var on = localStorage.getItem(LS_AI_FULLSCREEN) === '1';
  ov.classList.toggle('ai-fullscreen', on);
...
function toggleAiFullscreen(){
  ...
  var icon = document.getElementById('aiFullscreenIcon');
  if(!ov) return;
  var on = !ov.classList.contains('ai-fullscreen');
  ov.classList.toggle('ai-fullscreen', on);
  localStorage.setItem(LS_AI_FULLSCREEN, on ? '1' : '0');
  if(icon) icon.textContent = on ? 'close_fullscreen' : 'open_in_full';
}
function openAiPanel(){
  document.getElementById('aiPanelOverlay').classList.add('open');
  _applyAiFullscreenPref();
  renderAiBody();
}
...
function switchAiTab(tab){
  aiActiveTab = tab;
  ['chat','create','plan','deck','cats'].forEach(function(t){
...
function renderAiBody(){
  ...
  if(aiActiveTab==='chat'){
    ...
    setupAiCreateTab();
  } else if(aiActiveTab==='plan'){
    body.innerHTML = aiPlanHtml();
    setupAiPlanTab();
...
function renderAiChatMessages(){
  ...
  box.innerHTML = aiChatHistory.map(function(m,idx){
    if(m.archived){
      return '<div class="ai-msg-archive-marker" onclick="aiShowArchivedChat('+idx+')" style="text-align:center;font-size:11px;color:var(--muted);padding:8px;cursor:pointer;border:1px dashed var(--border2);border-radius:8px;margin-bottom:8px">📦 '+m.count+' eski mesaj Drive\'da saklanıyor — görüntülemek için tıkla</div>';
    }
    var cls = m.role==='user' ? 'ai-msg-user' : 'ai-msg-bot';
    var txt = (m.parts||[]).map(function(p){ return p.text || ''; }).join('');
    if(m.role==='user'){
...
function sendAiChatMessage(){
  ...
  var apiHistory = aiChatHistory.filter(function(m){ return !m.archived; }); // arşiv işaretleyicisi API'ye GİTMEMELİ — gerçek bir mesaj değil
  callGeminiAPI(apiHistory, AI_CHAT_SYSTEM, false, grounding).then(function(reply){
...
function clearAiChat(){
  ...
  if(aiChatHistory[0]&&aiChatHistory[0].archived&&typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(aiChatHistory[0].data);
  aiChatHistory = [];
  saveAiChat();
  renderAiBody();
}
...
/* Ctrl+V ile ekran görüntüsü yapıştırma — panel açıkken ve "Flashcard Oluştur"
   YA DA "Plan Tanıma" sekmesindeyken, hangisi aktifse o sekmenin resmine gider. */
function handleGlobalAiPaste(e){
  if(aiActiveTab!=='create' && aiActiveTab!=='plan') return;
  ...
  for(var i=0;i<items.length;i++){
    if(items[i].type.indexOf('image')!==-1){
      if(aiActiveTab==='plan') readAiPlanImageFile(items[i].getAsFile());
      else readAiImageFile(items[i].getAsFile());
      e.preventDefault();
      return;
    }
  }
...
/* ══════════════════════════════════════════════════════════════════════════
 * PLAN TANIMA — 2026-08
 * Flashcard-oluşturmadaki görsel-yorumlama akışının AYNISINI (callGeminiAPI
 * jsonMode) kullanarak, kullanıcının yüklediği bir HAFTALIK PLAN fotoğrafını
 * (defter, ajanda, ekran görüntüsü) analiz edip features.js'teki to-do list
 * sistemine (todosData) otomatik dolduran özellik.
 *
 * Akış: fotoğraf yükle → hafta seç (varsayılan: bu hafta) → "Analiz Et" →
 * Gemini görseldeki HER GÜN için etkinlikleri JSON olarak döndürür → gün
 * isimleri seçilen haftanın gerçek tarihlerine eşlenir → BUGÜNDEN ÖNCEKİ
 * günler (ve 30 günlük pencerenin dışına taşan günler) otomatik filtrelenip
 * kullanıcıya açıkça belirtilir → kullanıcı önizlemede etkinlik bazında
 * checkbox'larla ince ayar yapıp onaylar → todosData'ya yazılır (bkz.
 * features.js → todoAddItem'daki AYNI veri modeli, source:'ai' farkıyla).
 * ════════════════════════════════════════════════════════════════════════ */
var aiPlanImage = null;          // {mime, data} — "Plan Tanıma" sekmesinde yüklenen fotoğraf
var aiPlanSelectedWeekStart = null; // seçili haftanın Pazartesi'si, 'YYYY-MM-DD'
var aiPlanBusy = false;
var aiPlanParsedResult = null;   // Gemini'den dönen ham {gunler:[...]}
var aiPlanPreview = null;        // [{_idx, dateStr, gunAdi, notAddableReason, items:[{baslik,tur,ders,dakika,saat,_include}]}]

var AI_PLAN_DOW_TO_INDEX = {'Pazar':0,'Pazartesi':1,'Salı':2,'Çarşamba':3,'Perşembe':4,'Cuma':5,'Cumartesi':6};

var AI_PLAN_PROMPT = 'Bu görsel bir öğrencinin HAFTALIK ÇALIŞMA PLANI (defter sayfası, ajanda, tablo, elle yazılmış liste, ekran görüntüsü — her türlü olabilir). '+
  'Görseldeki HER GÜN için, o gün yapılması planlanan etkinlikleri çıkar. '+
  'Her gün nesnesinde "gun" alanına SADECE şu 7 değerden birini yaz (Türkçe, tam bu yazımla): "Pazartesi","Salı","Çarşamba","Perşembe","Cuma","Cumartesi","Pazar". '+
  'Görselde gün isimleri yazmıyorsa ama günler sırayla üstten alta veya soldan sağa dizilmişse, sırayı Pazartesi\'den başlat ve mantıken devam ettir. '+
  'Her etkinlik için: '+
  '"baslik" (kısa ve net, ne yapılacağı — örn: "Türev konusu tekrar", "3 test çöz", "Kimya videosunu bitir"), '+
  '"tur" (SADECE şu 4 değerden biri: "study" = ders çalışma/konu tekrarı/soru-test çözme, "video" = video izleme görevi, "deneme" = deneme/prova sınavı, "custom" = yukarıdakilere uymayan diğer/genel görev), '+
  '"ders" (belirtilmişse ders adı, örn "Biyoloji"; yoksa null), '+
  '"dakika" (süre belirtilmişse SAYI olarak dakika cinsinden, örn "30 dk" → 30; yoksa null), '+
  '"saat" (saat belirtilmişse "SS:DD" formatında 24 saatlik, örn "14:30"; yoksa null). '+
  'Görselde gerçekten karşılığı OLMAYAN bir günü veya etkinliği UYDURMA. '+
  'SADECE şu JSON formatında yanıt ver, başında/sonunda hiçbir açıklama veya markdown OLMASIN: '+
  '{"gunler":[{"gun":"Pazartesi","etkinlikler":[{"baslik":"...","tur":"study","ders":"...","dakika":30,"saat":"09:00"}]}]}';

function aiPlanHtml(){
  var weeks = _aiPlanGetWeekOptions();
  if(!aiPlanSelectedWeekStart || !weeks.some(function(w){return w.startStr===aiPlanSelectedWeekStart;})){
    aiPlanSelectedWeekStart = weeks.length ? weeks[0].startStr : null;
  }
  var weekOptions = weeks.map(function(w){
    return '<option value="'+w.startStr+'"'+(w.startStr===aiPlanSelectedWeekStart?' selected':'')+'>'+w.label+'</option>';
  }).join('');
  var html = '<div class="ai-plan-intro">📅 Haftalık çalışma planının fotoğrafını yükle (defter, ajanda, ekran görüntüsü — hepsi olur), yapay zeka gün gün etkinlikleri seçtiğin haftaya otomatik işlesin. Bugünden önceki günler otomatik atlanır.</div>';
  html += '<div class="ai-plan-week-row"><label for="aiPlanWeekSelect">Hangi haftaya eklensin?</label>'+
    '<select id="aiPlanWeekSelect" onchange="aiPlanWeekChanged()">'+weekOptions+'</select></div>';
  html += '<div class="ai-fc-drop" id="aiPlanDrop"><div id="aiPlanDropContent"></div>'+
    '<input type="file" id="aiPlanFileInput" accept="image/*" style="display:none"></div>';
  html += '<button class="ai-fc-gen-btn" id="aiPlanAnalyzeBtn" onclick="analyzeAiPlanImage()"'+(aiPlanBusy?' disabled':'')+'>'+
    (aiPlanBusy?'⏳ Analiz ediliyor…':'🔍 Planı Analiz Et')+'</button>';
  html += '<div id="aiPlanPreviewArea">'+_aiPlanRenderPreview()+'</div>';
  return html;
}

function setupAiPlanTab(){
  renderAiPlanDropContent();
  var drop = document.getElementById('aiPlanDrop');
  var fileInput = document.getElementById('aiPlanFileInput');
  if(!drop) return;
  drop.addEventListener('click', function(e){
    if(e.target.closest('.ai-fc-img-remove')) return;
    fileInput.click();
  });
  drop.addEventListener('dragover', function(e){ e.preventDefault(); drop.classList.add('dragover'); });
  drop.addEventListener('dragleave', function(){ drop.classList.remove('dragover'); });
  drop.addEventListener('drop', function(e){
    e.preventDefault();
    drop.classList.remove('dragover');
    if(e.dataTransfer.files && e.dataTransfer.files[0]) readAiPlanImageFile(e.dataTransfer.files[0]);
  });
  fileInput.addEventListener('change', function(){
    if(fileInput.files && fileInput.files[0]) readAiPlanImageFile(fileInput.files[0]);
  });
}

function renderAiPlanDropContent(){
  var content = document.getElementById('aiPlanDropContent');
  if(!content) return;
  if(aiPlanImage){
    content.innerHTML = '<div class="ai-fc-img-preview">'+
        '<img src="data:'+aiPlanImage.mime+';base64,'+aiPlanImage.data+'" alt="plan fotoğrafı">'+
        '<span class="ai-fc-img-remove" title="Kaldır" onclick="removeAiPlanImage(event)">✕</span>'+
      '</div>'+
      '<div style="margin-top:8px;font-size:11px;">Başka bir fotoğraf yapıştırmak (Ctrl+V) veya seçmek için tıkla</div>';
  } else {
    content.innerHTML = '📋 Plan fotoğrafını buraya yapıştır (Ctrl+V)<br><span style="font-size:11px">veya tıkla, dosya seç</span>';
  }
}
function removeAiPlanImage(e){
  if(e) e.stopPropagation();
  aiPlanImage = null;
  renderAiPlanDropContent();
}
function readAiPlanImageFile(file){
  if(!file) return;
  var reader = new FileReader();
  reader.onload = function(){
    var m = /^data:(image\/[a-zA-Z0-9+.-]+);base64,(.*)$/.exec(reader.result);
    if(!m) return;
    aiPlanImage = { mime:m[1], data:m[2] };
    renderAiPlanDropContent();
  };
  reader.readAsDataURL(file);
}

/* ── Hafta seçenekleri: bugünün haftasından başlayıp to-do list'in 30 günlük
   penceresini (bkz. features.js → _todoWindowEndStr) aşmayacak kadar ── */
function _aiPlanGetWeekOptions(){
  var today = new Date();
  var todayDow = today.getDay(); // 0=Pazar
  var mondayOffset = (todayDow===0) ? -6 : (1-todayDow);
  var thisMonday = new Date(today); thisMonday.setDate(today.getDate()+mondayOffset);
  var windowEnd = (typeof _todoWindowEndStr==='function') ? _todoWindowEndStr() : null;
  var weeks = [];
  for(var w=0; w<6; w++){
    var mon = new Date(thisMonday); mon.setDate(thisMonday.getDate()+w*7);
    var monStr = getDateStr(mon);
    if(windowEnd && monStr>windowEnd) break;
    var sun = new Date(mon); sun.setDate(mon.getDate()+6);
    var tag = w===0 ? ' (Bu hafta)' : (w===1 ? ' (Gelecek hafta)' : '');
    weeks.push({startStr:monStr, endStr:getDateStr(sun), label:_aiPlanFmtDM(mon)+' – '+_aiPlanFmtDM(sun)+tag});
    if(!windowEnd && w>=4) break; // to-do sistemi hiç yüklenmemişse bile makul bir üst sınır
  }
  return weeks;
}
function _aiPlanFmtDM(d){ return pad(d.getDate())+'.'+pad(d.getMonth()+1); }
function _aiPlanStrToDate(dateStr){ var p=dateStr.split('-').map(Number); return new Date(p[0],p[1]-1,p[2]); }

function aiPlanWeekChanged(){
  var sel = document.getElementById('aiPlanWeekSelect');
  aiPlanSelectedWeekStart = sel ? sel.value : aiPlanSelectedWeekStart;
  if(aiPlanParsedResult) _aiPlanBuildPreview();
  var area = document.getElementById('aiPlanPreviewArea');
  if(area) area.innerHTML = _aiPlanRenderPreview();
}

function analyzeAiPlanImage(){
  if(!aiPlanImage){ showToast('❌ Önce bir plan fotoğrafı ekle.'); return; }
  if(aiPlanBusy) return;
  if(!aiPlanSelectedWeekStart){ var w=_aiPlanGetWeekOptions(); aiPlanSelectedWeekStart = w.length?w[0].startStr:getTodayStr(); }
  aiPlanBusy = true;
  var btn = document.getElementById('aiPlanAnalyzeBtn');
  if(btn){ btn.disabled=true; btn.textContent='⏳ Analiz ediliyor…'; }

  var parts = [ {text: AI_PLAN_PROMPT}, {inline_data:{mime_type:aiPlanImage.mime, data:aiPlanImage.data}} ];
  callGeminiAPI([{role:'user', parts:parts}], null, true).then(function(raw){
    var parsed = _aiPlanParseJson(raw);
    if(!parsed || !parsed.gunler || !parsed.gunler.length){
      showToast('❌ Plan okunamadı. Daha net/aydınlık bir fotoğraf dene.');
      aiPlanParsedResult = null; aiPlanPreview = null;
      return;
    }
    aiPlanParsedResult = parsed;
    _aiPlanBuildPreview();
    if(!aiPlanPreview || !aiPlanPreview.length){
      showToast('⚠️ Görselde gün/etkinlik tanınamadı.');
    } else {
      showToast('✅ Plan okundu, önizlemeyi kontrol et.');
    }
  }).catch(function(err){
    showToast('❌ '+(err&&err.message?err.message:'Analiz başarısız.'));
  }).then(function(){
    aiPlanBusy = false;
    if(btn){ btn.disabled=false; btn.textContent='🔍 Planı Analiz Et'; }
    var area = document.getElementById('aiPlanPreviewArea');
    if(area) area.innerHTML = _aiPlanRenderPreview();
  });
}

function _aiPlanParseJson(raw){
  try{
    var cleaned = raw.trim().replace(/^```(json)?/i,'').replace(/```$/,'').trim();
    var obj = JSON.parse(cleaned);
    if(!obj || !Array.isArray(obj.gunler)) return null;
    return obj;
  }catch(e){ return null; }
}

/* Gemini'nin döndürdüğü gün isimlerini SEÇİLİ haftanın gerçek tarihlerine
   eşler; bugünden önceki ve 30 günlük pencerenin dışındaki günleri
   "notAddableReason" ile işaretler (silmiyor — kullanıcı görsün diye
   önizlemede gösterip NEDEN eklenmediğini açıklıyoruz). */
function _aiPlanBuildPreview(){
  if(!aiPlanParsedResult || !aiPlanParsedResult.gunler){ aiPlanPreview=null; return; }
  var monday = _aiPlanStrToDate(aiPlanSelectedWeekStart);
  var today = getTodayStr();
  var out=[];
  aiPlanParsedResult.gunler.forEach(function(g){
    var raw=(g.gun||'').toString().trim();
    if(!raw) return;
    var gunAdi = raw.charAt(0).toLocaleUpperCase('tr-TR') + raw.slice(1).toLocaleLowerCase('tr-TR');
    if(!AI_PLAN_DOW_TO_INDEX.hasOwnProperty(gunAdi)) return; // tanınmayan gün adı — atla
    var dowIdx = AI_PLAN_DOW_TO_INDEX[gunAdi];
    var offsetFromMonday = (dowIdx===0) ? 6 : (dowIdx-1); // TR haftası Pzt=0 ofset, Paz=6 ofset
    var d = new Date(monday); d.setDate(monday.getDate()+offsetFromMonday);
    var dateStr = getDateStr(d);
    var items = (g.etkinlikler||[]).map(function(e){
      if(!e) return null;
      var tur = ['study','video','deneme','custom'].indexOf(e.tur)!==-1 ? e.tur : 'custom';
      var dakika = null;
      if(typeof e.dakika==='number' && e.dakika>0) dakika=Math.round(e.dakika);
      else if(typeof e.dakika==='string' && parseInt(e.dakika,10)>0) dakika=parseInt(e.dakika,10);
      var saat = (typeof e.saat==='string' && /^\d{1,2}:\d{2}$/.test(e.saat.trim())) ? e.saat.trim() : null;
      var baslik = (e.baslik||'').toString().trim();
      if(!baslik) return null;
      return { baslik:baslik, tur:tur, ders:(e.ders&&typeof e.ders==='string')?e.ders.trim():null, dakika:dakika, saat:saat, _include:true };
    }).filter(Boolean);
    if(!items.length) return;
    var notAddableReason = null;
    if(dateStr < today) notAddableReason='past';
    else if(typeof _todoInWindow==='function' && !_todoInWindow(dateStr)) notAddableReason='window';
    out.push({ dateStr:dateStr, gunAdi:gunAdi, notAddableReason:notAddableReason, items:items });
  });
  out.sort(function(a,b){ return a.dateStr<b.dateStr?-1:(a.dateStr>b.dateStr?1:0); });
  out.forEach(function(d,i){ d._idx=i; });
  aiPlanPreview = out;
}

function _aiPlanRenderPreview(){
  if(!aiPlanPreview) return '';
  var skippedPast = aiPlanPreview.filter(function(d){return d.notAddableReason==='past';});
  var skippedWindow = aiPlanPreview.filter(function(d){return d.notAddableReason==='window';});
  var addable = aiPlanPreview.filter(function(d){return !d.notAddableReason;});
  var html = '';
  if(skippedPast.length){
    html += '<div class="ai-plan-skip-note">⏭ <b>'+skippedPast.map(function(d){return d.gunAdi;}).join(', ')+'</b> bugünden önceye denk geldiği için otomatik atlandı.</div>';
  }
  if(skippedWindow.length){
    html += '<div class="ai-plan-skip-note">⏭ <b>'+skippedWindow.map(function(d){return d.gunAdi;}).join(', ')+'</b> 30 günlük planlama sınırının dışında kaldığı için atlandı.</div>';
  }
  if(!aiPlanPreview.length) return html;
  if(!addable.length){
    html += '<div class="todo-empty">Bu haftadan eklenebilecek gün kalmadı.</div>';
    return html;
  }
  html += '<div class="ai-plan-days">';
  addable.forEach(function(d){
    html += '<div class="ai-plan-day-card">';
    html += '<div class="ai-plan-day-head"><b>'+d.gunAdi+'</b><span class="ai-plan-day-date">'+formatDateHuman(d.dateStr)+'</span></div>';
    d.items.forEach(function(it,ii){
      var meta = (typeof _todoKindMeta==='function') ? _todoKindMeta(it.tur) : {icon:'✅'};
      var chips='';
      if(it.ders) chips+='<span class="todo-chip">'+escapeHtml(it.ders)+'</span>';
      if(it.saat) chips+='<span class="todo-chip todo-chip-time">🕐 '+it.saat+'</span>';
      if(it.dakika) chips+='<span class="todo-chip todo-chip-min">⏱ '+it.dakika+' dk</span>';
      html += '<label class="ai-plan-item-row">'+
        '<input type="checkbox" checked onchange="aiPlanToggleItem('+d._idx+','+ii+',this.checked)">'+
        '<span class="ai-plan-item-title">'+meta.icon+' '+escapeHtml(it.baslik)+'</span>'+
        (chips?'<span class="ai-plan-item-chips">'+chips+'</span>':'')+
      '</label>';
    });
    html += '</div>';
  });
  html += '</div>';
  html += '<button class="ai-fc-gen-btn" onclick="confirmAiPlanAdd()">✅ Seçili Haftaya Ekle</button>';
  return html;
}

function aiPlanToggleItem(dayIdx, itemIdx, checked){
  if(!aiPlanPreview) return;
  var day = aiPlanPreview.find(function(d){return d._idx===dayIdx;});
  if(!day || !day.items[itemIdx]) return;
  day.items[itemIdx]._include = !!checked;
}

function confirmAiPlanAdd(){
  if(!aiPlanPreview || !aiPlanPreview.length){ showToast('⚠️ Eklenecek bir plan yok.'); return; }
  if(typeof _todoEnsureDay!=='function' || typeof saveTodos!=='function'){ showToast('❌ To-do list sistemi yüklenemedi.'); return; }
  var addableDays = aiPlanPreview.filter(function(d){ return !d.notAddableReason; });
  var totalAdded=0, firstDate=null;
  addableDays.forEach(function(d){
    var includedItems = d.items.filter(function(it){ return it._include!==false; });
    if(!includedItems.length) return;
    var day = _todoEnsureDay(d.dateStr);
    includedItems.forEach(function(it){
      day.items.push({
        id: 'todo_'+Date.now().toString(36)+'_'+Math.floor(Math.random()*100000),
        kind: it.tur, title: it.baslik, subject: it.ders, minutes: it.dakika, time: it.saat,
        done:false, doneAt:null, createdAt:Date.now(), order:day.items.length, source:'ai'
      });
      totalAdded++;
    });
    if(!firstDate) firstDate = d.dateStr;
  });
  if(totalAdded===0){ showToast('⚠️ Hiçbir etkinlik seçili değil.'); return; }
  saveTodos();
  showToast('✅ '+totalAdded+' etkinlik planlayıcıya eklendi!');
  aiPlanImage=null; aiPlanParsedResult=null; aiPlanPreview=null;
  if(firstDate) todoSelectedDate = firstDate; // features.js global — bu dosyadan ÖNCE yüklendiği için burada hazır
  closeAiPanel();
  openCalendarPanel();
  if(typeof switchCalTab==='function') switchCalTab('planner');
}
...
function deleteAiFlashcard(id){
  ...
  var removed = aiFlashcards.find(function(c){ return c.id===id; });
  ...
  // DRIVE ASSET STORAGE: kartın SS'i Drive'da saklanıyorduysa orada da sil
  if(removed && typeof ahaIsAssetPointer==='function' && ahaIsAssetPointer(removed.img) && typeof ahaDeleteAssetFromDrive==='function') ahaDeleteAssetFromDrive(removed.img);
}
...
function renderAiFcvCard(){
  ...
  // Ön yüz: SS butonu YOK (sadece arka yüzde), hint en sonda
  ...
  // Arka yüz: SS butonu + hint en sonda
...
function showAiFcvImage(){
  ...
  ov.innerHTML = '<span class="ai-fcv-img-overlay-close" title="Kapat" onclick="event.stopPropagation();this.parentElement.remove()">✕</span>'+
    '<div class="ai-fcv-img-zoom-wrap" id="aiFcvImgZoomWrap" style="overflow:hidden;width:100%;height:100%;display:flex;align-items:center;justify-content:center;">'+
      '<img id="aiFcvZoomImg" alt="ekran görüntüsü" style="transform-origin:center center;transition:transform 0.15s;cursor:zoom-in;opacity:0.4;">'+
    '</div>'+
    '<div style="position:absolute;bottom:8px;left:50%;transform:translateX(-50%);font-size:10px;color:rgba(255,255,255,0.4);font-family:JetBrains Mono,monospace;pointer-events:none;">🖱 scroll: yakınlaştır</div>';
  // DRIVE ASSET STORAGE: c.img artık bir Drive pointer'ı olabilir (büyük SS'ler
  // taşınmış olabilir) — göstermeden önce her zaman ahaResolveRawImage'dan geçir.
  var zoomImgEl = ov.querySelector('#aiFcvZoomImg');
  var imgResolveP = (typeof ahaResolveRawImage === 'function') ? ahaResolveRawImage(c.img) : Promise.resolve('data:'+c.img.mime+';base64,'+c.img.data);
  imgResolveP.then(function(dataUrl){
    zoomImgEl.src = dataUrl;
    zoomImgEl.style.opacity = '';
  }, function(){
    zoomImgEl.alt = '⚠️ Görsel Drive\'dan yüklenemedi';
    zoomImgEl.style.opacity = '';
  });
  ...
  ov.addEventListener('wheel', function(e){
    e.preventDefault();
    e.stopPropagation();
    var img = document.getElementById('aiFcvZoomImg');
    if(!img) return;
