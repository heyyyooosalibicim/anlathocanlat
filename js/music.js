...
function renderMusicContent(){
  ...
  var plSel = '<div class="music-pl-row"><select class="music-pl-select" id="ytMPlSel" onchange="onYTMusicPlChange(this.value)">';
  plSel += '<option value="">— YouTube Playlist seçin —</option>';
  ytMusicPlaylists.forEach(function(pl){
    plSel += '<option value="'+pl.id+'"'+(pl.id===ytMusicActivePlId?' selected':'')+'>'+(pl.custom?'🔗 ':'')+escapeHtml(pl.name)+' ('+pl.itemCount+')</option>';
  });
  if(ytMusicPlaylists.length===0){ plSel += '<option disabled>Yükleniyor...</option>'; }
  plSel += '</select>';
  if(ytMusicActivePlId){
    plSel += '<button class="music-pl-remove-btn" onclick="removeYTMusicPlaylist(\''+ytMusicActivePlId+'\')" title="Bu playlisti kaldır">🗑</button>';
  }
  plSel += '</div>';
  var addLinkHtml =
    '<div class="music-pl-add-row">' +
      '<input class="music-pl-add-input" id="musicPlAddInput" type="text" placeholder="YouTube playlist linki yapıştır..." onkeydown="if(event.key===\'Enter\'){event.preventDefault();addYTMusicPlaylistByLink();}">' +
      '<button class="music-pl-add-btn" onclick="addYTMusicPlaylistByLink()">+ Ekle</button>' +
    '</div>';
  ...
  c.innerHTML = plSel + addLinkHtml + nowHtml + ctrlHtml + trkHtml + bottomHtml;
...
function fetchYTMusicPlaylists(){
  ...
  fetch('https://www.googleapis.com/youtube/v3/playlists?part=snippet,contentDetails&mine=true&maxResults=50',{
    ...
    var mine = (d.items||[]).map(function(pl){
    ...
    // (2026-09) Link ile elle eklenen playlistler (custom:true) "mine=true"
    // sorgusunda dönmeyebilir (başkasının herkese açık playlisti olabilir) —
    // bu yüzden listenin ÜZERİNE YAZMAK yerine BİRLEŞTİRİYORUZ. Aynı ID hem
    // "mine=true" hem custom listede varsa (kullanıcı kendi playlistini link
    // ile de eklemişse) güncel "mine=true" bilgisi kazanır.
    var customOnes = ytMusicPlaylists.filter(function(pl){
      return pl.custom && !mine.some(function(m){return m.id===pl.id;});
    });
    ytMusicPlaylists = mine.concat(customOnes);
    saveMusicState();
    renderMusicContent();
  }).catch(function(){ showToast('❌ Playlistler çekilemedi.'); });
}

/* (2026-09) Link ile playlist ekleme — kaydedilince "kendi hesabındaki
   playlistler" ile AYNI listeye ve AYNI çalma altyapısına (onYTMusicPlChange /
   fetchYTMusicTracks) girer, normal bir şekilde çalınabilir. */
function addYTMusicPlaylistByLink(){
  var inp = document.getElementById('musicPlAddInput');
  if(!inp) return;
  var raw = inp.value.trim();
  if(!raw){ showToast('❌ Önce bir playlist linki yapıştır.'); return; }
  if(!_ytAccessToken){ showToast('❌ Google girişi gerekli.'); return; }
  var plId = (typeof extractPlaylistId==='function') ? extractPlaylistId(raw) : null;
  if(!plId){ showToast('❌ Geçerli bir YouTube playlist linki değil.'); return; }
  if(ytMusicPlaylists.some(function(p){return p.id===plId;})){
    showToast('ℹ️ Bu playlist zaten ekli, seçiliyor.');
    inp.value='';
    onYTMusicPlChange(plId);
    return;
  }
  inp.disabled = true;
  showToast('⏳ Playlist ekleniyor...');
  fetch('https://www.googleapis.com/youtube/v3/playlists?part=snippet,contentDetails&id='+encodeURIComponent(plId),{
    headers:{'Authorization':'Bearer '+_ytAccessToken}
  }).then(function(r){ return r.json(); }).then(function(d){
    if(inp) inp.disabled = false;
    if(d.error){ showToast('❌ '+d.error.message); return; }
    if(!d.items || d.items.length===0){ showToast('❌ Playlist bulunamadı veya herkese açık değil.'); return; }
    var it = d.items[0];
    var newPl = {
      id: plId,
      name: (it.snippet && it.snippet.title) ? it.snippet.title : plId,
      itemCount: it.contentDetails ? (it.contentDetails.itemCount||0) : 0,
      custom: true // fetchYTMusicPlaylists() bunu ÜZERİNE YAZMAZ, bkz. orada
    };
    ytMusicPlaylists.push(newPl);
    if(inp) inp.value = '';
    saveMusicState();
    showToast('✅ "'+newPl.name+'" eklendi.');
    onYTMusicPlChange(plId);
  }).catch(function(){
    if(inp) inp.disabled = false;
    showToast('❌ Playlist bilgisi alınamadı.');
  });
}

/* (2026-09) Playlisti müzik listesinden kaldır (link ile eklenenler için
   kalıcı; "mine=true" ile gelenler bir sonraki fetchYTMusicPlaylists()'te
   hesapta hâlâ varsa geri gelebilir — YouTube'daki gerçek durumu yansıtır). */
function removeYTMusicPlaylist(plId){
  var pl = ytMusicPlaylists.find(function(p){return p.id===plId;});
  if(!pl) return;
  if(!confirm('"'+pl.name+'" playlistini kaldırmak istediğinden emin misin?'))return;
  ytMusicPlaylists = ytMusicPlaylists.filter(function(p){return p.id!==plId;});
  if(ytMusicActivePlId===plId){
    ytMusicActivePlId = null;
    ytMusicTracks = [];
    ytMusicTrackIndex = -1;
    musicStop();
