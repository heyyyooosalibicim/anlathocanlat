...
/* SON ÇARE (Drive'a da taşınamadıysa — ör. Google'a giriş yapılmamışsa):
 * en büyük, HÂLÂ CİHAZDA duran (Drive pointer'ı OLMAYAN) PDF'ten başlayarak
 * tek tek çıkar, her adımda tekrar kaydetmeyi dene — SADECE sığana kadar
 * gereken kadarını feda et, sınavın kendisini (isim/kategori/net/sonuç) asla
 * kaybetme. Zaten Drive'a taşınmış (pointer) bir PDF burada hedef alınmaz —
 * onu silmek zaten neredeyse hiç yer açmaz (pointer küçücüktür). */
function _denemeLegacyStripPdfFallback(){
  var withPdf=denemeExams.filter(function(ex){return ex.pdf&&typeof ex.pdf.data==='string';});
  ...
  for(var i=0;i<withPdf.length;i++){
    ...
    try{
      ...
      showToast('⚠️ Depolama alanı doldu: '+list+' denemesindeki PDF eki kaldırıldı. Sınav kaydın (isim/net/sonuç) güvende — istersen daha küçük bir PDF ile tekrar ekleyebilirsin, ya da Ayarlar\'dan Google hesabına giriş yaparsan PDF\'ler otomatik olarak Drive\'a taşınır ve hiç kaldırılmaz.');
      return;
...
function denemeSaveExams(){
  /* DRIVE ASSET STORAGE (bkz. features.js): normal kayıt yeterliyse anında
   * biter; kota aşılırsa en büyük PDF'ten başlayarak OTOMATİK olarak (veri
   * KAYBETMEDEN) kullanıcının kendi Google Drive'ına taşır — notlardaki
   * büyük PDF sistemiyle TAMAMEN AYNI altyapı. "PDF'i sil" artık ilk çare
   * DEĞİL — sadece Drive'a da taşınamadıysa (ör. giriş yapılmamışsa) devreye
   * giren bir güvenlik ağı (_denemeLegacyStripPdfFallback). */
  if(typeof ahaSafeSetItem==='function'){
    ahaSafeSetItem(LS_DENEME,denemeExams).then(function(ok){
      if(!ok)_denemeLegacyStripPdfFallback();
    });
  }else{
    try{
      localStorage.setItem(LS_DENEME,JSON.stringify(denemeExams));
    }catch(e){
      _denemeLegacyStripPdfFallback();
    }
  }
}
...
function _denemeInjectStyles(){
  ...
  /* BUG FIX: PDF önizlemesinde eski geniş "Denemeyi Bitir" butonu (yukarıdaki
   * .dnm-finish-sticky-btn) topbar'ın içinde, PDF görüntüleyicinin kendi
   * kontrolleriyle (sayfa/zoom/indirme ikonları) aynı şeride denk gelip
   * onları örtüyordu ve tıklamayı engelliyordu. Küçük, yuvarlak, sadece ✕
   * simgeli bir butona indirildi ve topbar'ın DIŞINA, bağımsız/mutlak
   * konumlu olarak taşındı — üst kenardan biraz aşağıda, ortada. */
  '.dnm-finish-mini-btn{position:absolute;top:52px;left:50%;transform:translateX(-50%);z-index:3;background:linear-gradient(135deg,#ff6b6b,#e84545);color:#fff;border:none;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:14px;font-weight:800;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(232,69,69,0.45);transition:all 0.15s}'+
  '.dnm-finish-mini-btn:hover{filter:brightness(1.1);transform:translateX(-50%) scale(1.08)}'+
...
function _denemeInjectModals(){
  ...
  '<div class="modal-overlay" id="dnmPdfChoiceOverlay" onclick="if(event.target===this)denemeClosePdfChoice()">'+
    '<div class="modal">'+
      ...
      '<div style="display:flex;flex-direction:column;gap:8px" id="dnmPdfChoiceButtons">'+
      ...
      '</div>'+
      '<div id="dnmPdfChoiceBusyArea" style="display:none"></div>'+
...
function denemeRenderCalBody(){
  ...
  // NOT: deneme modunda da "Planlayıcı" sekmesi artık ORTAK to-do list
  // sistemini gösteriyor (bkz. features.js → renderTodoPlannerTab). Eski
  // denemeRenderPlannerTab (ileri tarihe deneme atama takvimi) BİR KENARA
  // KONDU — silinmedi, bkz. o fonksiyonun üstündeki not.
  else if(calActiveTab==='planner'){ if(typeof renderTodoPlannerTab==='function') renderTodoPlannerTab(body); }
...
function denemeRenderManagerTab(body,scope){
  ...
  filtered.forEach(function(cat){
    ...
    html+='<div class="mgr-pl-block" id="dnmcatblock'+scope+'-'+cat.id+'" style="opacity:'+(isCatHidden?0.45:1)+'">'+
      '<div class="mgr-pl-header" onclick="denemeToggleCatOpen(\''+cat.id+'\',\''+scope+'\')">'+
        dot+
        '<div class="mgr-pl-title" id="dnmcattitle'+scope+'-'+cat.id+'" style="'+(isCatHidden?'text-decoration:line-through;color:var(--muted)':'')+'">'+escapeHtml(cat.name)+'</div>'+
        '<div class="mgr-pl-stats">'+doneCount+'/'+exams.length+' tamamlandı</div>'+
        '<button class="mgr-pl-rename-btn" onclick="event.stopPropagation();denemeToggleCatHidden(\''+cat.id+'\',\''+scope+'\')" title="'+(isCatHidden?'Kategoriyi Göster':'Kategoriyi Gizle')+'" style="color:'+(isCatHidden?'var(--muted)':'var(--text2)')+'">'+(isCatHidden?'👁‍🗨':'👁')+'</button>'+
...
function denemeExamCardHtml(ex,scope){
  ...
  var pdfTxt=ex.pdf?('📄 PDF'+((typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(ex.pdf.data))?' ☁️':'')):'';
...
function denemeDeleteCat(catId,scope){
  ...
  denemeExamsInCat(catId).forEach(function(ex){_dnmCleanupOldExamPdf(ex.pdf,null);});
  ...
  denemeRefreshAllViews();  showToast('🗑 Kategori silindi.');
}

/* Playlist kategorileriyle BİREBİR AYNI desen (bkz. mgrToggleCatHidden,
   features.js) — kullanıcı manuel olarak bir deneme kategorisini gizleyip
   gösterebilir. */
function denemeToggleCatHidden(catId,scope){
  var cat=denemeFindCat(catId);if(!cat)return;
  cat.hidden=!cat.hidden;
  denemeSaveCats();
  denemeRefreshAllViews();
}
/* Her gece yarısı sıfırlamasında (bkz. features.js checkMidnightReset) TÜM
   deneme kategorilerini playlistlerle AYNI "temiz sayfa" felsefesiyle otomatik
   gizler. Ayarlar > "Günlük playlist & deneme gizlemesini durdur" açıksa bu
   fonksiyon HİÇ çağrılmaz. */
function denemeApplyDailyCatHide(){
  denemeCats.forEach(function(cat){ cat.hidden=true; });
  denemeSaveCats();
}
...
function denemeHandlePdfFile(inputEl){
   ...
   * sınırlı) saklanıyor. BUG FIX: bu uyarı eskiden Google girişi olsun ya da
   * olmasın HER ZAMAN gösteriliyordu — ama giriş varsa `denemeSaveExams()`
   * zaten `ahaSafeSetItem` üzerinden büyük PDF'i sessizce Drive'a taşıyor
   * (bkz. 4-F, 4-E madde 6), yani gereksiz/yanlış bir "büyük dosya" hatası
   * gibi görünüyordu. Artık SADECE Google'a giriş YOKSA gösteriliyor —
   * çünkü o durumda gerçek bir risk var (localStorage'a sığmazsa PDF
   * kalıcı olarak kaldırılabilir, Drive'a taşınacak bir yer yok). */
  var PDF_WARN_BYTES=4*1024*1024;
  if(file.size>PDF_WARN_BYTES&&!_ytAccessToken){
    showToast('⚠️ "'+file.name+'" oldukça büyük ('+(file.size/1024/1024).toFixed(1)+' MB). Google hesabına giriş yapmadığın için cihaz depolaması dolarsa bu PDF otomatik kaldırılabilir — mümkünse Google\'a giriş yap (otomatik Drive\'a taşınır) ya da daha küçük/sıkıştırılmış bir dosya kullan.');
  }
...
function denemeRefreshPdfChip(catId,scope){
  ...
  if(d){
    var cloudTag=(typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(d.data))?' ☁️':'';
    chipWrap.innerHTML='<span class="dnm-file-chip">📄 '+escapeHtml(d.name.length>26?d.name.substr(0,26)+'…':d.name)+cloudTag+'<button onclick="denemeRemovePdfDraft(\''+catId+'\',\''+scope+'\')" title="Kaldır">✕</button></span>';
  }else{
...
/* — Drive'a taşınmış eski PDF'i temizle (değiştirilince/kaldırılınca) —
   playlist eklerindeki deleteAttachment ile AYNI anlık-temizlik deseni:
   reconcile sweep zaten yetimleri er ya da geç yakalar ama kullanıcı
   deneyimi için beklemeye gerek yok, silme/değişiklik ANINDA temizleriz. */
function _dnmCleanupOldExamPdf(oldPdf,newPdf){
  if(!oldPdf||!oldPdf.data)return;
  if(typeof ahaIsAssetPointer!=='function'||!ahaIsAssetPointer(oldPdf.data))return;
  var samePointer=newPdf&&newPdf.data&&typeof newPdf.data==='object'&&newPdf.data.driveId===oldPdf.data.driveId;
  if(samePointer)return; // değişmedi, hâlâ kullanılıyor
  if(typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(oldPdf.data);
}
...
function denemeSaveExam(catId,scope){
  ...
  if(editId){
    ...
    if(exam){
      ...
      _dnmCleanupOldExamPdf(oldPdf,pdf);
...
function denemeDeleteExam(examId){
  ...
  _dnmCleanupOldExamPdf(exam.pdf,null);
...
function denemeLaunchExam(examId){
  ...
  // BUG FIX: PDF eklenmemiş ama süre (timerMin) eklenmişse eskiden zamanlayıcı
  // arka planda başlatılsa bile HEMEN skor modalı açılıyordu — süre tamamen
  // yok sayılıyordu. Artık PDF akışıyla (denemeMountPlayerWrap) BİREBİR AYNI
  // desende ('timeronly' modu) bir çalışma ekranı monte ediliyor: zamanlayıcı
  // akarken kullanıcı "🏁 Denemeyi Bitir" butonuna basana (veya süre dolup o
  // butona basana) kadar netleri girme ekranı hiç açılmıyor.
  else if(exam.timerMin)denemeMountPlayerWrap('timeronly',exam,null);
...
function _dnmHidePdfChoiceOverlay(){var ov=document.getElementById('dnmPdfChoiceOverlay');if(ov)ov.classList.remove('open');_dnmHidePdfChoiceBusy();}
...
/* — Seçim modalını yükleniyor/hata durumuna çevir (DRIVE ASSET STORAGE) —
   exam.pdf.data artık ya düz bir data-URI (yerel) ya da Drive pointer'ı
   olabilir; pointer'sa 3 açma yolunun (embedded/external/native) HİÇBİRİ
   veriyi doğrudan kullanamaz — önce ahaResolveAssetData ile çözülmesi
   gerekir. Bu üçünün ORTAK giriş noktası: yerelse anında devam eder,
   Drive'daysa modal içeriğini geçici olarak indiriliyor/hata gösterecek
   şekilde değiştirir (openAttachViewer'daki ⏳/❌ deseniyle aynı). */
function _dnmShowPdfChoiceBusy(msg){
  var btns=document.getElementById('dnmPdfChoiceButtons');
  var busy=document.getElementById('dnmPdfChoiceBusyArea');
  if(btns)btns.style.display='none';
  if(busy){
    busy.style.cssText='display:flex;flex-direction:column;align-items:center;gap:10px;padding:18px 6px';
    busy.innerHTML='<span style="font-size:24px">⏳</span><span style="font-size:12px;color:var(--muted);text-align:center">'+escapeHtml(msg)+'</span>';
  }
}
function _dnmHidePdfChoiceBusy(){
  var btns=document.getElementById('dnmPdfChoiceButtons');
  var busy=document.getElementById('dnmPdfChoiceBusyArea');
  if(btns)btns.style.display='flex';
  if(busy){busy.style.display='none';busy.innerHTML='';}
}
function _dnmShowPdfChoiceError(msg){
  var busy=document.getElementById('dnmPdfChoiceBusyArea');
  if(!busy)return;
  busy.style.cssText='display:flex;flex-direction:column;align-items:center;gap:10px;padding:14px 6px';
  var needsLogin=msg.indexOf('giriş yapmalısın')!==-1;
  busy.innerHTML='<span style="font-size:24px">❌</span>'+
    '<span style="font-size:12px;color:var(--muted);text-align:center;max-width:280px;line-height:1.5">'+escapeHtml(msg)+'</span>'+
    (needsLogin
      ? '<button class="dnm-exam-btn" style="padding:8px 14px" onclick="if(typeof signInGoogle===\'function\')signInGoogle()">🔑 Google\'a Giriş Yap</button>'
      : '<button class="dnm-exam-btn" style="padding:8px 14px" onclick="_dnmHidePdfChoiceBusy()">← Seçeneklere Dön</button>');
}
function _dnmResolveExamPdf(exam,onReady){
  var pdfData=exam&&exam.pdf&&exam.pdf.data;
  if(typeof pdfData!=='object'||!pdfData){ onReady(pdfData||null); return; } // yerel string ya da PDF yok — beklemeden devam
  if(typeof ahaResolveAssetData!=='function'){
    _dnmShowPdfChoiceError('Bu dosya Drive\'da saklanıyor ama Drive modülü yüklenemedi. Sayfayı yenile.');
    return;
  }
  _dnmShowPdfChoiceBusy('PDF Drive\'dan indiriliyor…');
  var examId=exam.id;
  ahaResolveAssetData(exam.pdf).then(function(dataUrl){
    if(_dnmActiveExamId!==examId)return; // bu sırada İptal'e basıldı/modal kapandı — artık geçersiz, oynatıcıyı açma
    if(typeof _ahaUploadDedup==='object'&&pdfData.driveId)_ahaUploadDedup[dataUrl]=pdfData.driveId;
    onReady(dataUrl);
  }, function(err){
    if(_dnmActiveExamId!==examId)return;
    _dnmShowPdfChoiceError(err&&err.message?err.message:'PDF indirilemedi.');
  });
}

function denemeOpenEmbeddedPdf(){
  var exam=denemeFindExam(_dnmActiveExamId);if(!exam)return;
  _dnmResolveExamPdf(exam,function(dataUrl){
    _dnmHidePdfChoiceOverlay();
    denemeMountPlayerWrap('embedded',exam,dataUrl);
  });
}
function denemeOpenExternalPdf(){
  ...
  _dnmResolveExamPdf(exam,function(dataUrl){
    ...
    denemeMountPlayerWrap('external',exam,dataUrl);
  });
}
...
function denemeOpenNativeAppPdf(){
  ...
  _dnmResolveExamPdf(exam,function(dataUrl){
    _dnmHidePdfChoiceOverlay();
    if(!dataUrl){denemeMountPlayerWrap('nativeapp',exam,dataUrl);return;}
    ...
    var shared=false;
    try{
      var file=_dnmDataUrlToFile(dataUrl,filename,'application/pdf');
      if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){
        ...
        navigator.share({files:[file],title:exam.isim}).catch(function(err){
          if(err&&err.name!=='AbortError'){ // kullanıcı paneli iptal ettiyse sessiz geç
            ...
            _dnmDownloadPdf(dataUrl,filename);
          }
        });
      }
    ...
    denemeMountPlayerWrap('nativeapp',exam,dataUrl);
  });
}
...
/* — #playerWrap içine PDF önizleme / HUGE buton montajı —
   resolvedPdfUrl: _dnmResolveExamPdf tarafından ÖNCEDEN çözülmüş, doğrudan
   kullanılabilir data-URI. exam.pdf.data ARTIK bir Drive pointer'ı olabileceği
   için burada asla doğrudan okunmaz. */
function denemeMountPlayerWrap(mode,exam,resolvedPdfUrl){
  ...
  if(mode==='embedded'){
    ...
    box.innerHTML='<iframe class="dnm-pdf-iframe" src="'+(resolvedPdfUrl||'')+'" title="'+escapeHtml(exam.isim)+'"></iframe>'+
      '<div class="dnm-pdf-topbar">'+
        '<button class="dnm-exit-mini-btn" onclick="denemeExitWorkspace()" title="Sınav ekranından çık">✕</button>'+
      '</div>'+
      '<button class="dnm-finish-mini-btn" onclick="denemeShowFinishConfirm()" title="Denemeyi Bitir">✕</button>';
  }else if(mode==='timeronly'){
    // PDF'siz, sadece süreli denemeler için: zamanlayıcı akarken kullanıcıyı
    // bekleten çalışma ekranı (bkz. denemeLaunchExam bug fix notu).
    ...
    box.innerHTML='<button class="dnm-exit-mini-btn" style="position:absolute;top:10px;left:10px" onclick="denemeExitWorkspace()" title="Sınav ekranından çık">✕</button>'+
      '<span class="dnm-huge-icon">⏱</span>'+
      '<div class="dnm-huge-label">"'+escapeHtml(exam.isim)+'" için zamanlayıcı çalışıyor.<br>Süre dolunca veya sınavı bitirdiğinde aşağıdaki butona bas.</div>'+
...
function denemeSaveScore(examId){
  ...
  // (2026-09) Planlayıcıdan açılmış bir göreve bağlıysa otomatik tamamla —
  // bkz. features.js todoOpenDenemeRef / _todoAutoCompleteItem.
  if(typeof _todoActiveDenemeTask!=='undefined' && _todoActiveDenemeTask && _todoActiveDenemeTask.examId===examId){
    var _t=_todoActiveDenemeTask; _todoActiveDenemeTask=null;
    if(typeof _todoAutoCompleteItem==='function') _todoAutoCompleteItem(_t.dateStr,_t.itemId);
  }
}
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
/* ══════════ [BİR KENARA KONDU — 2026-08] İleri tarihe DENEME planlama ══════════
 * Kullanıcı isteğiyle "Planlayıcı" sekmesinin ana görünümünden kaldırıldı,
 * yeni ORTAK to-do list sistemi lehine (bkz. denemeRenderCalBody yukarıda ve
 * features.js → renderTodoPlannerTab). Aşağıdaki fonksiyonlar (ve
 * denemeAssignExamToDate/denemeUnassignExam/denemeSelectPlannerDay) TAMAMEN
 * SİLİNMEDİ, sadece artık hiçbir yerden çağrılmıyor. denemeExams içindeki
 * plannedDate alanı da bozulmadı/kaybolmadı — sadece bu takvim arayüzü
 * gösterilmiyor. Yeni to-do list'te "deneme" TÜRÜNDE görev eklenebiliyor
 * (bkz. TODO_KIND_META.deneme), bu ondan tamamen ayrı, daha basit bir akış. */
