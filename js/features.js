...
function snapshotToday(){
  ...
  var videosWatched=0,videosTotal=0;
  plSnap.forEach(function(pl){pl.items.forEach(function(it){videosTotal++;if(it.watched)videosWatched++;});});
  var day=ensureArchiveDay(today);
  day.watch={summary:{swMs:swMs,videosWatched:videosWatched,videosTotal:videosTotal},detail:{playlists:plSnap}};
  saveArchive();
}
...
function checkMidnightReset(){
  ...
  if(lastCheckedDate&&lastCheckedDate!==today){
    ...
    prevDay.watch={summary:{swMs:swMs,videosWatched:videosWatched,videosTotal:videosTotal},detail:{playlists:plSnap}};
    ...
    if(!stopDailyHideEnabled){
      ...
      // Gizleme durdurulmuş olsa bile izlenme durumu/notlar her gün sıfırlanmaya
      // devam eder — SADECE görünürlük (hidden) adımı atlanıyor, "temiz sayfa"
      // felsefesinin geri kalanı aynı kalır.
      playlists.forEach(function(pl){
        pl.items.forEach(function(it){it.watched=false;});
        pl.notes={};
      });
...
function toggleHistDay(dateStr){
  ...
  if(el.dataset.loaded!=='1'){
    ...
    if(watchDetail){
      el.innerHTML='<button class="hist-restore-btn" onclick="restoreFromHistory(\''+dateStr+'\')">↩ Bu Günün Playlistlerini Geri Yükle</button>'+renderHistoryDayBody(watchDetail);
    } else {
