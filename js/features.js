...
/* ── MODÜL 3: Global App Mode State ── */
var currentAppMode = 'playlist'; // 'playlist' | 'deneme'
var LS_DENEMELER   = 'aha_denemeler_v1';
var _denemeler     = null; // lazy-loaded

function _loadDenemeler(){
  if(_denemeler) return _denemeler;
  try{ _denemeler = JSON.parse(localStorage.getItem(LS_DENEMELER)||'[]')||[]; }
  catch(e){ _denemeler = []; }
  return _denemeler;
}
function _saveDenemeler(){
  try{ localStorage.setItem(LS_DENEMELER, JSON.stringify(_denemeler||[])); }
  catch(e){ showToast('❌ Deneme kaydedilemedi.'); }
}
...
function switchCalTab(tab){
  calActiveTab=tab;
  var tabs = ['stats','history','planner','manager','deneme'];
...
function renderCalBody(){
  ...
  else if(calActiveTab==='deneme')renderDenemeTab(body);
}
...
/* ════════════════════════════════════════════════════════════
   MODÜL 3 — DENEME SINAVI SİSTEMİ
   ════════════════════════════════════════════════════════════ */

/* ── Deneme Yöneticisi (Ayarlar'dan açılır, renderManagerTab mimarisi aynen rekullanılır) ── */
var _denemeManagerOpen = null; // Açık deneme id'si (accordion)
var _denemeSearchQuery = '';

function openDenemeManager(){
  closeSettingsPanel();
  openCalendarPanel();
  setTimeout(function(){ switchCalTab('deneme'); }, 120);
}

/* ── Deneme Tab (Takvim paneli içinde) ── */
function renderDenemeTab(body){
  _loadDenemeler();
  var searchHtml =
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">'+
      '<input class="mgr-search" type="text" id="denemeSearch" placeholder="Deneme veya branş ara..." '+
        'oninput="onDenemeSearch()" value="'+escapeHtml(_denemeSearchQuery)+'">'+
      '<button class="settings-action-btn" onclick="openDenemeResultModal(null)" style="flex-shrink:0;white-space:nowrap">'+
        '+ Deneme Ekle'+
      '</button>'+
    '</div>';

  var filtered = (_denemeler||[]).filter(function(d){
    if(!_denemeSearchQuery) return true;
    var q = _denemeSearchQuery.toLowerCase();
    return (d.name||'').toLowerCase().indexOf(q) >= 0 || (d.brans||'').toLowerCase().indexOf(q) >= 0;
  });

  var listHtml = '';
  if(filtered.length === 0){
    listHtml = '<div style="font-size:12px;color:var(--muted);padding:18px 0;text-align:center">Henüz deneme yok. "+ Deneme Ekle" ile başla.</div>';
  } else {
    // Tarih bazlı sıralama (yeniden eskiye)
    var sorted = filtered.slice().sort(function(a,b){ return (b.tarih||0)-(a.tarih||0); });
    // Tarihe göre gruplandır
    var groups = {};
    sorted.forEach(function(d){
      var dt = d.tarih ? new Date(d.tarih) : null;
      var key = dt ? (pad(dt.getDate())+'.'+pad(dt.getMonth()+1)+'.'+dt.getFullYear()) : 'Tarihi Yok';
      if(!groups[key]) groups[key] = [];
      groups[key].push(d);
    });
    Object.keys(groups).forEach(function(dateKey){
      listHtml += '<div class="hist-day-block" style="margin-bottom:10px">'+
        '<div class="hist-day-header" style="cursor:default">'+
          '<span style="font-size:11px;font-weight:700;color:var(--text2)">📅 '+escapeHtml(dateKey)+'</span>'+
          '<span style="font-size:10px;color:var(--muted);margin-left:auto">'+groups[dateKey].length+' deneme</span>'+
        '</div>';
      groups[dateKey].forEach(function(d){
        var net = typeof d.net === 'number' ? d.net.toFixed(2) : '—';
        var brans = d.brans ? escapeHtml(d.brans) : '';
        var sinav = d.sinavTuru ? escapeHtml(d.sinavTuru) : '';
        listHtml +=
          '<div class="hist-day-item" style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:rgba(255,255,255,0.02);border-radius:7px;margin-bottom:4px;cursor:pointer" onclick="openDenemeResultModal(\''+d.id+'\')">'+
            '<span style="font-size:15px">📝</span>'+
            '<div style="flex:1;min-width:0">'+
              '<div style="font-size:12px;font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escapeHtml(d.name)+'</div>'+
              '<div style="font-size:10px;color:var(--muted)">'+[brans,sinav].filter(Boolean).join(' · ')+'</div>'+
            '</div>'+
            '<div style="text-align:right;flex-shrink:0">'+
              '<div style="font-size:12px;font-weight:700;color:var(--green)">Net: '+net+'</div>'+
              '<div style="font-size:9px;color:var(--muted)">D:'+d.dogru+' Y:'+d.yanlis+'</div>'+
            '</div>'+
            '<button class="mgr-vid-btn" onclick="event.stopPropagation();deleteDeneme(\''+d.id+'\')" title="Sil" style="color:var(--accent);flex-shrink:0">✕</button>'+
          '</div>';
      });
      listHtml += '</div>';
    });

    // Net trendi — basit çizgi grafik
    if(sorted.length > 1){
      listHtml += _renderDenemeNetChart(sorted);
    }
  }

  body.innerHTML = searchHtml + listHtml;
  var s = document.getElementById('denemeSearch');
  if(s && _denemeSearchQuery) s.setSelectionRange(s.value.length, s.value.length);
}

function _renderDenemeNetChart(items){
  var chartItems = items.filter(function(d){ return typeof d.net === 'number'; }).slice(0, 20).reverse();
  if(chartItems.length < 2) return '';
  var maxNet = Math.max.apply(null, chartItems.map(function(d){ return d.net; })) || 1;
  var bars = chartItems.map(function(d){
    var pct = Math.round((d.net / (maxNet||1)) * 100);
    var color = pct > 70 ? 'var(--green)' : pct > 40 ? 'var(--gold)' : 'var(--accent)';
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1;min-width:0">'+
      '<div style="font-size:8px;color:var(--muted);white-space:nowrap;overflow:hidden;max-width:100%;text-align:center">'+d.net.toFixed(1)+'</div>'+
      '<div style="width:100%;background:rgba(255,255,255,0.06);border-radius:3px;overflow:hidden;height:52px;display:flex;align-items:flex-end">'+
        '<div style="width:100%;height:'+pct+'%;background:'+color+';border-radius:3px;transition:height 0.4s"></div>'+
      '</div>'+
      '<div style="font-size:7px;color:var(--muted);white-space:nowrap;overflow:hidden;max-width:100%;text-align:center">'+escapeHtml((d.name||'').substring(0,8))+'</div>'+
    '</div>';
  }).join('');
  return '<div style="margin-top:14px;padding:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px">'+
    '<div style="font-size:9px;font-weight:700;color:var(--muted);letter-spacing:0.5px;text-transform:uppercase;margin-bottom:10px">📈 Net Trendi</div>'+
    '<div style="display:flex;gap:4px;align-items:flex-end;height:80px">'+bars+'</div>'+
  '</div>';
}

function onDenemeSearch(){
  var inp = document.getElementById('denemeSearch');
  _denemeSearchQuery = inp ? inp.value : '';
  renderCalBody();
}

function deleteDeneme(id){
  _loadDenemeler();
  _denemeler = _denemeler.filter(function(d){ return d.id !== id; });
  _saveDenemeler();
  renderCalBody();
  showToast('🗑 Deneme silindi.');
}

/* ── Deneme Sonuç Formu Modal ── */
var _denemeModalEditId = null;

function openDenemeResultModal(editId){
  _denemeModalEditId = editId || null;
  _loadDenemeler();
  var existing = editId ? (_denemeler||[]).find(function(d){ return d.id === editId; }) : null;

  var ov = document.getElementById('denemeResultModalOv');
  if(!ov){
    ov = document.createElement('div');
    ov.id = 'denemeResultModalOv';
    ov.className = 'modal-overlay';
    ov.style.zIndex = '2100';
    ov.addEventListener('click', function(e){ if(e.target === ov) closeDenemeResultModal(); });
    document.body.appendChild(ov);
  }

  ov.innerHTML =
    '<div class="modal" style="max-width:420px;overflow:visible">'+
      '<h3>'+(existing ? '✏ Deneme Düzenle' : '📝 Yeni Deneme Sonucu')+'</h3>'+

      '<div style="display:flex;flex-direction:column;gap:10px">'+

        // Ad
        '<div>'+
          '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">Deneme Adı</div>'+
          '<input type="text" id="drfName" placeholder="Örn: Türkçe Denemesi 1" '+
            'value="'+escapeHtml(existing ? existing.name||'' : '')+'" '+
            'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit">'+
        '</div>'+

        // Branş / Ders
        '<div>'+
          '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">Ders / Branş</div>'+
          '<input type="text" id="drfBrans" placeholder="Matematik, Türkçe, Fen..." '+
            'value="'+escapeHtml(existing ? existing.brans||'' : '')+'" '+
            'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit">'+
        '</div>'+

        // Sınav Türü
        '<div>'+
          '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">Sınav Türü</div>'+
          '<select id="drfSinavTuru" onchange="onDrfSinavChange()" '+
            'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit">'+
            '<option value="TYT" '+((!existing||existing.sinavTuru==='TYT')?'selected':'')+'>TYT</option>'+
            '<option value="AYT" '+(existing&&existing.sinavTuru==='AYT'?'selected':'')+'>AYT</option>'+
            '<option value="Diger" '+(existing&&existing.sinavTuru==='Diger'?'selected':'')+'>Diğer / Deneme</option>'+
          '</select>'+
        '</div>'+

        // AYT alt türü (koşullu)
        '<div id="drfAytSubWrap" style="display:'+(existing&&existing.sinavTuru==='AYT'?'block':'none')+'">'+
          '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">AYT Türü</div>'+
          '<select id="drfAytSub" '+
            'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit">'+
            '<option value="Sayısal" '+(existing&&existing.aytSub==='Sayısal'?'selected':'')+'>Sayısal</option>'+
            '<option value="Sözel" '+(existing&&existing.aytSub==='Sözel'?'selected':'')+'>Sözel</option>'+
            '<option value="EA" '+(existing&&existing.aytSub==='EA'?'selected':'')+'>Eşit Ağırlık (EA)</option>'+
          '</select>'+
        '</div>'+

        // D / Y / Net
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">'+
          _drfNumInput('drfDogru', 'Doğru', existing ? existing.dogru||0 : 0)+
          _drfNumInput('drfYanlis', 'Yanlış', existing ? existing.yanlis||0 : 0)+
          '<div>'+
            '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">Net</div>'+
            '<div id="drfNetDisplay" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:14px;font-weight:700;color:var(--green);text-align:center">0.00</div>'+
          '</div>'+
        '</div>'+

        // Tarih
        '<div>'+
          '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">Tarih</div>'+
          '<input type="date" id="drfTarih" '+
            'value="'+(existing && existing.tarih ? new Date(existing.tarih).toISOString().split('T')[0] : new Date().toISOString().split('T')[0])+'" '+
            'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit">'+
        '</div>'+
      ...
      '<div class="modal-btns" style="margin-top:16px">'+
        '<button class="modal-btn cancel" onclick="closeDenemeResultModal()">İptal</button>'+
        '<button class="modal-btn confirm" onclick="saveDenemeResult()">💾 Kaydet</button>'+
      '</div>'+
    '</div>';
  ...
  // D/Y değişince net hesapla
  ['drfDogru','drfYanlis'].forEach(function(id){
    var el = document.getElementById(id);
    if(el) el.addEventListener('input', _updateDrfNet);
  });
}

function _drfNumInput(id, label, val){
  return '<div>'+
    '<div style="font-size:10px;font-weight:700;color:var(--muted);letter-spacing:0.8px;text-transform:uppercase;margin-bottom:5px">'+label+'</div>'+
    '<input type="number" id="'+id+'" min="0" max="200" value="'+val+'" '+
      'style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:8px 11px;font-size:12px;color:var(--text);outline:none;font-family:inherit;text-align:center">'+
  '</div>';
}

function onDrfSinavChange(){
  var sel = document.getElementById('drfSinavTuru');
  var wrap = document.getElementById('drfAytSubWrap');
  if(sel && wrap) wrap.style.display = sel.value === 'AYT' ? 'block' : 'none';
}

function _updateDrfNet(){
  var d = parseFloat(document.getElementById('drfDogru') && document.getElementById('drfDogru').value) || 0;
  var y = parseFloat(document.getElementById('drfYanlis') && document.getElementById('drfYanlis').value) || 0;
  var net = d - (y / 4);
  var el = document.getElementById('drfNetDisplay');
  if(el){
    el.textContent = net.toFixed(2);
    el.style.color = net >= 0 ? 'var(--green)' : 'var(--accent)';
  }
}

function saveDenemeResult(){
  var name  = (document.getElementById('drfName').value || '').trim();
  var brans = (document.getElementById('drfBrans').value || '').trim();
  var sinav = document.getElementById('drfSinavTuru').value;
  var aytSub = document.getElementById('drfAytSub') ? document.getElementById('drfAytSub').value : '';
  var dogru  = parseFloat(document.getElementById('drfDogru').value) || 0;
  var yanlis = parseFloat(document.getElementById('drfYanlis').value) || 0;
  var net    = dogru - yanlis / 4;
  var tarihVal = document.getElementById('drfTarih').value;
  var tarih  = tarihVal ? new Date(tarihVal).getTime() : Date.now();

  if(!name){ showToast('❌ Deneme adı boş olamaz.'); return; }

  _loadDenemeler();

  if(_denemeModalEditId){
    var existing = _denemeler.find(function(d){ return d.id === _denemeModalEditId; });
    if(existing){
      existing.name = name; existing.brans = brans; existing.sinavTuru = sinav;
      existing.aytSub = aytSub; existing.dogru = dogru; existing.yanlis = yanlis;
      existing.net = net; existing.tarih = tarih;
    }
  } else {
    _denemeler.unshift({ id: uid(), name: name, brans: brans, sinavTuru: sinav, aytSub: aytSub,
      dogru: dogru, yanlis: yanlis, net: net, tarih: tarih, createdAt: Date.now() });
  }

  _saveDenemeler();
  closeDenemeResultModal();
  showToast('✅ Deneme kaydedildi!');
  if(calActiveTab === 'deneme') renderCalBody();
}

function closeDenemeResultModal(){
  var ov = document.getElementById('denemeResultModalOv');
  if(ov) ov.classList.remove('open');
  _denemeModalEditId = null;
}


/* ════════════════════════════════════════════════════════════
   MODÜL 3 — SIDEBAR MOD GEÇİŞİ (Playlist ↔ Deneme)
   ════════════════════════════════════════════════════════════ */

function setSidebarMode(mode){
  currentAppMode = mode;
  var btnPl = document.getElementById('sidebarModePlaylist');
  var btnDn = document.getElementById('sidebarModeDeneme');
  var plTabsWrap = document.querySelector('.pl-tabs-wrap');
  var plContent  = document.getElementById('plContent');
  var relatedBtn = document.getElementById('relatedTabBtn');
  var denemeArea = document.getElementById('sidebarDenemeArea');

  if(mode === 'deneme'){
    if(btnPl){ btnPl.style.background='transparent'; btnPl.style.color='rgba(196,198,208,0.45)'; btnPl.style.borderColor='rgba(255,255,255,0.08)'; }
    if(btnDn){ btnDn.style.background='rgba(245,200,66,0.15)'; btnDn.style.color='#f5c842'; btnDn.style.borderColor='rgba(245,200,66,0.25)'; }
    if(plTabsWrap) plTabsWrap.style.display='none';
    if(plContent) plContent.style.display='none';
    if(relatedBtn) relatedBtn.style.display='none';
    if(!denemeArea){ _createSidebarDenemeArea(); }
    else { denemeArea.style.display='flex'; _renderSidebarDenemeList(); }
  } else {
    if(btnPl){ btnPl.style.background='rgba(139,92,246,0.15)'; btnPl.style.color='#a78bfa'; btnPl.style.borderColor='rgba(255,255,255,0.12)'; }
    if(btnDn){ btnDn.style.background='transparent'; btnDn.style.color='rgba(196,198,208,0.45)'; btnDn.style.borderColor='rgba(255,255,255,0.08)'; }
    if(plTabsWrap) plTabsWrap.style.display='';
    if(plContent) plContent.style.display='';
    if(relatedBtn) relatedBtn.style.display='';
    if(denemeArea) denemeArea.style.display='none';
  }
}

function _createSidebarDenemeArea(){
  var playlistPanel = document.querySelector('.playlist-panel');
  if(!playlistPanel) return;

  var area = document.createElement('div');
  area.id = 'sidebarDenemeArea';
  area.style.cssText = 'display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden;';

  // Üst bar
  var topBar = document.createElement('div');
  topBar.style.cssText = 'display:flex;align-items:center;gap:7px;padding:8px 10px;border-bottom:1px solid rgba(255,255,255,0.05);';
  topBar.innerHTML =
    '<input type="text" id="sidebarDenemeSearch" placeholder="Deneme ara..." oninput="_renderSidebarDenemeList()" '+
      'style="flex:1;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:7px;padding:5px 9px;font-size:11px;color:var(--text);outline:none;font-family:inherit">'+
    '<button onclick="openDenemeResultModal(null)" title="Yeni Deneme Ekle" '+
      'style="background:rgba(245,200,66,0.1);border:1px solid rgba(245,200,66,0.25);border-radius:7px;color:#f5c842;cursor:pointer;padding:4px 8px;font-size:11px;font-weight:700;font-family:inherit;white-space:nowrap">'+
      '+ Ekle'+
    '</button>';
  ...
  list.id = 'sidebarDenemeList';
  list.style.cssText = 'flex:1;overflow-y:auto;padding:6px 0;';
  area.appendChild(list);

  // Playlist panelin içine, pl-progress-wrap'tan önce ekle
  var progressWrap = document.getElementById('plProgressWrap');
  if(progressWrap && progressWrap.parentNode === playlistPanel){
    playlistPanel.insertBefore(area, progressWrap);
  } else {
    playlistPanel.appendChild(area);
  }

  _renderSidebarDenemeList();
}

function _renderSidebarDenemeList(){
  var listEl = document.getElementById('sidebarDenemeList');
  if(!listEl) return;
  _loadDenemeler();

  var q = '';
  var searchEl = document.getElementById('sidebarDenemeSearch');
  if(searchEl) q = searchEl.value.toLowerCase().trim();

  var items = (_denemeler||[]).filter(function(d){
    if(!q) return true;
    return (d.name||'').toLowerCase().indexOf(q) >= 0 || (d.brans||'').toLowerCase().indexOf(q) >= 0;
  }).slice().sort(function(a,b){ return (b.tarih||0)-(a.tarih||0); });

  if(items.length === 0){
    listEl.innerHTML = '<div style="font-size:11px;color:var(--muted);text-align:center;padding:20px 10px">'+(q?'Sonuç yok.':'Henüz deneme yok.')+'</div>';
    return;
  }
  ...
  items.forEach(function(d){
    ...
    row.className = 'playlist-item'; // Playlist item stilini yeniden kullan
    row.style.cssText = 'cursor:pointer;padding:7px 10px;display:flex;align-items:center;gap:8px;';

    var net = typeof d.net === 'number' ? d.net.toFixed(1) : '?';
    var dt = d.tarih ? new Date(d.tarih) : null;
    var dateStr = dt ? pad(dt.getDate())+'.'+pad(dt.getMonth()+1) : '';
    var netColor = parseFloat(net) >= 0 ? 'var(--green)' : 'var(--accent)';

    row.innerHTML =
      '<span style="font-size:14px;flex-shrink:0">📝</span>'+
      '<div style="flex:1;min-width:0">'+
        '<div style="font-size:11px;font-weight:600;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escapeHtml(d.name)+'</div>'+
        '<div style="font-size:9px;color:var(--muted)">'+escapeHtml(d.brans||d.sinavTuru||'')+(dateStr?' · '+dateStr:'')+'</div>'+
      '</div>'+
      '<div style="font-size:11px;font-weight:700;color:'+netColor+';flex-shrink:0">'+net+'</div>';

    row.addEventListener('click', function(){ openDenemeResultModal(d.id); });
