...
var BACKUP_KEYS = [
  ...
  'aha_v4_history',       // 30 günlük izleme geçmişi
  'aha_v4_planner',       // Planlayıcı / takvim verileri
...
function ahaResolveAssetData(att){
  ...
  return fetch('https://www.googleapis.com/drive/v3/files/' + d.driveId + '?alt=media', {
    headers: { Authorization: 'Bearer ' + _ytAccessToken }
  })
  .then(function(r){ if (!r.ok) throw new Error('Drive\'dan indirilemedi (' + r.status + ')'); return r.blob(); })
  .then(function(blob){
    return new Promise(function(resolve, reject){
      var fr = new FileReader();
      fr.onload = function(){ _ahaAssetCache[d.driveId] = fr.result; resolve(fr.result); };
