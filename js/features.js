...
function renderAttachments(item,div){
  ...
  item.attachments.forEach(function(att,ai){
    ...
    var cloudTag=(typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(att.data))?' ☁️':'';
    chip.innerHTML=(att.type==='pdf'?'📄':'🖼')+cloudTag+'&nbsp;'+(att.name.length>18?att.name.substr(0,18)+'…':att.name)+'<button class="chip-del" title="Sil">✕</button>';
...
function deleteAttachment(item,ai,div){
  ...
  var removed=item.attachments[ai];
  item.attachments.splice(ai,1);
  renderAttachments(item,div);
  saveAll();
  // DRIVE ASSET STORAGE: ek Drive'da saklanıyorduysa orada da sil
  if(removed&&typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(removed.data)&&typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(removed.data);
}
function openAttachViewer(att){
  document.getElementById('attachViewTitle').textContent=att.name;
  var cont=document.getElementById('attachViewContent');
  // DRIVE ASSET STORAGE: att.data artık ya düz data-URI (yerel) ya da Drive
  // pointer'ı olabilir — göstermeden önce her zaman ahaResolveAssetData'dan geçir.
  cont.innerHTML='<div style="padding:60px 20px;text-align:center;color:var(--muted);font-size:13px">⏳ Yükleniyor...</div>';
  document.getElementById('attachViewModalOverlay').classList.add('open');
  ahaResolveAssetData(att).then(function(dataUrl){
    if(!document.getElementById('attachViewModalOverlay').classList.contains('open'))return; // bu sırada kapatıldıysa iptal
    cont.innerHTML='';
    if(att.type==='pdf'){
      var frame=document.createElement('iframe');frame.className='attach-viewer-frame';frame.src=dataUrl;frame.title=att.name;cont.appendChild(frame);
    }else{
      var img=document.createElement('img');img.className='attach-viewer-img';img.src=dataUrl;img.alt=att.name;cont.appendChild(img);
    }
  }).catch(function(err){
    cont.innerHTML='<div style="padding:40px 20px;text-align:center;color:var(--muted);font-size:13px">❌ '+escapeHtml(err&&err.message?err.message:'Dosya yüklenemedi')+'</div>';
  });
}
function closeAttachViewModal(){document.getElementById('attachViewModalOverlay').classList.remove('open');}

/* ══════════ BİRLEŞİK GÜNLÜK ARŞİV (Geçmiş + Planlayıcı + İstatistik özeti) ══════════
   NEDEN: eskiden "geçmiş" (dailyHistory) sadece son 30 günü tutup ÖTESİNİ KALICI
   OLARAK SİLİYORDU, "planlayıcı" (plannerData) ise hiç sınırlanmadan sonsuza dek
   büyüyordu. Artık bir gün geçmişe düştüğünde (gece yarısı) o günün verisi TEK bir
   arşive taşınır: hafif bir ÖZET (sonsuza dek saklanır — varsayılan grafik/liste
   görünümleri SADECE bunu okur, sürekli yeniden hesaplama yapılmaz) + isteğe bağlı
   bir DETAY (tam video/playlist dökümü — sadece kullanıcı bir günü açıkça
   genişlettiğinde/geri yüklemek istediğinde okunur). Detay, depolamayı şişirmemek
   için ARCHIVE_WATCH_DETAIL_DAYS gün sonra budanır ama ÖZET ASLA silinmez.
   Deneme (pratik sınav) verisi BURAYA taşınmaz/kopyalanmaz — Altın Kural gereği
   denemeExams zaten kalıcıdır ve hiç dokunulmaz; bir günün deneme özeti
   gerektiğinde doğrudan denemeExams'tan (bkz. archiveDenemeSummaryForDate) anlık
   hesaplanır, hiçbir yere kopyalanmaz. */
var LS_ARCHIVE='aha_v4_archive';
var archiveData={};              // {dateStr: {date, watch:{summary,detail|null}, planner:{summary,detail|null}}}
var ARCHIVE_WATCH_DETAIL_DAYS=60; // video/playlist dökümü + "geri yükle" bu kadar gün geriye kadar çalışır (eski sınır: 30)
var ARCHIVE_MIGRATED_LS_KEY='aha_archive_migrated_v1';

/* Eski sistem — artık SADECE tek seferlik göç (migration) kaynağı, bkz. loadArchive() */
var LS_HISTORY='aha_v4_history';
var dailyHistory={};
var MAX_HISTORY_DAYS=30;
var lastCheckedDate=null;
var lastCheckedMonth=null; // 'YYYY-MM' — planlayıcının ay geçişini takip eder
/* Ayarlar > "Günlük playlist & deneme gizlemesini durdur" — açıksa
   checkMidnightReset() playlistleri VE deneme kategorilerini artık
   otomatik gizlemez (bkz. checkMidnightReset, denemeApplyDailyCatHide). */
var stopDailyHideEnabled=false;
/* (2026-09) Ayarlar > "Kronometreyi Videodan Bağımsız Çalıştır" — açıksa
   core.js → handleStateChange() video oynatma/duraklatma/bitiş durumuna göre
   kronometreyi artık otomatik başlatmaz/durdurmaz; kronometre SADECE manuel
   S tuşu/butonuyla kontrol edilir (bkz. core.js). */
var swIndependentEnabled=false;
...
function loadHistory(){
  // Eski geçmiş artık SADECE göç kaynağı — bkz. loadArchive()
  ...
  try{var lcm=localStorage.getItem('aha_v4_lastmonth');lastCheckedMonth=lcm||getTodayStr().slice(0,7);}catch(e){lastCheckedMonth=getTodayStr().slice(0,7);}
  try{stopDailyHideEnabled=localStorage.getItem('aha_stop_daily_hide_v1')==='1';}catch(e){stopDailyHideEnabled=false;}
  try{swIndependentEnabled=localStorage.getItem('aha_sw_independent_v1')==='1';}catch(e){swIndependentEnabled=false;}
}

function loadArchive(){
  try{var raw=localStorage.getItem(LS_ARCHIVE);if(raw)archiveData=JSON.parse(raw);}catch(e){archiveData={};}
  /* TEK SEFERLİK GÖÇ: eski dailyHistory'de olup arşivde henüz olmayan günleri
     taşı — mevcut kullanıcılar güncellemeyle birlikte son 30 günlük geçmişini
     KAYBETMESİN. aha_archive_migrated_v1 işaretiyle idempotent (tekrar tekrar
     çalışmaz). */
  try{
    if(!localStorage.getItem(ARCHIVE_MIGRATED_LS_KEY)){
      Object.keys(dailyHistory).forEach(function(d){
        if(archiveData[d])return;
        var rec=dailyHistory[d];
        if(!rec)return;
        var videosWatched=0,videosTotal=0;
        (rec.playlists||[]).forEach(function(pl){
          (pl.items||[]).forEach(function(it){videosTotal++;if(it.watched)videosWatched++;});
        });
        archiveData[d]={date:d,
          watch:{summary:{swMs:rec.swMs||0,videosWatched:videosWatched,videosTotal:videosTotal},detail:{playlists:rec.playlists||[]}},
          planner:null
        };
      });
      localStorage.setItem(ARCHIVE_MIGRATED_LS_KEY,'1');
      saveArchive();
    }
  }catch(e){}
}
...
function saveArchive(){
  try{
    // Sadece DETAY'ı buda (ağır kısım — video/playlist dökümü) — ÖZET her zaman sonsuza dek kalır.
    var today=getTodayStr();
    Object.keys(archiveData).forEach(function(d){
      if(d>=today)return; // bugün/ileri tarih asla budanmaz
      var daysAgo=Math.round((new Date(today.replace(/-/g,'/'))-new Date(d.replace(/-/g,'/')))/86400000);
      if(daysAgo>ARCHIVE_WATCH_DETAIL_DAYS&&archiveData[d].watch&&archiveData[d].watch.detail){
        archiveData[d].watch.detail=null;
      }
    });
    localStorage.setItem(LS_ARCHIVE,JSON.stringify(archiveData));
  }catch(e){}
}

function ensureArchiveDay(dateStr){
  if(!archiveData[dateStr])archiveData[dateStr]={date:dateStr,watch:null,planner:null};
  return archiveData[dateStr];
}

/* O günün deneme (pratik sınav) özetini AN'lık hesaplar — hiçbir yere
   kopyalanmaz/kaydedilmez (deneme verisi asla arşivlenmez, her zaman
   denemeExams kendisi tek doğruluk kaynağıdır). */
function archiveDenemeSummaryForDate(dateStr){
  if(typeof denemeExams==='undefined'||!Array.isArray(denemeExams))return null;
  var dayExams=denemeExams.filter(function(ex){
    if(!ex.completed||!ex.completedAt)return false;
    return getDateStr(new Date(ex.completedAt))===dateStr;
  });
  if(dayExams.length===0)return null;
  var avgNet=dayExams.reduce(function(a,e){return a+(e.result?e.result.totalNet:0);},0)/dayExams.length;
  return {count:dayExams.length,avgNet:avgNet,examIds:dayExams.map(function(e){return e.id;})};
}
...
function snapshotToday(){
  ...
  var plSnap=playlists.map(function(pl){
    return {id:pl.id,name:pl.name,color:pl.color,categories:JSON.parse(JSON.stringify(pl.categories||[])),
      ...
      items:pl.items.map(function(it){return {id:it.id,title:it.title,watched:it.watched,watchedAt:it.watchedAt||null,categoryId:it.categoryId||null};})
    };
  });
  var videosWatchedToday=0,videosWatchedTotal=0,videosTotal=0;
  plSnap.forEach(function(pl){pl.items.forEach(function(it){
    videosTotal++;
    if(it.watched)videosWatchedTotal++;
    if(it.watchedAt&&getDateStr(new Date(it.watchedAt))===today)videosWatchedToday++;
  });});
  var day=ensureArchiveDay(today);
  day.watch={summary:{swMs:swMs,videosWatched:videosWatchedToday,videosWatchedTotal:videosWatchedTotal,videosTotal:videosTotal},detail:{playlists:plSnap}};
  saveArchive();
}
...
function checkMidnightReset(){
  ...
  if(lastCheckedDate&&lastCheckedDate!==today){
    // Snapshot the previous day before resetting — BİRLEŞİK ARŞİVE yaz (bkz. üstteki blok)
    ...
    // (2026-09) DEĞİŞİKLİK: 'watched' artık KALICI (aşağıda bir daha
    // sıfırlanmıyor) — bu yüzden videosWatched'i cumulative durumdan değil,
    // watchedAt'i TAM OLARAK bu güne (prev) denk gelen videolardan hesaplıyoruz;
    // aksi halde her gecenin özeti aynı büyüyen toplamı gösterir, "o gün kaç
    // video bitirdin" anlamını kaybederdi. videosWatchedTotal, o ana kadarki
    // KÜMÜLATİF tamamlanmayı ayrıca saklar (ileride ilerleme grafiği için —
    // deneme.js'teki completedAt/archiveDenemeSummaryForDate ile AYNI desen).
    var videosWatchedToday=0,videosWatchedTotal=0,videosTotal=0;
    plSnap.forEach(function(pl){pl.items.forEach(function(it){
      videosTotal++;
      if(it.watched)videosWatchedTotal++;
      if(it.watchedAt&&getDateStr(new Date(it.watchedAt))===prev)videosWatchedToday++;
    });});
    var prevDay=ensureArchiveDay(prev);
    prevDay.watch={summary:{swMs:swMs,videosWatched:videosWatchedToday,videosWatchedTotal:videosWatchedTotal,videosTotal:videosTotal},detail:{playlists:plSnap}};
    // GÜNLÜK ÇALIŞMA PLANI (to-do list) — gün bitince o günün planı canlı
    // (düzenlenebilir) 30 günlük pencereden çıkar, watch/planner ile AYNI
    // arşive taşınır (kalıcı geçmiş kaydı olarak kalır, silinmez).
    if(typeof todosData!=='undefined' && todosData[prev] && todosData[prev].items && todosData[prev].items.length>0){
      prevDay.todos={items:JSON.parse(JSON.stringify(todosData[prev].items))};
    }
    if(typeof todosData!=='undefined' && todosData[prev]){
      delete todosData[prev];
    ...
    // BUG FIX: artık SADECE stopDailyHideEnabled KAPALIYSA gizleniyor —
    // Ayarlar > "Günlük playlist & deneme gizlemesini durdur" açıksa bu adım
    // tamamen atlanır (bkz. core.js toggleStopDailyHideFromSettings).
    // (2026-09) BUG FIX — zaman damgası notları (pl.notes) ARTIK burada
    // sıfırlanmıyor. Eskiden hem gizleme hem gizleme-durdurma dalında
    // pl.notes={} çalışıyordu — kullanıcı bir videoya not ekleyip ertesi gün
    // (veya Ctrl+A+S testiyle) açtığında notların "silindiğini" fark ediyordu.
    // "Günlük temiz sayfa" felsefesi SADECE izlenme durumu (watched) için
    // tasarlanmıştı ama notlar aynı forEach'e yanlışlıkla dahil edilmişti.
    // Notlar artık flashcard/deneme kayıtları gibi KALICI kullanıcı verisi
    // sayılıyor (bkz. Altın Kural #6).
    // (2026-09) YENİ DEĞİŞİKLİK — kullanıcı isteğiyle: 'watched' (izlenme
    // durumu) ARTIK burada da sıfırlanmıyor. Bir video bir kez "izlendi"
    // işaretlenince (manuel ya da otomatik, bkz. core.js toggleWatched/
    // toggleWatchedByVideoId/_checkAutoMarkWatched/handleStateChange, ve
    // features.js toggleMgrVidWatched) bu durum KALICIDIR — notlar/flashcard/
    // deneme kayıtları gibi silinmeyen kullanıcı verisi sayılır (bkz. Altın
    // Kural #20). SADECE playlist GÖRÜNÜRLÜĞÜ (hidden) hâlâ bu ayara bağlı
    // olarak sıfırlanıyor — "temiz sayfa" felsefesi artık YALNIZCA görünürlük
    // + kronometre için geçerli, izlenme durumu için DEĞİL.
    if(!stopDailyHideEnabled){
      playlists.forEach(function(pl){ pl.hidden=true; });
    }
    // Planlayıcıdan "todolist" adıyla otomatik oluşturulan playlist kalıcı
    // DEĞİLDİR — her gece yarısı sıfırlamasında (bu ayarın durumundan
    // BAĞIMSIZ olarak, çünkü kalıcı olmaması bilinçli bir tasarım) tamamen
    // silinir. Bkz. TODO_AUTO_PLAYLIST_NAME / _todoEnsureAutoPlaylist (features.js).
    playlists=playlists.filter(function(pl){return pl.name!==TODO_AUTO_PLAYLIST_NAME;});
    if(playlists.length===0)initDefault();
    // Deneme kategorilerini de AYNI ayarla (stopDailyHideEnabled) günlük olarak
    // gizle — playlistlerle BİREBİR AYNI "temiz sayfa" felsefesi (bkz. deneme.js).
    if(!stopDailyHideEnabled && typeof denemeApplyDailyCatHide==='function'){
  ...
  checkMonthRolloverForPlanner(today);
}

/* AY GEÇİŞİ — planlayıcı takvimi SADECE "bu ay + gelecek ay"nı gösterir
   (bkz. renderPlannerTab); bir ay tamamen geride kaldığında o aya ait
   plannerData kayıtları artık takvimde HİÇBİR ZAMAN görünmez/tıklanamaz —
   bu yüzden ancak O ZAMAN güvenle arşive taşınıp canlı plannerData'dan
   silinebilirler (günlük değil, AYLIK bir tetikleyici — hâlâ bu ayın
   içindeyken, "geçmiş" günleri bile kullanıcı düzeltebilsin diye asla
   dokunulmaz). */
function checkMonthRolloverForPlanner(todayStr){
  var curMonth=todayStr.slice(0,7); // 'YYYY-MM'
  if(!lastCheckedMonth){ lastCheckedMonth=curMonth; try{localStorage.setItem('aha_v4_lastmonth',curMonth);}catch(e){} return; }
  if(lastCheckedMonth===curMonth)return;
  var changed=false;
  Object.keys(plannerData).forEach(function(d){
    if(d.slice(0,7)>=curMonth)return; // bu ay ya da ileri tarih — dokunma, takvimde hâlâ görünüyor
    var planned=plannerData[d];
    var plannedCount=(planned.playlists||[]).filter(function(p){return p.visible;}).length + (planned.extraVideos?planned.extraVideos.length:0);
    var day=ensureArchiveDay(d);
    day.planner={summary:{plannedCount:plannedCount},detail:{playlists:planned.playlists||[],extraVideos:planned.extraVideos||[]}};
    delete plannerData[d];
    changed=true;
  });
  if(changed){ saveArchive(); savePlanner(); }
  lastCheckedMonth=curMonth;
  try{localStorage.setItem('aha_v4_lastmonth',curMonth);}catch(e){}
}
...
/* ── Tam ekran (AI panelindeki _applyAiFullscreenPref/toggleAiFullscreen ile AYNI desen) ── */
var LS_CAL_FULLSCREEN = 'aha_cal_fullscreen_v1';
function _applyCalFullscreenPref(){
  var ov = document.getElementById('calPanelOverlay');
  var icon = document.getElementById('calFullscreenIcon');
  if(!ov) return;
  var on = localStorage.getItem(LS_CAL_FULLSCREEN) === '1';
  ov.classList.toggle('cal-fullscreen', on);
...
function toggleCalFullscreen(){
  ...
  var icon = document.getElementById('calFullscreenIcon');
  if(!ov) return;
  var on = !ov.classList.contains('cal-fullscreen');
  ov.classList.toggle('cal-fullscreen', on);
  localStorage.setItem(LS_CAL_FULLSCREEN, on ? '1' : '0');
  if(icon) icon.textContent = on ? 'fullscreen_exit' : 'fullscreen';
}

function switchCalTab(tab){
  calActiveTab=tab;
  var tabs = ['planner','history','stats','manager'];
  tabs.forEach(function(t){
    var el = document.getElementById('calTab'+t.charAt(0).toUpperCase()+t.slice(1));
...
function renderCalBody(){
  ...
  // NOT: "Planlayıcı" sekmesi artık modundan (playlist/deneme) bağımsız
  // olarak HER ZAMAN yeni to-do list sistemini gösterir — bkz. renderTodoPlannerTab
  // aşağıda "GÜNLÜK ÇALIŞMA PLANI" bölümünde. Eski renderPlannerTab() (ileri
  // tarihe playlist/video planlama) KASITLI olarak bir kenara kondu: silinmedi,
  // sadece buradan çağrılmıyor. Bkz. o fonksiyonun üstündeki not.
  else if(calActiveTab==='planner')renderTodoPlannerTab(body);
  else if(calActiveTab==='manager')renderManagerTab(body);
}

/* ── STATS TAB ── */
var statsRange=7; // default: last 7 days
/* Bir günün toplam çalışma süresini (ms) arşivin ÖZET kısmından okur — bu
   sayı sonsuza dek saklanır (detay budanmış olsa bile), bu yüzden grafik/liste
   gibi varsayılan görünümler asla ağır video dökümünü okumak zorunda kalmaz. */
function _archiveSwMsForDate(d){
  return (archiveData[d]&&archiveData[d].watch&&archiveData[d].watch.summary)?archiveData[d].watch.summary.swMs:0;
}

/* BUG FIX (bkz. Değişiklik Geçmişi): "Son 7/14/30 Gün" butonları ne seçilirse
   seçilsin hem üst istatistik kartları hem bar grafiği eskiden BAĞIMSIZ ve
   HATALI hesaplanıyordu — bar grafiği statsRange'i tamamen yok sayıp HER ZAMAN
   archiveData'daki en eski günden bugüne kadar (60 gün'e kadar) "Tüm Kayıtlar"
   gösteriyordu, bu yüzden aralık butonlarının grafik üzerinde HİÇBİR etkisi
   yoktu. Artık statsRange (7/14/30) BUGÜN dahil olmak üzere TAM statsRange
   adet ARDIŞIK TAKVİM GÜNÜ üretir — archiveData'da o gün için kayıt olmasa
   bile (örn. uygulama o gün hiç açılmadıysa) o gün 0dk olarak sayılır, "kaç
   günlük KAYIT var" değil "gerçekten son N takvim günü" esas alınır. Hem
   rangeKeys hem chartDays BURADAN türetilir — tek doğruluk kaynağı, ikisinin
   birbirinden sapmasını yapısal olarak imkansız kılar. */
function _getStatsRangeDays(range){
  var today=getTodayStr();
  var days=[];
  var endD=new Date(today+'T00:00:00');
  for(var i=range-1;i>=0;i--){
    var dcur=new Date(endD);
    dcur.setDate(dcur.getDate()-i);
    days.push(getDateStr(dcur));
  }
  return days; // eskiden-yeniye sıralı, son eleman her zaman bugün
}
...
function renderStatsTab(body){
  ...
  // Bar chart — SEÇİLİ ARALIĞI (statsRange: 7/14/30) gösterir. Eskiden bu
  // değişken statsRange'i tamamen yok sayıp HER ZAMAN en eski arşiv gününden
  // bugüne kadarki HER ŞEYİ (60 güne kadar) gösteriyordu — "Son 7 Gün"e
  // basmak ile "Son 30 Gün"e basmak arasında grafik açısından hiçbir fark
  // yoktu. Artık üstteki kartlarla (rangeKeys) AYNI kaynaktan (statsRange
  // gün sayısı kadar ardışık takvim günü) besleniyor.
  var chartDays=_getStatsRangeDays(statsRange);

  var chartMaxMs=chartDays.reduce(function(a,d){
    var ms2=d===today?todayMs:_archiveSwMsForDate(d);
    return Math.max(a,ms2);
  },1);

  var chartHtml='<div class="chart-wrap"><div class="chart-title">Son '+statsRange+' Gün <span style="font-size:10px;color:var(--muted);font-weight:400">(güne tıkla → detay)</span></div><div class="bar-chart" id="statsBarChart">';
...
function showStatsDayDetail(dateStr){
  ...
  var archDay=isToday?null:archiveData[dateStr];
  var watchDetail=archDay&&archDay.watch?archDay.watch.detail:null; // 60 günden eskiyse null olabilir — bkz. ARCHIVE_WATCH_DETAIL_DAYS
  var todayMs=swAccum+(swRunning?(Date.now()-swStartTime):0);
  var ms=isToday?todayMs:_archiveSwMsForDate(dateStr);

  var pls=isToday?playlists.map(function(pl){return {
    id:pl.id,name:pl.name,color:pl.color,
    notes:pl.notes||{},
    items:pl.items.map(function(it){return {id:it.id,title:it.title,watched:it.watched,watchedAt:it.watchedAt||null,categoryId:it.categoryId||null};})
  };}):((watchDetail&&watchDetail.playlists)||[]);
  ...
  if(pls.length===0){
    var detailWasPruned=!isToday&&archDay&&archDay.watch&&!archDay.watch.detail;
    if(detailWasPruned){
      html+='<div style="color:var(--muted);font-size:12px;padding:8px 0">📦 Bu günün video/playlist dökümü artık saklanmıyor (son '+ARCHIVE_WATCH_DETAIL_DAYS+' günden eski) — sadece toplam süre özeti yukarıda görünüyor.</div>';
    } else {
    ...
    pls.forEach(function(pl){
      ...
      pl.items.forEach(function(it){
        // (2026-09) 'watched' KALICI olduğundan, geçmiş bir günün detayında
        // görünen ✓'ler artık "o güne kadar biriken TOPLAM" demek — o günün
        // KENDİ aktivitesini (watchedAt tam o tarihe denk gelen videoları)
        // 🆕 ile ayırt ediyoruz (bkz. renderHistoryDayBody, AYNI mantık).
        var watchedToday=it.watchedAt&&getDateStr(new Date(it.watchedAt))===dateStr;
        var chk=it.watched?(watchedToday?'✓🆕':'✓'):'○';
        html+='<div class="stats-detail-vid'+(it.watched?' watched':'')+'">'+
          '<span class="stats-detail-chk">'+chk+'</span>'+
...
function renderHistoryTab(body){
  var today=getTodayStr();
  var keys=Object.keys(archiveData).filter(function(d){return archiveData[d].watch;}).sort().reverse(); // newest first
  ...
  // Filter by search
  var allKeys=Object.keys(archiveData).filter(function(d){return archiveData[d].watch;}).sort(); // oldest first for day numbering
  ...
  var infoHtml='<div style="font-size:11px;color:var(--muted);margin-bottom:8px">'+
    ...
    keys.length+' günün kaydı. Bir güne tıklayınca ayrıntıları (varsa) açılır, playlistleri geri yükleyebilirsiniz.')+
  '</div>';
  ...
  filteredKeys.forEach(function(dateStr){
    var archDay=archiveData[dateStr];
    var isToday=dateStr===today;
    var swMs=_archiveSwMsForDate(dateStr);
    ...
    html+='<div class="hist-day">'+
      '<div class="hist-day-header" onclick="toggleHistDay(\''+dateStr+'\')">'+
        ...
        '</div>'+
      '</div>'+
      /* PERFORMANS (kullanıcı isteği: "sürekli hesaplanmaya çalışmasın"):
         gövde BOŞ başlar — ağır video/playlist dökümü SADECE kullanıcı bu
         günü genişlettiğinde (bkz. toggleHistDay) inşa edilir. Eskiden bu
         HTML her gün için, gizli bile olsa, sekme her açıldığında TÜM
         geçmiş günler için yeniden hesaplanıp DOM'a basılıyordu. */
      '<div class="hist-day-body" id="hist-'+dateStr+'" data-loaded="0"></div>'+
    '</div>';
  });
...
function renderHistoryDayBody(rec,dateStr){
  var html='';
  ...
  pls.forEach(function(pl){
    ...
    pl.items.forEach(function(it){
      // (2026-09) 'watched' artık KALICI olduğundan bu liste zamanla SADECE
      // büyüyen kümülatif bir tamamlanmış-video listesine dönüşür — o günün
      // KENDİNE ÖZGÜ aktivitesini ayırt edebilmek için watchedAt tam olarak
      // bu tarihe denk gelen videoları "🆕 bugün" rozetiyle vurguluyoruz.
      var watchedToday=dateStr&&it.watchedAt&&getDateStr(new Date(it.watchedAt))===dateStr;
      var watched=it.watched?(watchedToday?'<span class="hist-vid-watched" title="Bu gün izlendi">✓🆕</span>':'<span class="hist-vid-watched" title="Daha önce izlenmiş">✓</span>'):'<span class="hist-vid-unwatched"></span>';
...
function toggleHistDay(dateStr){
  var el=document.getElementById('hist-'+dateStr);
  ...
  if(el.dataset.loaded!=='1'){
    // İLK açılış — ağır gövde SADECE ŞİMDİ, gerektiğinde inşa ediliyor (lazy)
    var archDay=archiveData[dateStr];
    var watchDetail=archDay&&archDay.watch?archDay.watch.detail:null;
    if(watchDetail){
      el.innerHTML='<button class="hist-restore-btn" onclick="restoreFromHistory(\''+dateStr+'\')">↩ Bu Günün Playlistlerini Geri Yükle</button>'+renderHistoryDayBody(watchDetail,dateStr);
    } else {
      el.innerHTML='<div style="color:var(--muted);font-size:11px;padding-top:6px">📦 Bu günün video/playlist dökümü artık saklanmıyor (son '+ARCHIVE_WATCH_DETAIL_DAYS+' günden eski) — sadece toplam süre özeti üstte görünüyor, geri yükleme de bu yüzden mümkün değil.</div>';
    }
    el.dataset.loaded='1';
  }
...
function restoreFromHistory(dateStr){
  ...
  var watchDetail=archDay&&archDay.watch?archDay.watch.detail:null;
  if(!watchDetail){
    showToast('❌ Bu günün ayrıntılı kaydı artık saklanmıyor (son '+ARCHIVE_WATCH_DETAIL_DAYS+' günden eski), geri yüklenemez.');
    return;
  }
  var rec=watchDetail; // {playlists:[...]} — doRestoreHistory/showRestoreChoiceModal zaten sadece bunu okuyor
  ...
  if(mode==='replace'){
    ...
    (rec.playlists||[]).forEach(function(hp){
      var newPl={id:hp.id,name:hp.name,color:hp.color||null,categories:hp.categories||[],notes:hp.notes||{},collapsedCats:{},catOrder:[],
        items:hp.items.map(function(it){return {id:it.id,title:it.title,watched:it.watched||false,watchedAt:it.watchedAt||null,categoryId:it.categoryId||null};})};
...
/* ══════════════════════════════════════════════════════════════════════════
 * GÜNLÜK ÇALIŞMA PLANI (TO-DO LIST) — 2026-08
 * "Planlayıcı" sekmesinin YENİ ana içeriği. Eski ileri-tarih playlist/video
 * planlayıcısının (aşağıda "BİR KENARA KONDU" notuyla duran renderPlannerTab)
 * ve deneme.js'teki ileri-tarih deneme planlayıcısının YERİNE geçer — o
 * ikisi silinmedi, sadece artık çağrılmıyor (bkz. renderCalBody).
 *
 * Veri modeli: todosData = { 'YYYY-MM-DD': { items: [ {...} ] } }
 * Bir item: {id, kind('study'|'video'|'deneme'|'custom'), title, subject,
 *            minutes, time('HH:MM'), done, doneAt, createdAt, order, source}
 *
 * Kapsam: SADECE bugünden başlayarak önündeki 30 gün (TODO_WINDOW_DAYS) —
 * kullanıcı isteği. Bir gün "bugün" olmaktan çıkınca (gece yarısı sıfırlama)
 * o günün verisi aha_v4_archive'a taşınır ve canlı todosData'dan silinir —
 * bkz. checkMidnightReset içindeki ekleme. Geçmiş günler bu yüzden burada
 * hiç görünmez (zaten pencere her zaman >= bugün).
 * ════════════════════════════════════════════════════════════════════════ */
var LS_TODOS = 'aha_v4_todos';
var LS_TODO_VIEWMODE = 'aha_todo_viewmode_v1';
var todosData = {};
var TODO_WINDOW_DAYS = 30; // bugün dahil en fazla 30 gün
var todoSelectedDate = null;
var todoViewMode = 'list'; // 'list' | 'table' — bkz. 2026-09 NOT: 'list' artık eski
                            // "tablo" görünümünün yerine geçti (basit satır listesi,
                            // sürükle-bırak ile), 'table' ise YENİ haftalık panodur.
var todoWeekOffset = 0;    // 'table' (haftalık) görünümde: 0=bugünün haftası, 1=gelecek hafta...
var todoActiveTimer = null; // {dateStr,itemId,totalMs,remainingMs,paused,intervalHandle}
var _todoDeleteScopePending = null;
var todoDragInfo = null;   // {dateStr,itemId} — sürüklenen görev (bkz. todoDragStart)
/* Planlayıcıdan doğrudan YouTube linki yapıştırarak (kendi playlistlerimizden
   biri yerine) bir göreve video atandığında, o video HİÇBİR playliste ait
   değilse otomatik olarak bu isimde bir playlist açılır ve video oraya
   eklenir (bkz. _todoEnsureAutoPlaylist). Bu playlist KALICI DEĞİLDİR —
   her gece yarısı sıfırlamasında tamamen silinir (bkz. checkMidnightReset). */
var TODO_AUTO_PLAYLIST_NAME = 'todolist';

var TODO_KIND_META = {
  study:  {icon:'📖', label:'Ders Çalışma', order:0},
  video:  {icon:'🎬', label:'Video',        order:1},
  deneme: {icon:'📝', label:'Deneme',       order:2},
  custom: {icon:'✅', label:'Genel Görev',  order:3}
};
function _todoKindMeta(kind){ return TODO_KIND_META[kind] || TODO_KIND_META.custom; }

/* Planlayıcıdan video atarken (kendi playlistlerimizden SEÇMEK yerine) doğrudan
   bir YouTube linki yapıştırıldığında ve o video o an HİÇBİR playlist'in
   içinde değilse, kalıcı olmayan bir "todolist" playlist'i otomatik açılır/
   yeniden kullanılır — video sanki oradaymış gibi (ve gerçekten de oraya
   eklenerek) açılabilsin diye. Zaten varsa mevcut olanı döndürür, aynı
   video ikinci kez eklenmeye çalışılırsa tekrar eklemez (dedup). */
function _todoEnsureAutoPlaylist(){
  var pl=playlists.find(function(p){return p.name===TODO_AUTO_PLAYLIST_NAME;});
  if(!pl){
    pl={id:uid(),name:TODO_AUTO_PLAYLIST_NAME,items:[],color:'#8b5cf6',categories:[],notes:{},collapsedCats:{},catOrder:[],hidden:false};
    playlists.push(pl);
  }
  if(pl.hidden){pl.hidden=false;}
  return pl;
}

function loadTodos(){
  try{ var raw=localStorage.getItem(LS_TODOS); todosData = raw ? (JSON.parse(raw)||{}) : {}; }catch(e){ todosData={}; }
  try{ var vm=localStorage.getItem(LS_TODO_VIEWMODE); if(vm==='list'||vm==='table') todoViewMode=vm; }catch(e){}
}
function saveTodos(){
  if(typeof ahaSafeSetItem==='function') ahaSafeSetItem(LS_TODOS, todosData);
  else{ try{ localStorage.setItem(LS_TODOS, JSON.stringify(todosData)); }catch(e){} }
}
function _todoEnsureDay(dateStr){
  if(!todosData[dateStr]) todosData[dateStr]={items:[]};
  if(!todosData[dateStr].items) todosData[dateStr].items=[];
  return todosData[dateStr];
}
function _todoAddDaysStr(dateStr,n){
  var p=dateStr.split('-').map(Number);
  var d=new Date(p[0],p[1]-1,p[2]);
  d.setDate(d.getDate()+n);
  return getDateStr(d);
}
function _todoWindowEndStr(){ return _todoAddDaysStr(getTodayStr(), TODO_WINDOW_DAYS-1); }
function _todoInWindow(dateStr){
  var today=getTodayStr();
  return dateStr>=today && dateStr<=_todoWindowEndStr();
}
/* Ortak sıralama: saati olan görevler saate göre (en erken ilk), saati
   olmayanlar `order` alanına göre ve HER ZAMAN saatli görevlerden SONRA
   gelir. Liste VE haftalık tablo görünümü BU AYNI sıralamayı kullanır.
   Sürükle-bırak sadece `order` değerini (ve saatsiz görevlerin arraydeki
   göreli konumunu) değiştirir — bkz. todoDrop / _todoValidateDrop. */
function _todoItemSortCmp(a,b){
...
/* ── ANA RENDER ── */
function renderTodoPlannerTab(body){
  if(!todoSelectedDate || !_todoInWindow(todoSelectedDate)) todoSelectedDate=getTodayStr();
  var html='<div class="todo-wrap">';
  html+='<div class="todo-head"><div class="todo-head-title">📋 Günlük Çalışma Planı</div>'+
    '<div class="todo-head-sub">Bugünden başlayarak önündeki en fazla 30 gün için plan yazabilirsin. Gün bitince o günün planı otomatik olarak geçmişe (Geçmiş sekmesi verisiyle birlikte) arşivlenir.</div></div>';
  if(todoActiveTimer){
    var d=todosData[todoActiveTimer.dateStr];
    var it=d?d.items.find(function(x){return x.id===todoActiveTimer.itemId;}):null;
    html+='<div class="todo-active-timer-banner" onclick="_todoOpenTimerModal()">⏱ Aktif zamanlayıcı: <b>'+escapeHtml(it?it.title:'')+'</b> — '+_todoMsToClock(todoActiveTimer.remainingMs)+' kaldı <span class="todo-active-timer-open">Göster ›</span></div>';
  }
  html+=_todoRenderStrip();
  html+='<div id="todoDetailWrap">'+_todoRenderDetail(todoSelectedDate)+'</div>';
  html+=_todoRenderFooterActions();
  html+='</div>';
  body.innerHTML=html;
  // Seçili günü şeritte görünür alana kaydır
  setTimeout(function(){
    var sel=body.querySelector('.todo-strip-day.selected');
    if(sel && typeof sel.scrollIntoView==='function') sel.scrollIntoView({inline:'center', block:'nearest', behavior:'auto'});
  },0);
  // (2026-09) Tablo görünümü — bugün Cuma/Cumartesi/Pazar ise (ve içinde
  // bulunulan/bugünü içeren hafta gösteriliyorsa) haftalık panoyu baştan
  // SAĞA kaydırılmış aç — kullanıcı hafta sonunu görmek için her seferinde
  // elle kaydırmak zorunda kalmasın.
  if(todoViewMode==='table' && todoWeekOffset===0){
    var todayDow=new Date().getDay(); // 0=Pazar,5=Cuma,6=Cumartesi
    if(todayDow===0 || todayDow===5 || todayDow===6){
      setTimeout(function(){
        var grid=body.querySelector('.todo-week-grid');
...
function _todoRenderStrip(){
  var today=getTodayStr();
  var dowShort=['Pz','Pt','Sa','Ça','Pe','Cu','Ct'];
  var html='<div class="todo-strip-wrap">'+
    '<button class="todo-strip-nav" onclick="todoScrollStrip(-1)" aria-label="Geri">‹</button>'+
    '<div class="todo-strip" id="todoStrip">';
  for(var i=0;i<TODO_WINDOW_DAYS;i++){
    var d=new Date(); d.setDate(d.getDate()+i);
    var dateStr=getDateStr(d);
    var day=todosData[dateStr];
    var total=day?day.items.length:0;
    var doneCount=day?day.items.filter(function(it){return it.done;}).length:0;
    var isSel=dateStr===todoSelectedDate;
    var cls='todo-strip-day'+(dateStr===today?' is-today':'')+(isSel?' selected':'')+(total>0?' has-items':'')+(total>0&&doneCount===total?' all-done':'');
    html+='<div class="'+cls+'" onclick="todoSelectDay(\''+dateStr+'\')">'+
      '<div class="todo-strip-dow">'+(dateStr===today?'BUGÜN':dowShort[d.getDay()])+'</div>'+
      '<div class="todo-strip-num">'+d.getDate()+'</div>'+
      (total>0?'<div class="todo-strip-badge">'+doneCount+'/'+total+'</div>':'<div class="todo-strip-badge empty">·</div>')+
    '</div>';
  }
  html+='</div><button class="todo-strip-nav" onclick="todoScrollStrip(1)" aria-label="İleri">›</button></div>';
  return html;
}
function todoScrollStrip(dir){
  var strip=document.getElementById('todoStrip');
  if(strip) strip.scrollBy({left: dir*220, behavior:'smooth'});
}
function todoSelectDay(dateStr){
  if(!_todoInWindow(dateStr)) return;
  todoSelectedDate=dateStr;
  todoWeekOffset=_todoDateToWeekOffset(dateStr);
  renderCalBody();
}

/* ── HAFTA MATEMATİĞİ (YENİ tablo/haftalık görünüm için) ──
   Haftalar Pazartesi'den başlar (bkz. ekteki "Haftalık Planım" görseli).
   offset=0 → bugünü içeren hafta, offset=1 → gelecek hafta, vb.
   Kullanıcı geçmiş haftalara gidemez (bkz. todoWeekNav). */
function _todoWeekMonday(offset){
  var d=new Date();
  var dow=d.getDay(); // 0=Pazar..6=Cumartesi
  var deltaToMonday=(dow===0?-6:1-dow); // bugünü o haftanın Pazartesi'sine çek
  d.setDate(d.getDate()+deltaToMonday+ (offset*7));
  return getDateStr(d);
}
function _todoDateToWeekOffset(dateStr){
  var thisMonday=_todoWeekMonday(0);
  var p1=thisMonday.split('-').map(Number), p2=dateStr.split('-').map(Number);
  var d1=new Date(p1[0],p1[1]-1,p1[2]), d2=new Date(p2[0],p2[1]-1,p2[2]);
  var diffDays=Math.round((d2-d1)/86400000);
  return Math.floor(diffDays/7);
}
function todoWeekNav(dir){
  var next=todoWeekOffset+dir;
  if(next<0)return; // güncel haftanın öncesine gidilemez
  todoWeekOffset=next;
  renderCalBody();
}

/* ── GÜN DETAY PANELİ ── */
function _todoRenderDetail(dateStr){
  var day=todosData[dateStr];
  var items=day?day.items.slice():[];
  items.sort(_todoItemSortCmp);
  var doneCount=items.filter(function(it){return it.done;}).length;
  var today=getTodayStr();

  var html='<div class="todo-detail">';

  html+='<div class="todo-view-toggle">'+
    '<button class="todo-view-btn'+(todoViewMode==='list'?' active':'')+'" onclick="todoSetViewMode(\'list\')">☰ Liste</button>'+
    '<button class="todo-view-btn'+(todoViewMode==='table'?' active':'')+'" onclick="todoSetViewMode(\'table\')">▦ Tablo</button>'+
  '</div>';

  if(todoViewMode==='table'){
    // YENİ haftalık pano görünümü — tek bir günle değil, o an seçili haftayla
    // ilgilenir (bkz. todoWeekOffset / _todoRenderWeeklyTable).
    html+=_todoRenderWeeklyTable();
    html+='</div>';
    return html;
  }

  html+='<div class="todo-detail-head"><div class="todo-detail-date">'+formatDateHuman(dateStr)+
    (dateStr===today?' <span class="todo-today-pill">Bugün</span>':'')+'</div>'+
    (items.length>0?'<div class="todo-progress-pill">'+doneCount+' / '+items.length+' tamamlandı</div>':'')+
  '</div>';

  html+='<div class="todo-add-form">'+_todoRenderAddForm(dateStr)+'</div>';

  if(items.length===0){
    html+='<div class="todo-empty">Bu güne henüz bir plan eklenmedi. Yukarıdan ekleyebilir, ya da AI panelindeki <b>Plan Tanıma</b> ile haftalık plan fotoğrafından otomatik doldurabilirsin.</div>';
  }else{
    html+='<div class="todo-list">'+items.map(function(it,i){return _todoRenderListRow(dateStr,it,i,items);}).join('')+'</div>';
  }
  html+='</div>';
  return html;
}

/* Planlayıcı ekleme formunda kind='video'/'deneme' seçiliyken geçici olarak
   tutulan seçim — "Ekle" butonuna basılınca todoAddItem() buradan okur,
   eklendikten (ya da form kapanınca) sonra temizlenir. Tarih (dateStr) bazlı
   tutulur ki haftalık pano görünümünde aynı anda birden fazla günün formu
   açık olsa bile karışmasın. */
var _todoPendingRef = {}; // { 'YYYY-MM-DD': {videoRef:{plId,vidId,title}|null, denemeRef:{examId,catId,title}|null} }
/* (2026-09) Planlayıcıdan açılan bir denemenin sonucu kaydedilince görevi
   otomatik "yapıldı" işaretlemek için — bkz. todoOpenDenemeRef / deneme.js
   denemeSaveScore. Video karşılığı (_todoActiveVideoTask) core.js'te. */
var _todoActiveDenemeTask = null; // {dateStr,itemId,examId}

function _todoRenderAddForm(dateStr){
  var sid=dateStr.replace(/-/g,'');
  var pend=_todoPendingRef[dateStr]||{};
  var vidLabel=pend.videoRef?('🎬 '+escapeHtml(pend.videoRef.title)):'Henüz video seçilmedi';
  var dnmLabel=pend.denemeRef?('📝 '+escapeHtml(pend.denemeRef.title)):'Henüz deneme seçilmedi';
  return '<div class="todo-add-row1">'+
      '<select class="todo-add-kind" id="todoAddKind_'+sid+'" onchange="_todoOnAddKindChange(\''+dateStr+'\')">'+
        '<option value="study">📖 Ders</option>'+
        '<option value="video">🎬 Video</option>'+
        '<option value="deneme">📝 Deneme</option>'+
        '<option value="custom">✅ Genel</option>'+
      '</select>'+
      '<input type="text" class="todo-add-title" id="todoAddTitle_'+sid+'" placeholder="Ne yapacaksın? örn: Hücre konusu tekrar" onkeydown="if(event.key===\'Enter\'){event.preventDefault();todoAddItem(\''+dateStr+'\');}">'+
    '</div>'+
    '<div class="todo-add-row2" id="todoAddRow2Text_'+sid+'">'+
      '<input type="text" class="todo-add-subject" id="todoAddSubject_'+sid+'" placeholder="Ders (ops.)" onkeydown="if(event.key===\'Enter\'){event.preventDefault();todoAddItem(\''+dateStr+'\');}">'+
    '</div>'+
    '<div class="todo-add-row2 todo-add-row2-ref" id="todoAddRow2Video_'+sid+'" style="display:none">'+
      '<button type="button" class="todo-add-pick-btn" onclick="todoOpenVideoPickerForAdd(\''+dateStr+'\')">🎬 Video Seç</button>'+
      '<span class="todo-add-picked-label" id="todoAddPickedVideo_'+sid+'">'+vidLabel+'</span>'+
    '</div>'+
    '<div class="todo-add-row2 todo-add-row2-ref" id="todoAddRow2Deneme_'+sid+'" style="display:none">'+
      '<button type="button" class="todo-add-pick-btn" onclick="todoOpenDenemePickerForAdd(\''+dateStr+'\')">📝 Deneme Seç</button>'+
      '<span class="todo-add-picked-label" id="todoAddPickedDeneme_'+sid+'">'+dnmLabel+'</span>'+
    '</div>'+
    '<div class="todo-add-row3">'+
      '<input type="number" min="1" max="600" class="todo-add-minutes" id="todoAddMinutes_'+sid+'" placeholder="dk" title="Süre (dk) — Ders/Genel görevlerde üstüne basınca zamanlayıcı açılır" onkeydown="if(event.key===\'Enter\'){event.preventDefault();todoAddItem(\''+dateStr+'\');}">'+
      '<input type="time" class="todo-add-time" id="todoAddTime_'+sid+'" title="Saat (ops.)">'+
      '<button class="todo-add-btn" onclick="todoAddItem(\''+dateStr+'\')">＋ Ekle</button>'+
    '</div>';
}

/* BUG FIX (kullanıcı isteği): video/deneme görevleri eskiden "ders adı"/
   "görev adı" gibi anlamsız serbest metin alanlarıyla ekleniyordu. Artık
   kind seçimine göre doğru alan grubu gösteriliyor: video/deneme seçilince
   başlık YAZILMAZ, doğrudan kendi playlist'lerimizden/denemelerimizden
   SEÇİLİR (ya da video için YouTube linki yapıştırılır) — bkz.
   todoOpenVideoPickerForAdd / todoOpenDenemePickerForAdd. */
function _todoOnAddKindChange(dateStr){
  var sid=dateStr.replace(/-/g,'');
  var kind=document.getElementById('todoAddKind_'+sid).value;
  var titleInp=document.getElementById('todoAddTitle_'+sid);
  var rowText=document.getElementById('todoAddRow2Text_'+sid);
  var rowVideo=document.getElementById('todoAddRow2Video_'+sid);
  var rowDeneme=document.getElementById('todoAddRow2Deneme_'+sid);
  var isRef=(kind==='video'||kind==='deneme');
  if(titleInp){ titleInp.style.display=isRef?'none':''; }
  if(rowText) rowText.style.display=isRef?'none':'';
  if(rowVideo) rowVideo.style.display=(kind==='video')?'':'none';
  if(rowDeneme) rowDeneme.style.display=(kind==='deneme')?'':'none';
}

function todoAddItem(dateStr){
  if(!_todoInWindow(dateStr)){ showToast('⚠️ Sadece önündeki 30 gün için plan ekleyebilirsin.'); return; }
  var sid=dateStr.replace(/-/g,'');
  var kindSel=document.getElementById('todoAddKind_'+sid);
  var kind=kindSel?kindSel.value:'custom';
  var titleInp=document.getElementById('todoAddTitle_'+sid);
  var subjInp=document.getElementById('todoAddSubject_'+sid);
  var minInp=document.getElementById('todoAddMinutes_'+sid);
  var timeInp=document.getElementById('todoAddTime_'+sid);
  var pend=_todoPendingRef[dateStr]||{};

  var newItem={
    id:'todo_'+Date.now().toString(36)+'_'+Math.floor(Math.random()*100000),
    kind:kind, title:null, subject:null, videoRef:null, denemeRef:null,
    subjectColor:null, titleColor:null,
    minutes:null, time:null, done:false, doneAt:null, createdAt:Date.now(), order:0, source:'manual'
  };

  if(kind==='video'){
    if(!pend.videoRef){ showToast('⚠️ Önce "Video Seç" ile bir video seç.'); return; }
    newItem.title=pend.videoRef.title;
    newItem.videoRef={plId:pend.videoRef.plId,vidId:pend.videoRef.vidId,title:pend.videoRef.title};
  } else if(kind==='deneme'){
    if(!pend.denemeRef){ showToast('⚠️ Önce "Deneme Seç" ile bir deneme seç.'); return; }
    newItem.title=pend.denemeRef.title;
    newItem.denemeRef={examId:pend.denemeRef.examId,catId:pend.denemeRef.catId,title:pend.denemeRef.title};
  } else {
    if(!titleInp || !titleInp.value.trim()){ showToast('⚠️ Görev açıklaması boş olamaz.'); if(titleInp) titleInp.focus(); return; }
    newItem.title=titleInp.value.trim();
    newItem.subject=(subjInp && subjInp.value.trim()) ? subjInp.value.trim() : null;
  }

  if(minInp && minInp.value){ newItem.minutes=Math.max(1,Math.min(600,parseInt(minInp.value,10)||0)); }
  if(timeInp && timeInp.value){ newItem.time=timeInp.value; }

  var day=_todoEnsureDay(dateStr);
  _todoInsertChronologically(day.items,newItem);
  saveTodos();
  delete _todoPendingRef[dateStr];
  renderCalBody();
  showToast('✅ Plana eklendi.');
}

/* Saati olan bir görev eklenince "otomatik olarak en başa/doğru sıraya
   koyulsun" isteği: diğer SAATLİ görevler arasında kronolojik doğru
   pozisyona eklenir. Saati yoksa en sona eklenir (order en büyük değer+1) —
   kullanıcı isterse sonradan sürükleyip taşıyabilir. */
function _todoInsertChronologically(items,newItem){
  if(newItem.time){
    var insertAt=items.length;
    for(var i=0;i<items.length;i++){
      if(items[i].time && items[i].time>newItem.time){ insertAt=i; break; }
    }
    items.splice(insertAt,0,newItem);
  } else {
    items.push(newItem);
  }
  _todoRenumberOrder(items);
}
function _todoRenumberOrder(items){
  items.forEach(function(it,i){ it.order=i; });
}

/* YENİ "Liste" görünümü — eskiden "Tablo" idi, kullanıcı isteğiyle eski basit
   checklist görünümünün YERİNE geçti (o tamamen kaldırıldı). Artık: sürükle-
   bırak ile sıralama (bkz. todoDragStart/todoDrop), ders adı/görev adı ayrı
   ayrı renklendirilebilir (bkz. todoOpenSubjectColorPicker/
   todoOpenTitleColorPicker), video/deneme görevlerine tıklayınca ana ekranda
   AÇILIR (bkz. todoOpenVideoRef/todoOpenDenemeRef) — ders/genel görevlerde
   süre girildiyse eskisi gibi zamanlayıcı açılır. */
function _todoRenderListRow(dateStr,it,idx,items){
  var meta=_todoKindMeta(it.kind);
  var isRefKind=(it.kind==='video'||it.kind==='deneme');
  var chips='';
  if(!isRefKind && it.subject) chips+='<span class="todo-chip" style="'+(it.subjectColor?('color:'+it.subjectColor+';background:'+it.subjectColor+'1f'):'')+'">'+escapeHtml(it.subject)+'</span>';
  if(it.time) chips+='<span class="todo-chip todo-chip-time">🕐 '+it.time+'</span>';
  if(it.minutes) chips+='<span class="todo-chip todo-chip-min">⏱ '+it.minutes+' dk'+(!isRefKind&&!it.done?' · başlat':'')+'</span>';
  /* AI "Plan Tanıma" ile fotoğraftan okunan video/deneme görevlerinde henüz
     GERÇEK bir video/deneme atanmamış olabilir (AI sadece türünü tahmin eder,
     hangi videoyu/denemeyi kastettiğini bilemez) — bu durumda tıklanabilir
     gibi göstermek yerine açıkça "atanmadı" uyarısı gösterilir. */
  if(it.kind==='video' && !it.videoRef) chips+='<span class="todo-chip todo-chip-warn">⚠️ video atanmadı</span>';
  if(it.kind==='deneme' && !it.denemeRef) chips+='<span class="todo-chip todo-chip-warn">⚠️ deneme atanmadı</span>';

  var bodyClickAttr='';
  if(it.kind==='video' && it.videoRef) bodyClickAttr=' onclick="todoOpenVideoRef(\''+dateStr+'\',\''+it.id+'\')" style="cursor:pointer" title="Videoyu Aç"';
  else if(it.kind==='deneme' && it.denemeRef) bodyClickAttr=' onclick="todoOpenDenemeRef(\''+dateStr+'\',\''+it.id+'\')" style="cursor:pointer" title="Denemeyi Aç"';
  else if(!isRefKind && it.minutes && !it.done) bodyClickAttr=' onclick="todoStartTimer(\''+dateStr+'\',\''+it.id+'\')" style="cursor:pointer" title="Zamanlayıcıyı başlat"';

  var titleStyle=it.titleColor?(' style="color:'+it.titleColor+'"'):'';
  var subjColorBtn=(!isRefKind)?'<button class="todo-color-btn" onclick="event.stopPropagation();todoOpenSubjectColorPicker(event,\''+dateStr+'\',\''+it.id+'\')" title="Ders Adı Rengi">🎨</button>':'';

  return '<div class="todo-item'+(it.done?' done':'')+'" id="todoitem-'+dateStr+'-'+it.id+'" draggable="true" '+
    'ondragstart="todoDragStart(event,\''+dateStr+'\',\''+it.id+'\')" ondragend="todoDragEnd(event)" '+
    'ondragover="todoDragOver(event,\''+dateStr+'\',\''+it.id+'\')" ondragleave="todoDragLeave(event)" '+
    'ondrop="todoDrop(event,\''+dateStr+'\',\''+it.id+'\')">'+
    '<span class="todo-drag-handle" title="Sürükle">⠿</span>'+
    '<div class="todo-item-check'+(it.done?' checked':'')+'" onclick="todoToggleDone(\''+dateStr+'\',\''+it.id+'\')">'+(it.done?'✓':'')+'</div>'+
    '<div class="todo-item-body"'+bodyClickAttr+'>'+
      '<div class="todo-item-title"'+titleStyle+'>'+meta.icon+' '+escapeHtml(it.title)+
        '<button class="todo-color-btn" onclick="event.stopPropagation();todoOpenTitleColorPicker(event,\''+dateStr+'\',\''+it.id+'\')" title="Görev Adı Rengi">🎨</button>'+
      '</div>'+
      (chips?'<div class="todo-item-meta">'+chips+subjColorBtn+'</div>':(subjColorBtn?'<div class="todo-item-meta">'+subjColorBtn+'</div>':''))+
    '</div>'+
    '<button class="todo-item-del" onclick="todoDeleteItem(\''+dateStr+'\',\''+it.id+'\')" title="Sil">🗑</button>'+
  '</div>';
}

/* ── SÜRÜKLE-BIRAK (Liste + Haftalık Tablo — bkz. _todoItemSortCmp) ──
   Kural: iki SAATLİ görev birbirine göre kronolojik sırasını KORUMALI —
   saat 11:58 olan bir görev saat 10:00 olanın önüne taşınamaz. Görevlerden
   biri (veya ikisi) saatsizse serbestçe herhangi bir yere sürüklenebilir. */
function todoDragStart(e,dateStr,itemId){
  todoDragInfo={dateStr:dateStr,itemId:itemId};
  e.dataTransfer.effectAllowed='move';
  try{ e.dataTransfer.setData('text/tododrag',itemId); }catch(err){}
  e.stopPropagation();
  setTimeout(function(){var el=document.getElementById('todoitem-'+dateStr+'-'+itemId);if(el)el.classList.add('todo-dragging');},0);
}
function todoDragEnd(e){
  document.querySelectorAll('.todo-item.todo-dragging').forEach(function(el){el.classList.remove('todo-dragging');});
  document.querySelectorAll('.todo-item.todo-drag-over').forEach(function(el){el.classList.remove('todo-drag-over');});
  todoDragInfo=null;
}
function todoDragOver(e,dateStr,itemId){
  if(!todoDragInfo||todoDragInfo.dateStr!==dateStr||todoDragInfo.itemId===itemId)return;
  e.preventDefault();e.stopPropagation();
  document.querySelectorAll('.todo-item.todo-drag-over').forEach(function(el){el.classList.remove('todo-drag-over');});
  var el=document.getElementById('todoitem-'+dateStr+'-'+itemId);if(el)el.classList.add('todo-drag-over');
}
function todoDragLeave(e){
  if(e.currentTarget&&e.currentTarget.contains&&!e.currentTarget.contains(e.relatedTarget)) e.currentTarget.classList.remove('todo-drag-over');
}
function todoDrop(e,dateStr,targetItemId){
  e.preventDefault();e.stopPropagation();
  document.querySelectorAll('.todo-item.todo-drag-over').forEach(function(el){el.classList.remove('todo-drag-over');});
  if(!todoDragInfo||todoDragInfo.dateStr!==dateStr||todoDragInfo.itemId===targetItemId){todoDragInfo=null;return;}
  var day=todosData[dateStr];if(!day){todoDragInfo=null;return;}
  // Ekranda GÖRÜNEN (saat-öncelikli) sırayı temel al, sürüklemeyi o sıra
  // üzerinde uygula — array'in ham (kayıtlı) sırası görüntülenenle
  // eşleşmeyebilir.
  var sorted=day.items.slice().sort(_todoItemSortCmp);
  var fromIdx=sorted.findIndex(function(x){return x.id===todoDragInfo.itemId;});
  var toIdx=sorted.findIndex(function(x){return x.id===targetItemId;});
  if(fromIdx<0||toIdx<0){todoDragInfo=null;return;}
  var moved=sorted.splice(fromIdx,1)[0];
  sorted.splice(toIdx,0,moved);
  if(!_todoValidateTimeOrder(sorted)){
    showToast('⚠️ Saat sırasını bozan bir taşıma yapamazsın.');
    todoDragInfo=null;
    return;
  }
  _todoRenumberOrder(sorted);
  day.items=sorted;
  todoDragInfo=null;
  saveTodos();
  renderCalBody();
}
/* SAATLİ görevlerin birbirine göre kronolojik sırası bozulmuş mu kontrol
   eder (saatsiz görevler atlanır, hiçbir kısıtlama getirmezler). */
function _todoValidateTimeOrder(sortedItems){
  var lastTime=null;
  for(var i=0;i<sortedItems.length;i++){
    var t=sortedItems[i].time;
    if(t){
      if(lastTime && t<lastTime) return false;
      lastTime=t;
    }
  }
  return true;
...
/* ── RENK SEÇİCİ (ders adı / görev adı) — mevcut showColorPicker/
   colorPickerCallback mekanizmasını kullanır (bkz. mgrOpenCatColorPicker). ── */
function todoOpenSubjectColorPicker(e,dateStr,itemId){
  e.stopPropagation();e.preventDefault();
  var day=todosData[dateStr];if(!day)return;
  var it=day.items.find(function(x){return x.id===itemId;});if(!it)return;
  var btn=e.currentTarget||e.target;
  colorPickerCallback=function(color){
    it.subjectColor=color;saveTodos();renderCalBody();
  };
  showColorPicker(btn,it.subjectColor,null,null);
}
function todoOpenTitleColorPicker(e,dateStr,itemId){
  e.stopPropagation();e.preventDefault();
  var day=todosData[dateStr];if(!day)return;
  var it=day.items.find(function(x){return x.id===itemId;});if(!it)return;
  var btn=e.currentTarget||e.target;
  colorPickerCallback=function(color){
    it.titleColor=color;saveTodos();renderCalBody();
  };
  showColorPicker(btn,it.titleColor,null,null);
}

/* ── GÖREVE TIKLAYINCA VİDEO/DENEME AÇMA — bkz. istem: "playlist sistemiyle
   AYNI mantık": playlist/kategori gizliyse önce gizliliği açılır, sonra
   sanki oradan açılmış gibi ana ekranda oynatılır/başlatılır. openVideoFromNoteRef
   (notes.js) ile BİREBİR AYNI desen. ── */
function todoOpenVideoRef(dateStr,itemId){
  var day=todosData[dateStr];if(!day)return;
  var it=day.items.find(function(x){return x.id===itemId;});if(!it||!it.videoRef)return;
  closeCalendarPanel();
  var ref=it.videoRef;
  // (2026-09) Video %90 izlenince bu görevi otomatik tamamla — bkz. core.js
  // _todoCheckVideoAutoComplete / handleVideoChange.
  if(!it.done && typeof _todoActiveVideoTask!=='undefined') _todoActiveVideoTask={dateStr:dateStr,itemId:itemId,vidId:ref.vidId};
  var targetPl=playlists.find(function(p){return p.id===ref.plId;});
  if(targetPl){
    if(targetPl.hidden){
      targetPl.hidden=false;saveAll();
      showToast('🔓 "'+targetPl.name+'" playlisti tekrar görünür yapıldı.');
    }
    var idx=targetPl.items.findIndex(function(x){return x.id===ref.vidId;});
    if(activePlaylistId!==ref.plId) switchPlaylist(ref.plId);
    if(idx>=0) playPlaylistVideo(idx);
    else loadVideoInPlayer(ref.vidId);
  } else {
    loadVideoInPlayer(ref.vidId);
  }
}
/* ── GÖREVE TIKLAYINCA DENEME AÇMA — "başlat tuşuna basmışız gibi" ana ekrana
   yönlendirir; kategori gizliyse açılır (bkz. istem). Sınav zaten
   tamamlanmışsa (ex.completed) doğrudan sonuç/skor ekranını açar — kart
   listesindeki (denemeExamCardHtml) davranışla TUTARLI. ── */
function todoOpenDenemeRef(dateStr,itemId){
  var day=todosData[dateStr];if(!day)return;
  var it=day.items.find(function(x){return x.id===itemId;});if(!it||!it.denemeRef)return;
  var ref=it.denemeRef;
  var ex=(typeof denemeFindExam==='function')?denemeFindExam(ref.examId):null;
  if(!ex){ showToast('⚠️ Bu deneme artık mevcut değil (silinmiş olabilir).'); return; }
  var cat=(typeof denemeFindCat==='function')?denemeFindCat(ex.catId):null;
  if(cat&&cat.hidden){
    cat.hidden=false;denemeSaveCats();
    showToast('🔓 "'+cat.name+'" kategorisi tekrar görünür yapıldı.');
  }
  closeCalendarPanel();
  // (2026-09) Bu denemenin sonucu kaydedilince (denemeSaveScore) görevi
  // otomatik tamamla — bkz. features.js _todoAutoCompleteItem.
  if(!it.done) _todoActiveDenemeTask={dateStr:dateStr,itemId:itemId,examId:ref.examId};
  if(ex.completed) denemeOpenScoreModal(ex.id);
  else denemeLaunchExam(ex.id);
}

/* ── EKLEME FORMU: VİDEO SEÇİCİ (kendi playlistlerimizden VEYA doğrudan
   YouTube linki yapıştırarak) — notes.js openNoteVideoPicker ile AYNI desen,
   sadece hedefi bir todo görevi. ── */
var _todoAddPickerDate=null;
function todoOpenVideoPickerForAdd(dateStr){
  _todoAddPickerDate=dateStr;
  var linkInp=document.getElementById('todoVideoPickerLink');if(linkInp)linkInp.value='';
  var searchInp=document.getElementById('todoVideoPickerSearch');if(searchInp)searchInp.value='';
  renderTodoVideoPickerList();
  document.getElementById('todoVideoPickerOverlay').classList.add('open');
}
function closeTodoVideoPicker(){ document.getElementById('todoVideoPickerOverlay').classList.remove('open'); }
function renderTodoVideoPickerList(){
  var q=(document.getElementById('todoVideoPickerSearch').value||'').trim().toLowerCase();
  var listEl=document.getElementById('todoVideoPickerList');
  listEl.innerHTML='';
  var any=false;
  playlists.forEach(function(pl){
    ...
    items.forEach(function(it){
      ...
      row.innerHTML='<img class="note-picker-item-thumb" src="https://i.ytimg.com/vi/'+it.id+'/mqdefault.jpg" loading="lazy" onerror="this.style.visibility=\'hidden\'">'+
      ...
      row.addEventListener('click',function(){
        _todoSetPendingVideoRef(pl.id,it.id,it.title);
        closeTodoVideoPicker();
      });
      listEl.appendChild(row);
    });
  });
  if(!any) listEl.innerHTML='<div class="note-picker-empty">Eşleşen video bulunamadı. İstersen üstten doğrudan bir YouTube linki de yapıştırabilirsin.</div>';
}
function todoVideoPickerUseLink(){
  var linkInp=document.getElementById('todoVideoPickerLink');
  var val=linkInp?linkInp.value.trim():'';
  if(!val){ showToast('⚠️ Önce bir YouTube linki yapıştır.'); return; }
  var vid=extractVideoId(val);
  if(!vid){ showToast('❌ Geçersiz YouTube linki!'); return; }
  // Video zaten bir playlist'te mi? Öyleyse ORAYI referans al (dedup) —
  // değilse kalıcı olmayan "todolist" playlistine ekle (bkz. istem).
  var foundPl=null;
  playlists.forEach(function(pl){ if(!foundPl && pl.items.some(function(it){return it.id===vid;})) foundPl=pl; });
  var title='Video '+vid;
  if(foundPl){
    var existing=foundPl.items.find(function(it){return it.id===vid;});
    title=existing.title;
    _todoSetPendingVideoRef(foundPl.id,vid,title);
    closeTodoVideoPicker();
    fetchTitle(vid); // başlık hâlâ "Yükleniyor..." olabilir, taze çek
  } else {
    var auto=_todoEnsureAutoPlaylist();
    auto.items.push({id:vid,title:'Yükleniyor...',watched:false});
    saveAll();
    fetchTitle(vid);
    _todoSetPendingVideoRef(auto.id,vid,'Yükleniyor...');
    closeTodoVideoPicker();
    showToast('🎬 Video "'+TODO_AUTO_PLAYLIST_NAME+'" playlistine eklendi.');
  }
}
function _todoSetPendingVideoRef(plId,vidId,title){
  if(!_todoAddPickerDate)return;
  if(!_todoPendingRef[_todoAddPickerDate])_todoPendingRef[_todoAddPickerDate]={};
  _todoPendingRef[_todoAddPickerDate].videoRef={plId:plId,vidId:vidId,title:title};
  var sid=_todoAddPickerDate.replace(/-/g,'');
  var lbl=document.getElementById('todoAddPickedVideo_'+sid);
  if(lbl) lbl.textContent='🎬 '+title;
}

/* ── EKLEME FORMU: DENEME SEÇİCİ — kendi deneme kategorilerimizden/
   denemelerimizden seçim, notes.js video seçiciyle AYNI görsel desen. ── */
function todoOpenDenemePickerForAdd(dateStr){
  _todoAddPickerDate=dateStr;
  var searchInp=document.getElementById('todoDenemePickerSearch');if(searchInp)searchInp.value='';
  renderTodoDenemePickerList();
  document.getElementById('todoDenemePickerOverlay').classList.add('open');
}
function closeTodoDenemePicker(){ document.getElementById('todoDenemePickerOverlay').classList.remove('open'); }
function renderTodoDenemePickerList(){
  var q=(document.getElementById('todoDenemePickerSearch').value||'').trim().toLowerCase();
  var listEl=document.getElementById('todoDenemePickerList');
  listEl.innerHTML='';
  var any=false;
  (typeof denemeCats!=='undefined'?denemeCats:[]).forEach(function(cat){
    var exams=(typeof denemeExamsInCat==='function')?denemeExamsInCat(cat.id):[];
    var filtered=exams.filter(function(ex){
      if(!q)return true;
      return ex.isim.toLowerCase().indexOf(q)>=0||cat.name.toLowerCase().indexOf(q)>=0;
    });
    if(filtered.length===0)return;
    any=true;
    var label=document.createElement('div');
    label.className='note-picker-group-label';
    label.textContent=cat.name+(cat.hidden?' 🔒':'');
    listEl.appendChild(label);
    filtered.forEach(function(ex){
      var row=document.createElement('div');
      row.className='note-picker-item';
      var netTxt=ex.result?('Net: '+denemeFmtNetPlain(ex.result.totalNet)):'Başlatılmadı';
      row.innerHTML='<div class="note-picker-item-icon">'+(ex.completed?'✅':'📝')+'</div>'+
        '<div class="note-picker-item-info">'+
          '<div class="note-picker-item-title">'+escapeHtml(ex.isim)+'</div>'+
          '<div class="note-picker-item-sub">'+escapeHtml(cat.name)+' — '+netTxt+'</div>'+
        '</div>';
      row.addEventListener('click',function(){
        if(!_todoAddPickerDate)return;
        if(!_todoPendingRef[_todoAddPickerDate])_todoPendingRef[_todoAddPickerDate]={};
        _todoPendingRef[_todoAddPickerDate].denemeRef={examId:ex.id,catId:cat.id,title:ex.isim};
        var sid=_todoAddPickerDate.replace(/-/g,'');
        var lbl=document.getElementById('todoAddPickedDeneme_'+sid);
        if(lbl) lbl.textContent='📝 '+ex.isim;
        closeTodoDenemePicker();
      });
      listEl.appendChild(row);
    });
  });
  if(!any) listEl.innerHTML='<div class="note-picker-empty">'+(q?'Eşleşen deneme bulunamadı.':'Henüz hiç deneme eklemedin. Önce Deneme Modu\'ndan bir deneme oluştur.')+'</div>';
}

function todoSetViewMode(mode){
  todoViewMode = (mode==='table') ? 'table' : 'list';
  try{ localStorage.setItem(LS_TODO_VIEWMODE, todoViewMode); }catch(e){}
  renderCalBody();
}

/* ══════════ YENİ HAFTALIK TABLO GÖRÜNÜMÜ (ekli "Haftalık Planım" görseline
   göre) — kullanıcı isteğiyle eski basit 6 sütunlu tablonun YERİNE, "Liste"
   adını alan görünüme (bkz. _todoRenderListRow) taşındı. Haftalar Pazartesi-
   Pazar, sadece bugünün haftası ve gelecek haftalara gidilebilir (bkz.
   todoWeekNav). Görselde HER saat satırı vardı ama BURADA öyle değil — bir
   göreve saat girilmediyse hiç saat yazmaz, boş satır/saat ızgarası yok. ══════════ */
var todoWeekAddOpenDate=null;
function todoToggleWeekAdd(dateStr){
  todoWeekAddOpenDate=(todoWeekAddOpenDate===dateStr)?null:dateStr;
  renderCalBody();
}
function _todoWeekRangeLabel(monday){
  return _todoShortDate(monday)+' – '+_todoShortDate(_todoAddDaysStr(monday,6));
}
function _todoShortDate(d){ var p=d.split('-'); return p[2]+'.'+p[1]; }

function _todoRenderWeeklyTable(){
  var monday=_todoWeekMonday(todoWeekOffset);
  var dowNames=['Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi','Pazar'];
  var today=getTodayStr();
  var html='<div class="todo-week-wrap">';
  html+='<div class="todo-week-nav">'+
    '<button class="todo-week-nav-btn" onclick="todoWeekNav(-1)"'+(todoWeekOffset<=0?' disabled':'')+' title="'+(todoWeekOffset<=0?'Güncel haftanın öncesine gidemezsin':'Önceki Hafta')+'">‹</button>'+
    '<div class="todo-week-nav-label">'+_todoWeekRangeLabel(monday)+(todoWeekOffset===0?' <span class="todo-today-pill">Bu Hafta</span>':'')+'</div>'+
    '<button class="todo-week-nav-btn" onclick="todoWeekNav(1)" title="Sonraki Hafta">›</button>'+
  '</div>';
  html+='<div class="todo-week-grid">';
  for(var i=0;i<7;i++){
    var d=_todoAddDaysStr(monday,i);
    var inWindow=_todoInWindow(d);
    var day=todosData[d];
    var items=day?day.items.slice().sort(_todoItemSortCmp):[];
    html+='<div class="todo-week-col'+(d===today?' is-today':'')+(inWindow?'':' out-of-window')+'">';
    html+='<div class="todo-week-col-head"><div class="todo-week-col-dow">'+dowNames[i]+(d===today?' • Bugün':'')+'</div><div class="todo-week-col-date">'+_todoShortDate(d)+'</div></div>';
    html+='<div class="todo-week-col-body">';
    if(items.length===0){
      html+='<div class="todo-week-empty">'+(inWindow?'Görev yok':'Plan dışı')+'</div>';
    } else {
      items.forEach(function(it){ html+=_todoRenderWeekCard(d,it); });
    }
    html+='</div>';
    if(inWindow){
      if(todoWeekAddOpenDate===d){
        html+='<div class="todo-week-add-inline">'+_todoRenderAddForm(d)+
          '<button class="todo-week-add-close" onclick="todoToggleWeekAdd(\''+d+'\')">✕ Kapat</button></div>';
      } else {
        html+='<button class="todo-week-add-btn" onclick="todoToggleWeekAdd(\''+d+'\')">＋ Görev Ekle</button>';
      }
    }
    html+='</div>';
  }
...
/* Haftalık pano kartı — istem: "ders adını en başa yazsın |Biyoloji| sonra
   görev yazsın {30 dk tekrar}, altına dk'si yazılsın". Saat sadece görevde
   GERÇEKTEN varsa gösterilir (görseldeki gibi her satırı doldurmuyoruz). */
function _todoRenderWeekCard(dateStr,it){
  var meta=_todoKindMeta(it.kind);
  var isRefKind=(it.kind==='video'||it.kind==='deneme');
  var bodyClickAttr='',bodyCursor='';
  if(it.kind==='video' && it.videoRef){ bodyClickAttr=' onclick="todoOpenVideoRef(\''+dateStr+'\',\''+it.id+'\')"'; bodyCursor=' style="cursor:pointer"'; }
  else if(it.kind==='deneme' && it.denemeRef){ bodyClickAttr=' onclick="todoOpenDenemeRef(\''+dateStr+'\',\''+it.id+'\')"'; bodyCursor=' style="cursor:pointer"'; }
  else if(!isRefKind && it.minutes && !it.done){ bodyClickAttr=' onclick="todoStartTimer(\''+dateStr+'\',\''+it.id+'\')"'; bodyCursor=' style="cursor:pointer"'; }
  var titleStyle=it.titleColor?(' style="color:'+it.titleColor+'"'):'';
  var subjHtml=(!isRefKind&&it.subject)?('<div class="todo-week-card-subject" style="'+(it.subjectColor?('color:'+it.subjectColor):'')+'">|'+escapeHtml(it.subject)+'|</div>'):'';
  var refWarn=((it.kind==='video'&&!it.videoRef)||(it.kind==='deneme'&&!it.denemeRef))?'<div class="todo-week-card-warn">⚠️ atanmadı</div>':'';
  return '<div class="todo-week-card'+(it.done?' done':'')+'" id="todoitem-'+dateStr+'-'+it.id+'" draggable="true" '+
    'ondragstart="todoDragStart(event,\''+dateStr+'\',\''+it.id+'\')" ondragend="todoDragEnd(event)" '+
    'ondragover="todoDragOver(event,\''+dateStr+'\',\''+it.id+'\')" ondragleave="todoDragLeave(event)" '+
    'ondrop="todoDrop(event,\''+dateStr+'\',\''+it.id+'\')">'+
    '<div class="todo-week-card-headrow">'+
      '<span class="todo-drag-handle" title="Sürükle">⠿</span>'+
      (it.time?'<span class="todo-week-card-time">🕐 '+it.time+'</span>':'<span></span>')+
      '<button class="todo-item-del" onclick="event.stopPropagation();todoDeleteItem(\''+dateStr+'\',\''+it.id+'\')" title="Sil">🗑</button>'+
    '</div>'+
    '<div class="todo-week-card-body"'+bodyClickAttr+bodyCursor+'>'+
      subjHtml+
      '<div class="todo-week-card-title"'+titleStyle+'>'+meta.icon+' '+escapeHtml(it.title)+'</div>'+
      (it.minutes?'<div class="todo-week-card-dur">'+it.minutes+' dk</div>':'')+
      refWarn+
    '</div>'+
    '<div class="todo-week-card-footrow">'+
      '<div class="todo-item-check'+(it.done?' checked':'')+'" onclick="event.stopPropagation();todoToggleDone(\''+dateStr+'\',\''+it.id+'\')" title="Tamamlandı">'+(it.done?'✓':'')+'</div>'+
      '<button class="todo-color-btn" onclick="event.stopPropagation();todoOpenTitleColorPicker(event,\''+dateStr+'\',\''+it.id+'\')" title="Görev Adı Rengi">🎨</button>'+
      (!isRefKind?'<button class="todo-color-btn" onclick="event.stopPropagation();todoOpenSubjectColorPicker(event,\''+dateStr+'\',\''+it.id+'\')" title="Ders Adı Rengi">🎨</button>':'')+
    '</div>'+
  '</div>';
}

function todoToggleDone(dateStr,itemId){
  var day=todosData[dateStr]; if(!day) return;
  var it=day.items.find(function(x){return x.id===itemId;}); if(!it) return;
  it.done=!it.done;
  it.doneAt = it.done ? Date.now() : null;
  if(todoActiveTimer && todoActiveTimer.itemId===itemId) todoCancelTimer(false);
  saveTodos();
  renderCalBody();
}

/* (2026-09) Görev — video %90 izlenince (core.js) VEYA bağlı deneme sonucu
   kaydedilince (deneme.js) otomatik "yapıldı" işaretlemek için ORTAK yardımcı.
   todoTimerComplete() ile AYNI kalıp: zaten tamamlanmışsa dokunmaz. */
function _todoAutoCompleteItem(dateStr,itemId){
  var day=todosData[dateStr]; if(!day) return;
  var it=day.items.find(function(x){return x.id===itemId;});
  if(!it || it.done) return;
  it.done=true; it.doneAt=Date.now();
  saveTodos();
  showToast('✅ "'+(it.title||'Görev')+'" otomatik tamamlandı işaretlendi!');
  renderCalBody();
}

function todoDeleteItem(dateStr,itemId){
  var day=todosData[dateStr]; if(!day) return;
...
/* ── KOLAY SİLME (bugün / önümüzdeki hafta / 30 gün) ── */
function _todoRenderFooterActions(){
  return '<div class="todo-footer-actions">'+
    '<div class="todo-footer-label">🗑 Kolay Silme</div>'+
    '<div class="todo-footer-btns">'+
      '<button class="todo-del-scope-btn" onclick="todoConfirmDeleteScope(\'today\')">Bugünü Sil</button>'+
      '<button class="todo-del-scope-btn" onclick="todoConfirmDeleteScope(\'week\')">Önümüzdeki Haftayı Sil</button>'+
      '<button class="todo-del-scope-btn danger" onclick="todoConfirmDeleteScope(\'all\')">30 Günün Tümünü Sil</button>'+
    '</div>'+
  '</div>';
}
function todoConfirmDeleteScope(scope){
  _todoDeleteScopePending=scope;
  var label = scope==='today' ? 'bugünkü plan verilerini' : (scope==='week' ? 'önümüzdeki 7 günün plan verilerini' : 'önümüzdeki 30 günün TÜM plan verilerini');
  var el=document.getElementById('todoDeleteConfirmText');
  if(el) el.textContent='Bu işlem '+label+' kalıcı olarak silecek. Bu işlem geri alınamaz. Emin misin?';
  var ov=document.getElementById('todoDeleteConfirmOverlay');
  if(ov) ov.classList.add('open');
}
function todoCloseDeleteConfirm(){
  var ov=document.getElementById('todoDeleteConfirmOverlay'); if(ov) ov.classList.remove('open');
  _todoDeleteScopePending=null;
}
function todoExecuteDeleteScope(){
  var scope=_todoDeleteScopePending;
  if(!scope){ todoCloseDeleteConfirm(); return; }
  var today=getTodayStr();
  var endStr = scope==='today' ? today : (scope==='week' ? _todoAddDaysStr(today,6) : _todoWindowEndStr());
  Object.keys(todosData).forEach(function(dateStr){
    if(dateStr>=today && dateStr<=endStr) delete todosData[dateStr];
  });
  if(todoActiveTimer && todoActiveTimer.dateStr>=today && todoActiveTimer.dateStr<=endStr) todoCancelTimer(false);
  saveTodos();
  todoCloseDeleteConfirm();
  renderCalBody();
  showToast('🗑 Silindi.');
}

/* ── ÖZEL ZAMANLAYICI (bkz. istem: "dk yazdıysa üstüne basınca zamanlayıcı
   açılsın, bitince otomatik tik atsın"). Global kronometre/pomodoro'dan
   BAĞIMSIZ, kendi başına çalışan hafif bir geri sayım — ikisi karışmasın
   diye video-izleme kronometresine hiç dokunmuyor. ── */
function todoStartTimer(dateStr,itemId){
  var day=todosData[dateStr]; if(!day) return;
  var it=day.items.find(function(x){return x.id===itemId;}); if(!it || !it.minutes || it.done) return;
  if(todoActiveTimer) todoCancelTimer(false);
  todoActiveTimer={dateStr:dateStr, itemId:itemId, totalMs:it.minutes*60000, remainingMs:it.minutes*60000, paused:false, intervalHandle:null};
  _todoOpenTimerModal();
  _todoRenderTimerModal();
  todoActiveTimer.intervalHandle=setInterval(_todoTimerTick,1000);
}
function _todoTimerTick(){
  if(!todoActiveTimer) return;
  if(!todoActiveTimer.paused) todoActiveTimer.remainingMs-=1000;
  if(todoActiveTimer.remainingMs<=0){
    todoActiveTimer.remainingMs=0;
    _todoRenderTimerModal();
    todoTimerComplete();
    return;
  }
  _todoRenderTimerModal();
  // Panel kapalıyken/başka sekmedeyken de banner güncel kalsın diye,
  // ana planlayıcı görünümdeyse arka plandaki banner'ı da tazele.
  var banner=document.querySelector('.todo-active-timer-banner');
  if(banner) banner.innerHTML='⏱ Aktif zamanlayıcı: <b></b> — '+_todoMsToClock(todoActiveTimer.remainingMs)+' kaldı <span class="todo-active-timer-open">Göster ›</span>';
}
function _todoRenderTimerModal(){
  if(!todoActiveTimer) return;
  var day=todosData[todoActiveTimer.dateStr];
  var it=day ? day.items.find(function(x){return x.id===todoActiveTimer.itemId;}) : null;
  var titleEl=document.getElementById('todoTimerTitle'); if(titleEl) titleEl.textContent=it?it.title:'';
  var timeEl=document.getElementById('todoTimerTime'); if(timeEl) timeEl.textContent=_todoMsToClock(todoActiveTimer.remainingMs);
  var ring=document.getElementById('todoTimerRingFg');
  if(ring){
    var pct = todoActiveTimer.totalMs>0 ? (todoActiveTimer.remainingMs/todoActiveTimer.totalMs) : 0;
    var circumference = 2*Math.PI*54;
    ring.style.strokeDasharray = circumference;
    ring.style.strokeDashoffset = circumference*(1-pct);
  }
  var btn=document.getElementById('todoTimerPauseBtn'); if(btn) btn.textContent = todoActiveTimer.paused ? '▶ Devam Et' : '⏸ Duraklat';
}
function _todoMsToClock(ms){
  var totalSec=Math.max(0,Math.round(ms/1000));
  return pad(Math.floor(totalSec/60))+':'+pad(totalSec%60);
}
function todoTimerPauseResume(){
  if(!todoActiveTimer) return;
  todoActiveTimer.paused=!todoActiveTimer.paused;
  _todoRenderTimerModal();
}
function todoTimerComplete(){
  if(!todoActiveTimer) return;
  var dateStr=todoActiveTimer.dateStr, itemId=todoActiveTimer.itemId;
  clearInterval(todoActiveTimer.intervalHandle);
  var day=todosData[dateStr];
  var it=day ? day.items.find(function(x){return x.id===itemId;}) : null;
  if(it){ it.done=true; it.doneAt=Date.now(); saveTodos(); }
  todoActiveTimer=null;
  _todoCloseTimerModalUI();
  showToast('⏰ Süre doldu — "'+(it?it.title:'')+'" otomatik tamamlandı işaretlendi!');
  if(typeof beep==='function') beep([880,660],[0,450]);
  renderCalBody();
}
function todoCancelTimer(showMsg){
  if(!todoActiveTimer) return;
  clearInterval(todoActiveTimer.intervalHandle);
  todoActiveTimer=null;
  _todoCloseTimerModalUI();
  if(showMsg){ showToast('Zamanlayıcı durduruldu.'); renderCalBody(); }
}
function _todoOpenTimerModal(){
  var ov=document.getElementById('todoTimerOverlay');
  if(ov) ov.classList.add('open');
  _todoRenderTimerModal();
}
function _todoCloseTimerModalUI(){
  var ov=document.getElementById('todoTimerOverlay');
  if(ov) ov.classList.remove('open');
}
function todoTimerHideModal(){
  // Sadece modali gizle — zamanlayıcı arka planda ÇALIŞMAYA DEVAM eder
  // (bkz. üstteki banner). Kullanıcı istemeden kapatınca ilerlemeyi kaybetmesin.
  _todoCloseTimerModalUI();
}

/* ══════════ [BİR KENARA KONDU — 2026-08] İleri tarihe PLAYLIST/VİDEO planlama ══════════
 * Kullanıcı isteğiyle "Planlayıcı" sekibinin ana görünümünden kaldırıldı (bkz.
 * renderCalBody → artık renderTodoPlannerTab çağırıyor), YENİ günlük to-do list
 * sistemi lehine. Bu fonksiyon ve altındaki togglePlannerPl/addPlannerVideo/
 * removePlannerVideo/vb. TAMAMEN SİLİNMEDİ — sadece hiçbir yerden çağrılmıyor.
 * plannerData (LS_PLANNER) verisi de HÂLÂ yükleniyor/duruyor, kaybolmadı.
 * İleride tekrar bir sekme/buton ile bağlanmak istenirse bu fonksiyonlar
 * hazır durumda. */
...
function renderPlannerDayDetail(dateStr){
  ...
  var planned=getPlannedForDate(dateStr);
  if(!planned&&dateStr.slice(0,7)<today.slice(0,7)){
    // Bu ay artık geçmişte kaldığı için plannerData'dan arşive taşınmış olabilir
    // (bkz. checkMonthRolloverForPlanner) — salt-okunur özet göster.
    var archDay=archiveData[dateStr];
    if(archDay&&archDay.planner&&archDay.planner.detail){
      planned={playlists:archDay.planner.detail.playlists||[],extraVideos:archDay.planner.detail.extraVideos||[]};
      var d0=new Date(dateStr.replace(/-/g,'/'));
      var dayNames0=['Pazar','Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi'];
      var monthNames0=['Ocak','Şubat','Mart','Nisan','Mayıs','Haziran','Temmuz','Ağustos','Eylül','Ekim','Kasım','Aralık'];
      return '<div class="planner-detail-panel">'+
        '<div class="planner-detail-header">'+
          '<div class="planner-detail-date">'+dayNames0[d0.getDay()]+', '+d0.getDate()+' '+monthNames0[d0.getMonth()]+' '+d0.getFullYear()+' <span style="font-size:10px;color:var(--muted)">Arşivlendi</span></div>'+
        ...
        '</div>'+
        '<div style="font-size:11px;color:var(--muted);padding:6px 0">📦 Bu ay artık geçmişte — plan sadece görüntülenebilir, düzenlenemez.</div>'+
        '<div style="font-size:11px;color:var(--text2)">Planlanan: '+archDay.planner.summary.plannedCount+' öğe</div>'+
      '</div>';
    }
    planned={playlists:[]};
  }
  planned=planned||{playlists:[]};
...
function mgrDeleteVideo(plId,idx){
  var pl=playlists.find(function(p){return p.id===plId;});if(!pl)return;
  var removed=pl.items[idx];
  pl.items.splice(idx,1);saveAll();renderPlaylist();
  if(calActiveTab==='manager')renderCalBody();
  // DRIVE ASSET STORAGE: video silinince Drive'a taşınmış ekleri de temizle
  if(removed&&removed.attachments)removed.attachments.forEach(function(a){if(typeof ahaIsAssetPointer==='function'&&ahaIsAssetPointer(a.data)&&typeof ahaDeleteAssetFromDrive==='function')ahaDeleteAssetFromDrive(a.data);});
}
...
var _ytTokenRefreshTimer = null;

/* SESSİZ TOKEN YENİLEME: mevcut access token süresi dolmadan ~5 dakika önce,
   kullanıcı hiçbir şey fark etmeden (popup YOK) otomatik olarak yenile.
   Amaç: uzun bir çalışma oturumunda (sekme açık kalırken) token'ın ortasında
   sessizce geçersiz hâle gelip "giriş yapmalısın" hatasına yol açmasını önlemek.
   Hiçbir yeni gizli bilgi SAKLANMIYOR — sadece zaten var olan e-posta ipucuyla
   aynı sessiz akış (prompt:'none') biraz erkenden tekrar tetikleniyor. */
function _ytScheduleTokenRefresh(expiresInSec){
  if (_ytTokenRefreshTimer) { clearTimeout(_ytTokenRefreshTimer); _ytTokenRefreshTimer = null; }
  if (!expiresInSec || !_musicLastEmail) return;
  var refreshInMs = Math.max((expiresInSec - 300) * 1000, 30000); // en az 30sn sonra, normalde süre dolmadan ~5dk önce
  _ytTokenRefreshTimer = setTimeout(function(){
    if (_ytTokenClient && _musicLastEmail) {
...
function initGoogleAuth() {
  ...
  _ytTokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: function(resp) {
      ...
      _ytScheduleTokenRefresh(resp.expires_in);
      ...
      /* Giriş beklerken açık kalmış bir Drive dosya önizlemesi varsa otomatik tekrar dene
         (bkz. notes.js noteBlockOpenPdfPreview / deneme.js _dnmResolveExamPdf) */
      var _pdfOv = document.getElementById('notePdfPreviewOv');
      if (_pdfOv && _pdfOv.classList.contains('open') && typeof _pdfOv._retry === 'function') _pdfOv._retry();
      var _dnmOv = document.getElementById('dnmPdfChoiceOverlay');
      if (_dnmOv && _dnmOv.classList.contains('open') && typeof _dnmHidePdfChoiceBusy === 'function') _dnmHidePdfChoiceBusy();
...
function updateGoogleAuthUI(loggedIn) {
  ...
  // DRIVE ASSET STORAGE: girişten kısa süre sonra bir kez yetim dosya temizliği yap
  if(loggedIn && typeof ahaReconcileDriveAssets === 'function') setTimeout(ahaReconcileDriveAssets, 3000);
  // OTOMATİK YEDEKLEME: reconcile'dan biraz sonra, girişi yavaşlatmadan, sessizce
  if(loggedIn && typeof ahaAutoBackupIfDue === 'function') setTimeout(ahaAutoBackupIfDue, 8000);
}
...
/* Yedeklenecek tüm localStorage key'leri.
   API anahtarları (aha_ai_key_v1, aha_user_yt_key_v1) kasıtlı olarak
   hariç tutulmuştur — bunlar cihaza özeldir ve Drive'da saklanmamalıdır. */
var BACKUP_KEYS = [
  'aha_v4_data',          // Playlistler, videolar, OOP öğeleri, ayarlar
  'aha_v4_archive',       // BİRLEŞİK GÜNLÜK ARŞİV — geçmiş özet+detay, planlayıcı özet+detay (sonsuza dek saklanan özetler dahil)
  'aha_v4_history',       // Eski geçmiş sistemi — artık sadece göç kaynağı, geriye dönük uyumluluk için hâlâ yedekleniyor
  'aha_v4_planner',       // Planlayıcı / takvim verileri (SADECE bu ay + gelecek ay — geçmiş aylar artık archive'da)
  'aha_v4_lastdate',      // Gece yarısı sıfırlama için son tarih
  'aha_v4_lastmonth',     // Planlayıcı ay-geçişi arşivleme için son ay
  'aha_notes_v1',         // Notlar (tüm sayfalar)
  'aha_ai_chat_v1',       // AI sohbet geçmişi
  'aha_flashcards_v1',    // Flashcard'lar
  'aha_flashcards_cats_v1', // Flashcard kategorileri
  /* BUG FIX: Deneme (pratik sınav) modülünün 3 localStorage anahtarı bu listede
   * hiç yoktu — yani Drive'a yedekle / Drive'dan geri yükle, playlist/not/flashcard
   * verisini korurken sınav kategorilerini VE sınav kayıtlarını tamamen atlıyordu.
   * Yeni cihaza geçişte veya geri yüklemede tüm deneme geçmişi sessizce kaybolurdu. */
  'aha_deneme_v1',        // Deneme (pratik sınav) kayıtları
  'aha_deneme_cats_v1',   // Deneme kategorileri
  'aha_deneme_mode_v1',   // Son seçili mod ('playlist' | 'deneme')
  /* Su Takibi: aha_water_v1 günlük log/toplamı da taşır ama bu SORUN DEĞİL —
   * water.js → loadWaterData() her yüklemede tarihi kontrol edip gün
   * eşleşmiyorsa logları otomatik temizler, yani eski bir yedekten geri
   * yüklenen bayat bir günlük log asla sızmaz; sadece hedef/ölçek tercihi
   * (goalMl/unitMl) kalıcı olarak taşınmış olur. */
  'aha_water_enabled_v1', // Su takibi üst bar widget'ı açık mı (varsayılan: kapalı)
  'aha_water_v1',         // Su takibi: {date, goalMl, unitMl, logs[]} — bkz. water.js
  'aha_note_symbol_shortcuts_v1', // Notlar: orta-tık ile atanmış sembol->kısayol eşleşmeleri — bkz. notes.js SEMBOL KISAYOLLARI
  'aha_v4_todos',         // Günlük Çalışma Planı (to-do list) — SADECE canlı 30 günlük pencere; gün bitince aha_v4_archive'a taşınır (bkz. checkMidnightReset)
  'aha_todo_viewmode_v1', // To-do list'te son seçili görünüm ('list' | 'table')
  'aha_stop_daily_hide_v1', // Ayarlar > "Günlük playlist & deneme gizlemesini durdur" — bkz. checkMidnightReset
  'aha_sw_independent_v1', // Ayarlar > "Kronometreyi Videodan Bağımsız Çalıştır" — bkz. core.js handleStateChange
  'aha_music_v2' // Müzik çalar: YouTube playlistleri (link ile eklenenler dahil), parça listesi, karışık/tekrar ayarı — bkz. music.js
];
...
var AHA_AUTO_BACKUP_MIN_INTERVAL_MS = 20 * 60 * 60 * 1000; // ~20 saat — fiilen günde bir, giriş saatine göre kayabilir
var AHA_AUTO_BACKUP_LS_KEY = 'aha_last_auto_backup_v1';

/* OTOMATİK YEDEKLEME (sessiz): backupToDrive() ile AYNI yükleme mantığı ama
   HİÇBİR UI gürültüsü yok (toast/buton yanıp sönmesi YOK) — kullanıcı fark
   etmeden arka planda çalışır. Sadece son otomatik yedeklemenin üzerinden
   yeterince zaman geçtiyse (~1 gün) bir şey yapar. Böylece kullanıcı manuel
   "Yedekle" butonuna hiç basmasa bile veri kaybı riski büyük ölçüde azalır. */
function ahaAutoBackupIfDue(){
  if (!_ytAccessToken) return;
  var last = 0;
  try { last = parseInt(localStorage.getItem(AHA_AUTO_BACKUP_LS_KEY) || '0', 10) || 0; } catch (e) {}
  if (Date.now() - last < AHA_AUTO_BACKUP_MIN_INTERVAL_MS) return; // çok yakın zamanda zaten yapıldı
  var token = _ytAccessToken;
  var payload = JSON.stringify(getAllLocalData(), null, 2);
  var blob = new Blob([payload], { type: 'application/json' });
  findDriveBackupFile(token).then(function(fileId){
    if (fileId) {
      ...
      return fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id', {
        ...
        headers: { Authorization: 'Bearer ' + token },
        body: form
      }).then(function(r){ return r.json(); }).then(function(d){ _driveFileId = d.id; });
    }
  }).then(function(){
    try { localStorage.setItem(AHA_AUTO_BACKUP_LS_KEY, String(Date.now())); } catch (e) {}
  }).catch(function(){ /* sessizce vazgeç — bu otomatik bir denemeydi, kullanıcıyı rahatsız etme; bir sonraki girişte tekrar denenir */ });
}
...
function restoreFromDrive() {
  ...
  if (!confirm('⚠️ Mevcut tüm veriler (notlar, flashcard\'lar, planlayıcı, geçmiş, sohbet) Drive\'daki yedekle değiştirilecek. Devam et?')) return;
  var token = _ytAccessToken;
  findDriveBackupFile(token)
  .then(function(fileId) {
...
function setDriveBtn(state, text) {
  var btn = document.getElementById('settingsDriveBackupBtn');
  if (!btn) return;
...
/* ══════════ DRIVE ASSET STORAGE — Büyük Veri Taşması (Overflow) ══════════
   localStorage TÜM aha_* anahtarları arasında PAYLAŞILAN tek bir kotaya
   sahiptir (tarayıcıya göre toplam ~5-10MB). Büyük bir PDF/görsel base64'e
   çevrilip bu kotayı aşarsa saveAll() eskiden sessizce (catch(e){}) HİÇBİR ŞEY
   kaydetmiyordu — kullanıcı fark etmeden o oturumdaki tüm değişiklikleri
   kaybediyordu.

   Bu sistem, localStorage'a sığmayan veri parçalarını kullanıcının kendi
   Google Drive'ındaki gizli appDataFolder alanına (Drive Yedekleme'nin zaten
   kullandığı 'drive.appdata' scope — ekstra izin istemez, kullanıcının normal
   Drive'ında GÖRÜNMEZ) tek tek dosyalar halinde yükler; yerel veride sadece
   küçük bir "pointer" nesnesi bırakır:
     { __ahaAsset:true, driveId, assetId, mime, size, name }
   Bu pointer'ı kullanan HER yer (ör. openAttachViewer) veriyi göstermeden hemen
   önce ahaResolveAssetData() ile asenkron çekmeli — att.data artık ya düz bir
   data-URI string'i (küçük/yerel ek, DEĞİŞMEDİ) ya da yukarıdaki pointer
   nesnesi olabilir. Var olan tüm yerel veriler string data'ya sahip olmaya
   devam eder — bu yüzden geriye dönük hiçbir migration gerekmez.

   YENİ BİR KAYDETME FONKSİYONUNA (notes.js, ai.js vb.) EKLEMEK İÇİN:
   localStorage.setItem(KEY, JSON.stringify(veri)) satırını
   ahaSafeSetItem(KEY, veri) ile değiştirmen yeterli — jenerik çalışır, hangi
   alanın "attachment" olduğunu bilmesine gerek yok; ağacı gezip data: ile
   başlayan büyük string alanları kendisi bulur. Notlardaki PDF/görsel embed'i
   ayrı bir yapıdaysa (ör. contenteditable HTML'i İÇİNE gömülü base64) bu genel
   ağaç-gezme yöntemi onu YAKALAYAMAZ — o durumda bana notes.js'i gönder, aynı
   Drive alt yapısını (ahaUploadAssetToDrive/ahaResolveAssetData) o formata özel
   bir çıkarma/yerleştirme adımıyla bağlarız. */

var AHA_ASSET_PROP_KEY = 'ahaAsset';
var AHA_OFFLOAD_MIN_BYTES = 20 * 1024;   // kota aşılınca (ya da proaktif kontrolde) bu boyuttan büyük data-URI alanları Drive adayı sayılır
  /* BUG FIX: eskiden 40KB'ydi. Canlı testte ortaya çıktı ki flashcard görselleri
     tek tek genelde 40KB'nin ALTINDA kalıyor — hiçbiri tek başına aday
     SAYILMIYORDU, ama toplamda (yüzlerce kart) 4-5MB'a kadar çıkabiliyorlardı.
     20KB, tipik sıkıştırılmış bir telefon fotoğrafı/ekran görüntüsü parçası
     için hâlâ makul bir eşik ama bu "birçok orta-küçük görsel birikir"
     senaryosunu artık yakalıyor. */
var _ahaAssetCache = {};                 // driveId -> data URL (oturum içi RAM cache — aynı dosya Drive'dan tekrar tekrar indirilmesin)

function ahaIsAssetPointer(v){
  return !!(v && typeof v === 'object' && v.__ahaAsset === true && v.driveId);
}

/* data:mime;base64,xxx → Blob (JSON içine ikinci kez base64 gömmeden, ham
   binary olarak Drive'a yüklemek için) */
function ahaDataUrlToBlob(dataUrl){
...
/* Tek bir dosyayı Drive appDataFolder'ına RESUMABLE upload ile yükler (basit/
   multipart upload büyük PDF'lerde güvenilir değil — Google'ın kendi önerisi
   budur). Sonuç: {__ahaAsset,driveId,assetId,mime,size,name} pointer'ı. */
function ahaUploadAssetToDrive(dataUrl, name, mime){
  if (!_ytAccessToken) return Promise.reject(new Error('Google Drive\'a giriş yapılmamış'));
  var token = _ytAccessToken;
  var blob = ahaDataUrlToBlob(dataUrl);
  var assetId = 'ast_' + Date.now() + '_' + uid();
  var meta = {
    name: assetId,
    parents: ['appDataFolder'],
    appProperties: { ahaAsset: '1', ahaOrigName: (name || '').slice(0, 120) }
  };
  return fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&fields=id,size', {
    method: 'POST',
    headers: {
  ...
  .then(function(r){
    if (!r.ok) return r.json().catch(function(){ return {}; }).then(function(e){
      throw new Error((e.error && e.error.message) || ('Drive oturumu başlatılamadı (' + r.status + ')'));
    });
    var loc = r.headers.get('Location');
    if (!loc) throw new Error('Drive yükleme oturumu alınamadı (Location header yok)');
    return fetch(loc, { method: 'PUT', headers: { 'Content-Type': blob.type || 'application/octet-stream' }, body: blob });
  })
  .then(function(r){
    if (!r.ok) throw new Error('Drive yükleme hatası (' + r.status + ')');
    return r.json();
  })
  .then(function(d){
    var ptr = { __ahaAsset: true, driveId: d.id, assetId: assetId, mime: mime || blob.type, size: blob.size, name: name || assetId };
    _ahaAssetCache[d.id] = dataUrl; // az önce yüklediğimiz veri zaten elimizde — tekrar indirmeye gerek yok
    return ptr;
  });
}

function ahaDeleteAssetFromDrive(ptr){
  if (!ahaIsAssetPointer(ptr)) return Promise.resolve();
  delete _ahaAssetCache[ptr.driveId];
  if (!_ytAccessToken) return Promise.resolve(); // giriş yoksa şimdilik atla — reconcile sweep girişten sonra temizler
  return fetch('https://www.googleapis.com/drive/v3/files/' + ptr.driveId, {
    method: 'DELETE',
    headers: { Authorization: 'Bearer ' + _ytAccessToken }
  }).catch(function(){});
}

/* att ya {data:'data:...'} (yerel) ya {data:pointer} (Drive'da) ya da çıplak
   pointer/string olabilir. Ekran bir eki göstermeden HEMEN ÖNCE bunu çağırmalı. */
var AHA_ASSET_FETCH_TIMEOUT_MS = 30000; // Drive indirmesi bu süreden uzun sürerse iptal edilir — ağ takılırsa sonsuza kadar (görünürde "hiç açılmayan") bekleme YAŞANMASIN

function ahaResolveAssetData(att){
  if (!att) return Promise.reject(new Error('Ek bulunamadı'));
  var d = (att && typeof att === 'object' && 'data' in att) ? att.data : att;
  if (typeof d === 'string') return Promise.resolve(d); // zaten yerel — hızlı yol, durumların büyük çoğunluğu bu
  if (!ahaIsAssetPointer(d)) return Promise.reject(new Error('Geçersiz ek verisi'));
  if (_ahaAssetCache[d.driveId]) return Promise.resolve(_ahaAssetCache[d.driveId]);
  if (!_ytAccessToken) return Promise.reject(new Error('Bu dosya Drive\'da saklanıyor — görüntülemek için Google hesabına giriş yapmalısın.'));
  var driveId = d.driveId;
  /* BUG FIX (notlardaki "tekrar girişten sonra PDF bir türlü açılmıyor"
     sorunu): eskiden bu fetch'in HİÇBİR zaman aşımı yoktu — ağ isteği
     herhangi bir sebeple (zayıf bağlantı, takılan istek vb.) askıda kalırsa
     kullanıcı süresiz biçimde "yükleniyor" gibi görünen ama asla bitmeyen bir
     bekleyişte kalıyordu, hiçbir hata/geri bildirim de gösterilmiyordu.
     AbortController ile artık AHA_ASSET_FETCH_TIMEOUT_MS sonunda istek gerçekten
     iptal edilir ve net, yakalanabilir bir hata döner. */
  var ctrl = (typeof AbortController !== 'undefined') ? new AbortController() : null;
  var timedOut = false;
  var timer = ctrl ? setTimeout(function(){ timedOut = true; ctrl.abort(); }, AHA_ASSET_FETCH_TIMEOUT_MS) : null;
  return fetch('https://www.googleapis.com/drive/v3/files/' + driveId + '?alt=media', {
    headers: { Authorization: 'Bearer ' + _ytAccessToken },
    signal: ctrl ? ctrl.signal : undefined
  })
  .then(function(r){ if (timer) clearTimeout(timer); if (!r.ok) throw new Error('Drive\'dan indirilemedi (' + r.status + ')'); return r.blob(); })
  .then(function(blob){
    return new Promise(function(resolve, reject){
      var fr = new FileReader();
      fr.onload = function(){ _ahaAssetCache[driveId] = fr.result; resolve(fr.result); };
      fr.onerror = function(){ reject(new Error('Dosya okunamadı')); };
      fr.readAsDataURL(blob);
    });
  })
  .catch(function(err){
    if (timer) clearTimeout(timer);
    if (timedOut || (err && err.name === 'AbortError')) throw new Error('Bağlantı zaman aşımına uğradı — internetini kontrol edip tekrar dene.');
    throw err;
  });
}

/* Flashcard c.img gibi {mime,data} şeklinde (data HAM base64, "data:" öneki
   YOK) saklanan görseller için. img, offload sonrası TAMAMEN pointer'la
   değişmiş olabilir (bkz. ahaFindOffloadCandidates kind:'rawimg') — bu
   fonksiyon HER İKİ durumu da (pointer veya {mime,data}) kullanılabilir tam
   bir data-URI'ye çözer. Ekran görüntülemeden önce her zaman bunu kullan. */
function ahaResolveRawImage(img){
  if (!img) return Promise.reject(new Error('Görsel yok'));
  if (ahaIsAssetPointer(img)) return ahaResolveAssetData(img);
  if (typeof img.data === 'string') return Promise.resolve('data:' + (img.mime || 'image/png') + ';base64,' + img.data);
  return Promise.reject(new Error('Geçersiz görsel verisi'));
}

/* ── HTML İçine Gömülü Asset'ler (Notlar gibi contenteditable içerikler) ──
   notePages[].html gibi TEK BİR HTML string'i İÇİNDE (ayrı bir {data:...}
   alanı olarak DEĞİL) gömülü base64 bulunabilir: <img src="data:..."> veya
   data-pdf-src="data:..." / data-raw-snapshot="data:..." gibi. Yukarıdaki
   ahaUploadAssetToDrive/ahaResolveAssetData "tüm değer bir data-URI" varsayar;
   bunlar o varsayımı YAPMADAN aynı Drive alt yapısını kullanır. */
var AHA_HTML_ASSET_RE = /(\S+)="(data:[^;"]+;base64,[A-Za-z0-9+/=]+)"/g;
var _ahaUploadDedup = {}; // dataUrl (birebir) -> driveId — aynı içerik (ör. notlarda her autosave'de DOM'dan yeniden okunan aynı base64, ya da bir çizim bloğunda data-raw-snapshot+src'de İKİ KEZ geçen aynı PNG) tekrar tekrar Drive'a yüklenmesin

/* html string'ini tarar, threshold'u aşan gömülü data-URI'leri Drive'a taşır
   ve yerlerine aha-asset://driveId koyar. Aynı data-URI HTML içinde birden
   fazla yerde geçiyorsa (çizim bloğu) TEK SEFER yüklenir, hepsi aynı
   placeholder'ı alır. Giriş yoksa veya taşınacak bir şey yoksa html'i
   OLDUĞU GİBİ döner — hiçbir zaman veri kaybına yol açmaz, sadece kota
   korumasını uygulayamaz. */
function ahaOffloadHtmlAssets(html, minBytes){
  if (!html || html.indexOf('data:') === -1) return Promise.resolve(html);
  minBytes = minBytes || AHA_OFFLOAD_MIN_BYTES;
  var seen = {};
  var jobs = [];
  var m;
  AHA_HTML_ASSET_RE.lastIndex = 0;
  while ((m = AHA_HTML_ASSET_RE.exec(html))) {
    var full = m[2];
    if (full.length <= minBytes || seen[full]) continue;
    seen[full] = true;
    jobs.push(full);
  }
  if (jobs.length === 0) return Promise.resolve(html);
  if (!_ytAccessToken) {
    showToast('⚠️ Notta büyük bir dosya var — Drive\'a taşınabilmesi için Ayarlar\'dan Google hesabına giriş yap, yoksa bu içerik kaydedilemeyebilir.');
    return Promise.resolve(html);
  }
  return Promise.all(jobs.map(function(dataUrl){
    if (_ahaUploadDedup[dataUrl]) return { dataUrl: dataUrl, driveId: _ahaUploadDedup[dataUrl] };
    var mime = (dataUrl.match(/^data:([^;]+);/) || [])[1];
    return ahaUploadAssetToDrive(dataUrl, 'not-eki', mime).then(function(ptr){
      _ahaUploadDedup[dataUrl] = ptr.driveId;
      return { dataUrl: dataUrl, driveId: ptr.driveId };
    }, function(err){
      showToast('❌ Nottaki bir dosya Drive\'a yüklenemedi: ' + (err && err.message ? err.message : ''));
      return null;
    });
    ...
    results.forEach(function(r){
      if (!r) return;
      out = out.split(r.dataUrl).join('aha-asset://' + r.driveId); // literal replace-all, regex kaçışı gerekmez
    });
    return out;
  });
}

/* containerEl (DOM'a EKLENMİŞ olmalı) içindeki aha-asset://driveId
   placeholder'larını bulup gerçek veriyle DEĞİŞTİRİR. page.html gibi
   Drive-offload edilmiş bir HTML'i innerHTML olarak enjekte eden HER yerden
   HEMEN SONRA çağrılmalı (bkz. notes.js → openNotePage). */
function ahaHydrateHtmlAssets(containerEl){
  if (!containerEl) return;
  ['src', 'data-pdf-src', 'data-raw-snapshot'].forEach(function(attr){
    containerEl.querySelectorAll('[' + attr + '^="aha-asset://"]').forEach(function(el){
      var driveId = el.getAttribute(attr).replace('aha-asset://', '');
      if (attr === 'src') el.style.opacity = '0.4';
      ahaResolveAssetData({ __ahaAsset: true, driveId: driveId }).then(function(dataUrl){
        el.setAttribute(attr, dataUrl);
        if (attr === 'src') el.style.opacity = '';
        /* BUG FIX (gereksiz tekrar-yükleme / geçici çift depolama): Drive'dan
           indirilen bu dataUrl'in HANGİ driveId'den geldiğini burada da
           kaydediyoruz — öncesinde sadece YENİ bir dosya YÜKLERKEN
           kaydediliyordu. flushCurrentNoteSave() her not kapatma/geri dönüşte
           KOŞULSUZ ahaOffloadHtmlAssets() çalıştırdığından, hydrate edilmiş
           (yani içeriği tam base64 olarak DOM'a geri konmuş) bir sayfa hiç
           değiştirilmeden kapatılsa bile, bu satır olmadan aynı büyük dosya
           HER YENİ OTURUMDA (sayfa yenilenince _ahaUploadDedup boşaldığı için)
           Drive'a sessizce tekrar yükleniyordu — yavaş + gereksiz trafik +
           reconcile temizleyene kadar geçici çift kopya. */
        _ahaUploadDedup[dataUrl] = driveId;
      }, function(){
        if (attr === 'src') { el.style.opacity = ''; el.alt = '⚠️ Drive\'dan yüklenemedi'; }
      });
    });
  });
}

/* obj ağacını gezip threshold'u aşan data-URI/base64 alanlarını bulur (en
   büyükten küçüğe sıralı döner). Şemadan bağımsızdır — belirli bir alan adı
   ARAMAZ. Üç farklı yerleşim şeklini ayırt eder:
     kind:'whole'  → alan BAŞTAN SONA bir data-URI (ör. attachments[].data) —
                      ahaUploadAssetToDrive ile TÜMÜ Drive'a taşınır, alan
                      pointer nesnesiyle değişir.
     kind:'html'   → alan İÇİNDE bir veya daha fazla gömülü data-URI var ama
                      kendisi çok daha büyük bir HTML/metin string'i (ör.
                      notePages[].html) — ahaOffloadHtmlAssets ile sadece o
                      alt-parçalar çıkarılır, geri kalan HTML olduğu gibi kalır.
     kind:'rawimg' → {mime:'image/png', data:'HAM_BASE64'} şeklinde sibling
                      alan çifti, "data:" ÖNEKİ YOK (ör. flashcard c.img).
                      mime alanının gerçek bir mime-type gibi görünmesi VE
                      data'nın SADECE base64 alfabesinden oluşması BİRLİKTE
                      aranıyor — ikisinin rastlantısal birlikte oluşma
                      ihtimali pratikte yok denecek kadar az. Tüm {mime,data}
                      nesnesi (node[k]) pointer'la değişir.
   Bu ayrım sayesinde ileride eklenecek yeni attachment türleri için özel
   kod gerekmez. */
function ahaFindOffloadCandidates(obj, minBytes){
  var found = [];
  var MIME_RE = /^[\w.+-]+\/[\w.+-]+$/;
  var B64_RE = /^[A-Za-z0-9+/]+=*$/;
  function looksLikeRawImg(v){
    return !!(v && typeof v === 'object' && !ahaIsAssetPointer(v) &&
      typeof v.mime === 'string' && MIME_RE.test(v.mime) &&
      typeof v.data === 'string' && v.data.length > minBytes && B64_RE.test(v.data));
  }
  function walk(node){
    if (!node || typeof node !== 'object') return;
    if (Array.isArray(node)) { node.forEach(walk); return; }
    if (ahaIsAssetPointer(node)) return; // zaten Drive'a taşınmış, atla
    Object.keys(node).forEach(function(k){
      var v = node[k];
      if (typeof v === 'string') {
        if (v.length <= minBytes) return;
        if (v.indexOf('data:') === 0) {
          found.push({ holder: node, key: k, name: node.name, mime: (v.match(/^data:([^;]+);/) || [])[1], kind: 'whole', size: v.length });
        } else if (v.indexOf('data:') !== -1 && /data:[^;"'\s]+;base64,/.test(v)) {
          found.push({ holder: node, key: k, kind: 'html', size: v.length });
        }
      } else if (v && typeof v === 'object') {
        if (looksLikeRawImg(v)) {
          found.push({ holder: node, key: k, name: v.name || node.title || node.soru || node.baslik, mime: v.mime, rawData: v.data, kind: 'rawimg', size: v.data.length });
        } else {
          walk(v);
        }
      }
...
var AHA_PROACTIVE_OFFLOAD_KEY_BUDGET = 400 * 1024; // bir TEK anahtar bu boyutu aşarsa, kota henüz dolmasa bile arka planda hafifletilmeye çalışılır

/* PROAKTİF OFFLOAD (canlı testte bulunan gerçek soruna karşılık): eskiden
   offload SADECE localStorage.setItem gerçekten fırlatınca (kota tamamen
   dolunca) çalışıyordu. Ama bir anahtar (ör. flashcard'lar) tek başına
   birkaç MB'a çıkıp toplam kotayı sessizce tüketebiliyordu — çünkü tarayıcı
   açısından "hâlâ sığıyor", hiçbir hata fırlamıyordu. Bu fonksiyon "artık
   sığıyor mu" değil "makul bir anahtar bütçesinin altında mı" sorusuna bakar;
   TEK bir büyük dosyayı taşıyıp durmaz, gerçekten bütçenin altına düşene ya da
   taşınabilecek bir şey kalmayana kadar candidate'ları sırayla Drive'a gönderir.
   Sessizdir (toast YOK) — kullanıcı bunun farkına varmasına gerek yok, tıpkı
   otomatik yedekleme gibi arka planda çalışır. Giriş yapılmamışsa sessizce
   hiçbir şey yapmaz; bir dahaki girişte (bir sonraki normal kayıtta) tekrar
   denenir. */
function ahaProactiveOffloadIfOversized(key, obj, currentJsonLength){
  if (!_ytAccessToken) return; // sessizce vazgeç — Drive olmadan yapılabilecek bir şey yok, ısrar etmeye gerek yok
  if (currentJsonLength <= AHA_PROACTIVE_OFFLOAD_KEY_BUDGET) return;
  var candidates = ahaFindOffloadCandidates(obj, AHA_OFFLOAD_MIN_BYTES);
  if (candidates.length === 0) return; // taşınabilecek bir şey yok (muhtemelen düz metin) — proaktif offload'un yapabileceği bir şey yok, dokunma
  candidates.sort(function(a, b){ return b.size - a.size; });
  function tryNext(i){
    if (i >= candidates.length) { try{ localStorage.setItem(key, JSON.stringify(obj)); }catch(e){} return; }
    var c = candidates[i];
    var work = (c.kind === 'html') ? ahaOffloadHtmlAssets(c.holder[c.key])
      : (c.kind === 'rawimg') ? ahaUploadAssetToDrive('data:' + c.mime + ';base64,' + c.rawData, c.name, c.mime)
      : ahaUploadAssetToDrive(c.holder[c.key], c.name, c.mime);
    work.then(function(result){
      c.holder[c.key] = result;
      var newJson;
      try{ newJson = JSON.stringify(obj); }catch(e){ newJson = ''; }
      try{ localStorage.setItem(key, newJson); }catch(e){}
      if (newJson.length > AHA_PROACTIVE_OFFLOAD_KEY_BUDGET) tryNext(i + 1); // hâlâ bütçenin üstünde — bir sonraki en büyüğü de taşı
    }, function(){ tryNext(i + 1); }); // biri başarısız olursa diğerlerine geç, sessizce
  }
  tryNext(0);
}

/* localStorage.setItem başarısız olunca (kota aşımı) çağrılır. obj İÇİNDEKİ
   playlists/oopItems gibi alanlar canlı state ile AYNI referanslardır (klon
   DEĞİLDİR) — bu yüzden burada yapılan pointer değişimi otomatik olarak
   uygulamanın kendi hafızasına da yansır, ayrıca senkronize etmeye gerek yok. */
function ahaOffloadAndSave(key, obj){
  if (!_ytAccessToken) {
    showToast('❌ Kaydedilemedi — veri çok büyük. Büyük ekleri saklayabilmek için Ayarlar\'dan Google hesabına giriş yap.');
    return Promise.resolve(false);
  }
  var candidates = ahaFindOffloadCandidates(obj, AHA_OFFLOAD_MIN_BYTES);
  if (candidates.length === 0) {
    showToast('❌ Kaydedilemedi — veri çok büyük ve Drive\'a taşınabilecek bir dosya bulunamadı.');
    return Promise.resolve(false);
  }
  showToast('☁️ Depolama dolu — büyük dosyalar Drive\'a taşınıyor...');
  function tryNext(i){
    if (i >= candidates.length) {
      showToast('❌ Büyük dosyalar Drive\'a taşınmasına rağmen veri hâlâ sığmadı.');
      return false;
    }
    var c = candidates[i];
    var work = (c.kind === 'html') ? ahaOffloadHtmlAssets(c.holder[c.key])                                        // sonuç: yer değiştirilmiş HTML string
      : (c.kind === 'rawimg') ? ahaUploadAssetToDrive('data:' + c.mime + ';base64,' + c.rawData, c.name, c.mime)   // sonuç: pointer nesnesi ({mime,data} yerine TÜMÜYLE geçer)
      : ahaUploadAssetToDrive(c.holder[c.key], c.name, c.mime);                                                    // sonuç: pointer nesnesi
    return work.then(function(result){
      ...
      try {
        ...
        return tryNext(i + 1); // hâlâ sığmadı — bir sonraki en büyük dosyayı da taşı
      }
    }, function(err){
      showToast('❌ "' + (c.name || 'veri') + '" Drive\'a taşınamadı: ' + (err && err.message ? err.message : 'bağlantı hatası'));
      return tryNext(i + 1);
    });
  }
  return tryNext(0).then(function(ok){
    if (ok) showToast('✅ Büyük dosya(lar) Drive\'a taşındı, kayıt tamamlandı.');
    return ok;
  });
}

/* localStorage.setItem'in yerine geçen genel amaçlı fonksiyon: önce normal
   yolu dener; kota aşılırsa büyük alanları otomatik Drive'a taşıyıp tekrar
   dener. HER save fonksiyonunda (saveAll, saveNotes, saveAiFlashcards vb.)
   localStorage.setItem(KEY,JSON.stringify(x)) yerine ahaSafeSetItem(KEY,x)
   kullanılabilir — şemadan bağımsız çalışır. Normal yol başarılı olsa BİLE,
   bu TEK anahtar tek başına çok büyükse (bkz. ahaProactiveOffloadIfOversized)
   arka planda sessizce hafifletilmeye çalışılır — kaydın kendisini YAVAŞLATMAZ
   ya da sonucunu ETKİLEMEZ, sadece ek bir arka plan işidir. */
function ahaSafeSetItem(key, obj){
  var json;
  try { json = JSON.stringify(obj); } catch (e) { return Promise.resolve(false); }
  try {
    localStorage.setItem(key, json);
    if (json.length > AHA_PROACTIVE_OFFLOAD_KEY_BUDGET) ahaProactiveOffloadIfOversized(key, obj, json.length);
    return Promise.resolve(true);
  } catch (e) {
    return ahaOffloadAndSave(key, obj);
  }
}

/* ── Yetim Temizliği (Reconciliation) ──
   Not: "her 5 saniyede bir karşılaştır" fikri yerine burada daha hafif bir
   yaklaşım kullanıldı — yeni veriyi Drive'a TAŞIMA işi zaten her save'de
   (yukarıdaki ahaSafeSetItem) anında ve olay-tabanlı olarak gerçekleşiyor,
   5 saniyelik bir döngüye asıl ihtiyaç duyulan şey bu değil. Asıl ihtiyaç,
   artık hiçbir yerde referans edilmeyen (video/playlist silindiğinde vb.)
   Drive dosyalarının temizlenmesi — ki bu her silme noktasını tek tek
   yakalamak yerine burada garantiye alınıyor. Bunu 5 saniyede bir yapmak
   Drive API'sini gereksiz yere yorar ve pilden yer; bunun yerine 60 saniyede
   bir + her Google girişinden kısa süre sonra çalışır. Doğrudan silmeler
   zaten anlıktır (bkz. deleteAttachment / mgrDeleteVideo / removeFromPlaylist)
   — bu sweep sadece kaçırılan durumlar (ör. tüm playlist silme) için bir
   güvenlik ağıdır. */
/* html string'i içindeki tüm aha-asset://driveId referanslarını (tekrarsız) döner. */
function ahaExtractHtmlAssetIds(html){
  var ids = [];
  if (!html) return ids;
  var re = /aha-asset:\/\/([A-Za-z0-9_-]+)/g;
  var m;
  while ((m = re.exec(html))) ids.push(m[1]);
  return ids;
}

function ahaCollectReferencedDriveIds(){
  var ids = {};
  function walk(v){
    if (!v || typeof v !== 'object') return;
    if (Array.isArray(v)) { v.forEach(walk); return; }
    if (v.__ahaAsset === true && v.driveId) { ids[v.driveId] = true; return; }
    Object.keys(v).forEach(function(k){ walk(v[k]); });
  }
  walk(playlists);
  walk(oopItems);
  /* Notlar (aha_notes_v1) ve Flashcard'lar (aha_flashcards_v1): canlı
     notePages/aiFlashcards değişkenleri yerine BİLEREK doğrudan
     localStorage'dan okunuyor — ilgili panel bu oturumda hiç açılmamışsa
     (ya da henüz init olmamışsa) canlı değişken boş/eksik kalabilir; ona
     güvenseydik henüz belleğe yüklenmemiş verilerdeki Drive dosyaları
     yanlışlıkla "yetim" sayılıp SİLİNEBİLİRDİ. */
  try {
    var notesRaw = localStorage.getItem('aha_notes_v1');
    if (notesRaw) {
      var pages = JSON.parse(notesRaw) || [];
      pages.forEach(function(p){ ahaExtractHtmlAssetIds(p.html).forEach(function(id){ ids[id] = true; }); });
    }
  } catch (e) {}
  try {
    var cardsRaw = localStorage.getItem('aha_flashcards_v1');
    if (cardsRaw) {
      var cards = JSON.parse(cardsRaw) || [];
      cards.forEach(function(c){ if (c.img && ahaIsAssetPointer(c.img)) ids[c.img.driveId] = true; });
    }
  } catch (e) {}
  /* Deneme (pratik sınav) kayıtları (aha_deneme_v1): notlar/flashcard'larla
     AYNI gerekçeyle BİLEREK doğrudan localStorage'dan okunuyor — deneme paneli
     bu oturumda hiç açılmamışsa canlı denemeExams değişkeni boş/eksik kalabilir;
     ona güvenmek, henüz belleğe yüklenmemiş sınavlardaki Drive dosyalarını
     yanlışlıkla "yetim" sayıp SİLEBİLİRDİ. Bu blok eksik olsaydı: deneme.js'e
     Drive offload eklendiğinde, yüklenen HER sınav PDF'i referanssız sanılıp
     bu sweep tarafından (girişten 3sn sonra ya da 60sn'lik döngüde) silinirdi. */
  try {
    var examsRaw = localStorage.getItem('aha_deneme_v1');
    if (examsRaw) {
      var exams = JSON.parse(examsRaw) || [];
      exams.forEach(function(ex){ if (ex.pdf && ahaIsAssetPointer(ex.pdf.data)) ids[ex.pdf.data.driveId] = true; });
    }
  } catch (e) {}
  /* AI sohbet arşivi (aha_ai_chat_v1): TIER-2 son çare arşivleme (bkz. ai.js
     ahaArchiveOldChatMessages) aiChatHistory[0]'a bir işaretleyici koyabilir —
     bu da AYNI şekilde doğrudan localStorage'dan taranmazsa, kendi az önce
     yarattığı arşiv dosyasını bu sweep birkaç dakika içinde silerdi. */
  try {
    var chatRaw = localStorage.getItem('aha_ai_chat_v1');
    if (chatRaw) {
      var chatMsgs = JSON.parse(chatRaw) || [];
      if (chatMsgs[0] && chatMsgs[0].archived && ahaIsAssetPointer(chatMsgs[0].data)) ids[chatMsgs[0].data.driveId] = true;
    }
  } catch (e) {}
  return ids;
}

function ahaReconcileDriveAssets(pageToken, referenced){
  if (!_ytAccessToken) return;
  referenced = referenced || ahaCollectReferencedDriveIds();
  var q = "appProperties has { key='" + AHA_ASSET_PROP_KEY + "' and value='1' }";
  var url = 'https://www.googleapis.com/drive/v3/files?spaces=appDataFolder&q=' + encodeURIComponent(q) +
            '&fields=' + encodeURIComponent('nextPageToken,files(id,name,size)') + '&pageSize=1000' +
  ...
  .then(function(d){
    (d.files || []).forEach(function(f){
      if (!referenced[f.id]) {
        fetch('https://www.googleapis.com/drive/v3/files/' + f.id, { method: 'DELETE', headers: { Authorization: 'Bearer ' + _ytAccessToken } }).catch(function(){});
      }
    });
    if (d.nextPageToken) ahaReconcileDriveAssets(d.nextPageToken, referenced);
  })
  .catch(function(){});
}

setInterval(ahaReconcileDriveAssets, 60000);
